...
bool Parsers::Move::Parse(
  ...
  if (nullptr == eState) {
    LogGeneralError([&]() {
      return libcomp::String(
                 "Invalid entity ID received from a move request: %1\n")
  ...
  if (!eState->CanMove() || state->IsMovementLocked(entityID)) {
    ...
    uint8_t result = zoneManager->CorrectClientPosition(
        eState, src, dest, startTime, stopTime, true);
    if (result) {
      switch (result) {
        case 1:
          LogGeneralDebug([&]() {
            return libcomp::String(
                       "Player movement rolled-back in zone %1: %2\n")
        ...
        case 2:
          LogGeneralDebug([&]() {
            return libcomp::String(
                       "Player movement corrected in zone %1: %2 ([%3, %4] => "
                       "[%5, %6] to [%7, %8])\n")
                .Arg(zone->GetDefinitionID())
                .Arg(state->GetAccountUID().ToString())
                .Arg(src.x)
  ...
  if (clientVisible || positionCorrected) {
    ...
    if (clientVisible) {
      ...
      zConnections = zoneManager->GetZoneConnections(client, positionCorrected);
    } else {
    ...
    if (zConnections.size() > 0) {
      ...
      ChannelClientConnection::SendRelativeTimePacket(zConnections, notify,
                                                      timeMap);
    }
  }
    ...
    case EntityType_t::PARTNER_DEMON:
      ...
      // character to the destination spot
      {
        ...
        if (state->GetCharacterState()->StatusEffectActive(demonOnly)) {
          zoneManager->Warp(client, state->GetCharacterState(), destX, destY,
                            0.f);
        }
      }
      break;
