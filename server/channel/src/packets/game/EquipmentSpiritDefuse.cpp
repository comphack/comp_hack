...
bool Parsers::EquipmentSpiritDefuse::Parse(
  ...
  if (!error) {
    ...
    auto basicDef = definitionManager->GetItemData(equipment->GetBasicEffect());
    auto specialDef =
        definitionManager->GetItemData(equipment->GetSpecialEffect());
    ...
    uint32_t kzCost = (uint32_t)ceil((double)costSum / 5000);
    ...
    if (characterManager->CalculateItemRemoval(client, fuseItemType,
      ...
      LogItemError([&]() {
        return libcomp::String(
                   "EquipmentSpiritDefuse request attempted with insufficient "
                   "fusion item count: %1\n")
            .Arg(state->GetAccountUID().ToString());
      });

      error = true;
    } else if (characterManager->CalculateItemRemoval(
                   client, SVR_CONST.ITEM_KREUZ, kzCost, updateItems) > 0) {
      LogItemError([&]() {
        return libcomp::String(
                   "EquipmentSpiritDefuse request attempted with insufficient "
                   "kreuz: %1\n")
  ...
  if (!error) {
    ...
    double chainBoost =
        (double)(0.325 * (floor(0.1 * cState->GetExpertiseRank(
    ...
    for (auto& pair : results) {
      ...
      if (chainBoost > 1.0 || expBoost > 1.0) {
        // CP boosts success
        double cpBoost = CharacterManager::IsCPItem(itemDef) ? 1.2 : 1.0;
    ...
    for (size_t i = 0; i < results.size(); i++) {
      ...
      if (i == basicIdx) {
        if (item != equipment) {
          ...
          auto itemDef = definitionManager->GetItemData(equipment->GetType());
          auto restr = itemDef->GetRestriction();

          for (uint8_t k = 0; k < 5; k++) {
            equipment->SetModSlots(
                (size_t)k, k < restr->GetModSlots() ? MOD_SLOT_NULL_EFFECT : 0);
          }
        }
    ...
    if (!characterManager->UpdateItems(client, false, insertItems,
      ...
      LogItemError([&]() {
        return libcomp::String(
                   "EquipmentSpiritDefuse failed to update items: %1\n")
            .Arg(state->GetAccountUID().ToString());
      });
