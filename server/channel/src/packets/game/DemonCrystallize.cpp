...
bool Parsers::DemonCrystallize::Parse(
  ...
  if (responseCode == EntrustErrorCodes_t::SUCCESS) {
    ...
    if (!updateItem) {
      ...
      if (useItem->GetStackSize() == 1 &&
          (crystalTargetSlot == -1 ||
           useItem->GetBoxSlot() < crystalTargetSlot)) {
      ...
      if (crystalTargetSlot == -1) {
        ...
        if (rewards.find(1) != rewards.end() ||
            rewards.find(2) != rewards.end()) {
  ...
  if (responseCode == EntrustErrorCodes_t::SUCCESS) {
    if (characterManager->GetSynthOutcome(state, exchangeSession, itemType,
                                          successRates)) {
  ...
  if (responseCode == EntrustErrorCodes_t::SUCCESS) {
    ...
    notify.WritePacketCode(
        ChannelToClientPacketCode_t::PACKET_DEMON_CRYSTALLIZED);
    ...
    if (!success) {
      ...
      int32_t newFam =
          (int32_t)(targetDemon->GetFamiliarity() -
      ...
      if (updateItem->GetStackSize() == 0) {
        ...
        // Increase stack
        updateItem->SetStackSize((uint16_t)(updateItem->GetStackSize() + 1));
    ...
    if (!server->GetWorldDatabase()->ProcessChangeSet(dbChanges)) {
      LogDemonErrorMsg(
          "Crystallize result failed to save, disconnecting player(s)\n");
