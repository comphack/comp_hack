...
void AccountManager::HandleLoginRequest(
  ...
  if (nullptr != account) {
    ...
    LogAccountManagerDebug([&]() {
      return libcomp::String("Logging in account '%1' with session key %2\n")
          .Arg(username)
    ...
    LogAccountManagerError([&]() {
      return libcomp::String("Account '%1' not found. Can't log them in.\n")
          .Arg(username);
    });
  }
}

void AccountManager::HandleLoginResponse(
    const std::shared_ptr<channel::ChannelClientConnection>& client) {
  auto server = mServer.lock();
  auto worldDB = server->GetWorldDatabase();
  auto state = client->GetClientState();
  ...
  if (InitializeCharacter(character, state)) {
    ...
    if (channelLogin && channelLogin->GetFromChannel() >= 0) {
      ...
      cState->SetActiveSwitchSkills(channelLogin->GetActiveSwitchSkills());

      // If the character was digitalized, set that up again
      auto dgDemon = std::dynamic_pointer_cast<objects::Demon>(
          libcomp::PersistentObject::GetObjectByUUID(
              channelLogin->GetDigitalizeDemon()));
      if (dgDemon &&
          cState->StatusEffectActive(
    ...
    cState->SetCurrentSkills(cState->GetAllSkills(definitionManager, false));
    ...
    LogAccountManagerError([&]() {
      return libcomp::String("User account could not be logged in: %1\n")
...
void AccountManager::HandleLogoutRequest(
  ...
  libcomp::Packet reply;
  reply.WritePacketCode(ChannelToClientPacketCode_t::PACKET_LOGOUT);
  reply.WriteU32Little((uint32_t)LogoutPacketAction_t::LOGOUT_PREPARE);
    ...
    case LogoutCode_t::LOGOUT_CODE_QUIT: {
      ...
      mServer.lock()->GetTimerManager()->ScheduleEventIn(
          ...
          [](std::shared_ptr<channel::ChannelClientConnection> pClient,
             uint64_t pTimeout) {
            if (pClient->GetClientState()->GetLogoutTimer() == pTimeout) {
              libcomp::Packet p;
              p.WritePacketCode(ChannelToClientPacketCode_t::PACKET_LOGOUT);
              p.WriteU32Little(
      ...
      if (channelLogin) {
        ...
        server->GetTimerManager()->ScheduleEventIn(
            ...
            [](std::shared_ptr<ChannelServer> pServer,
              ...
              if (pClient->GetClientState()->GetLogoutTimer() == pTimeout) {
                auto pChannelLogin =
                    pServer->GetAccountManager()->PrepareChannelChange(
                        pClient, 0, 0, pChannelIdx);
                auto account =
                    pClient->GetClientState()->GetAccountLogin()->GetAccount();
...
void AccountManager::Logout(
  ...
  if (!delay) {
    ...
    if (!LogoutCharacter(state)) {
      LogAccountManagerError([&]() {
        return libcomp::String("Character %1 failed to save on account %2.\n")
    ...
    // logout completes
    libcomp::ObjectReference<objects::Account>::Unload(account->GetUUID());
    libcomp::ObjectReference<objects::Character>::Unload(character->GetUUID());

    // Remove all secondary caching
    server->GetTokuseiManager()->RemoveTrackingEntities(state->GetWorldCID());
  }
}

void AccountManager::RequestDisconnect(
    const std::shared_ptr<channel::ChannelClientConnection>& client) {
  libcomp::Packet request;
  request.WritePacketCode(ChannelToClientPacketCode_t::PACKET_LOGOUT);
  request.WriteU32Little((uint32_t)LogoutPacketAction_t::LOGOUT_DISCONNECT);
...
void AccountManager::Authenticate(
    const std::shared_ptr<channel::ChannelClientConnection>& client) {
  auto state = client->GetClientState();
...
std::unordered_map<int32_t, std::shared_ptr<objects::CharacterLogin>>
AccountManager::GetActiveLogins() {
  return mActiveLogins;
}

std::shared_ptr<objects::CharacterLogin> AccountManager::GetActiveLogin(
...
void AccountManager::UpdateLogins(
  ...
  for (auto update : updates) {
    ...
    mCIDMap[update->GetCharacter().GetUUID().ToString()] =
        update->GetWorldCID();
  }
...
bool AccountManager::InitializeCharacter(
    libcomp::ObjectReference<objects::Character>& character,
  ...
  if (character.IsNull() || !character.Get(db) ||
    ...
    LogAccountManagerError([&]() {
      return libcomp::String(
                 "Character or character stats could not be initialized for "
                 "account: %1\n")
  ...
  if (newCharacter && !InitializeNewCharacter(character.Get())) {
    LogAccountManagerError([&]() {
      return libcomp::String(
                 "Failed to initialize new character for account: %1\n")
          .Arg(state->GetAccountUID().ToString());
    });
  ...
  auto worldData =
      objects::AccountWorldData ::LoadAccountWorldDataByAccount(db, account);
  if (worldData == nullptr) {
    worldData = libcomp::PersistentObject::New<objects::AccountWorldData>(true);

    worldData->SetAccount(account);

    auto itemDepo = libcomp::PersistentObject::New<objects::ItemBox>(true);

    itemDepo->SetType(objects::ItemBox::Type_t::ITEM_DEPO);
    itemDepo->SetAccount(account);
    worldData->SetItemBoxes(0, itemDepo);

    auto demonDepo = libcomp::PersistentObject::New<objects::DemonBox>(true);
    ...
    if (!itemDepo->Insert(db) || !demonDepo->Insert(db) ||
      ...
      LogAccountManagerError([&]() {
        return libcomp::String(
                   "AccountWorldData could not be created during character "
                   "initialization for account: %1\n")
            .Arg(state->GetAccountUID().ToString());
      });
  ...
  if (!worldData->GetBazaarData().IsNull()) {
    if (!worldData->LoadBazaarData(db)) {
      LogAccountManagerError([&]() {
        return libcomp::String(
                   "BazaarData %1 could not be initialized for account: %2\n")
    ...
    auto allBazaarItems =
        objects::BazaarItem::LoadBazaarItemListByAccount(db, account);
    ...
    for (size_t i = 0; i < 15; i++) {
      ...
      if (!bItem.Get()) {
        LogAccountManagerWarning([&]() {
          return libcomp::String(
                     "Clearing invalid BazaarItem %1 saved on BazaarData for "
                     "account: %2\n")
      ...
      state->SetObjectID(bItem->GetItem().GetUUID(), server->GetNextObjectID());
    ...
    if (openSlots.size() > 0) {
      ...
      if (recovered > 0) {
        LogAccountManagerWarning([&]() {
          return libcomp::String(
                     "Recovered %1 orphaned BazaarItem(s) from account: %2\n")
              .Arg(recovered)
              .Arg(state->GetAccountUID().ToString());
        });
      }
  ...
  if (!character->LoadProgress(db)) {
    LogAccountManagerError([&]() {
      return libcomp::String(
                 "CharacterProgress %1 could not be initialized for account: "
                 "%2\n")
  ...
  if (!character->LoadFriendSettings(db)) {
    LogAccountManagerError([&]() {
      return libcomp::String(
                 "FriendSettings %1 could not be initialized for account: %2\n")
  ...
  if (!character->GetCultureData().IsNull()) {
    ...
    if (!cultureData ||
        (!cultureData->GetItem().IsNull() && !cultureData->LoadItem(db))) {
      LogAccountManagerError([&]() {
        return libcomp::String(
                   "CultureData %1 could not be initialized for account: %2\n")
  ...
  // PvP
  if (!character->GetPvPData().IsNull() && !character->LoadPvPData(db)) {
    LogAccountManagerError([&]() {
      return libcomp::String(
                 "PvPData %1 could not be initialized for account: %2\n")
  ...
  for (auto itemBox : allBoxes) {
    ...
    if (!itemBox.Get(db)) {
      LogAccountManagerError([&]() {
        return libcomp::String(
                   "ItemBox %1 could not be initialized for account: %2\n")
    ...
    auto allBoxItems =
        objects::Item::LoadItemListByItemBox(db, itemBox.GetUUID());
    ...
    for (size_t i = 0; i < 50; i++) {
      ...
      if (!item.Get(db) || item->GetItemBox() != itemBox.GetUUID()) {
        LogAccountManagerWarning([&]() {
          return libcomp::String(
                     "Clearing invalid Item %1 saved on ItemBox for account: "
                     "%2\n")
      ...
      if (allItems.count(item.Get())) {
        LogAccountManagerWarning([&]() {
          return libcomp::String(
                     "Clearing duplicate Item %1 saved on ItemBox for account: "
                     "%2\n")
              .Arg(item.GetUUID().ToString())
              .Arg(state->GetAccountUID().ToString());
        });
      ...
      state->SetObjectID(item->GetUUID(), server->GetNextObjectID());
    ...
    if (openSlots.size() > 0) {
      ...
      if (recovered > 0) {
        LogAccountManagerWarning([&]() {
          return libcomp::String(
                     "Recovered %1 orphaned Item(s) from account: %2\n")
              .Arg(recovered)
              .Arg(state->GetAccountUID().ToString());
        });
      }
  ...
  for (size_t i = 0; i < 15; i++) {
    ...
    if (state->GetObjectID(equip.GetUUID()) <= 0) {
      if (equip.Get(db)) {
        state->SetObjectID(equip->GetUUID(), server->GetNextObjectID());
      } else {
        LogAccountManagerWarning([&]() {
          return libcomp::String(
                     "Clearing invalid Equipped Item %1 on character: %2\n")
  ...
  for (auto expertise : character->GetExpertises()) {
    if (!expertise.IsNull() && !expertise.Get(db)) {
      LogAccountManagerError([&]() {
        return libcomp::String(
                   "Expertise %1 could not be initialized for account: %2\n")
  ...
  auto allStatusEffects = objects::StatusEffect::LoadStatusEffectListByEntity(
  ...
  for (auto effect : allStatusEffects) {
    ...
    if (!exists) {
      LogAccountManagerWarning([&]() {
        return libcomp::String(
                   "Recovered orphaned StatusEffect %1 on character: %2\n")
  ...
  if (character->StatusEffectsCount() > 0) {
    ...
    for (int32_t i = seCount - 1; i >= 0; i--) {
      ...
      if (effect.IsNull() || !effect.Get(db) ||
        ...
        LogAccountManagerWarning([&]() {
          return libcomp::String(
                     "Removing invalid character StatusEffect %1 saved for "
                     "account: %2\n")
              .Arg(effect.GetUUID().ToString())
              .Arg(state->GetAccountUID().ToString());
        });
  ...
  for (auto box : demonBoxes) {
    ...
    if (!box.Get(db)) {
      LogAccountManagerError([&]() {
        return libcomp::String(
                   "DemonBox %1 could not be initialized for account: %2\n")
    ...
    for (auto demon : box->GetDemons()) {
      ...
      if (!demon.Get(db) || !demon->LoadCoreStats(db)) {
        LogAccountManagerError([&]() {
          return libcomp::String(
                     "Demon or demon stats for %1 could not be initialized for "
                     "account: %2\n")
      ...
      for (auto iSkill : demon->GetInheritedSkills()) {
        if (!iSkill.Get(db)) {
          LogAccountManagerError([&]() {
            return libcomp::String(
                       "InheritedSkill %1 could not be initialized for "
                       "account: %2\n")
      ...
      state->SetObjectID(demon->GetUUID(), server->GetNextObjectID());

      // Demon status effects (recover first)
      allStatusEffects = objects::StatusEffect::LoadStatusEffectListByEntity(
          db, demon->GetUUID());
      for (auto effect : allStatusEffects) {
        bool exists = false;
        ...
        if (!exists) {
          LogAccountManagerWarning([&]() {
            return libcomp::String(
                       "Recovered orphaned StatusEffect %1 on demon: %2\n")
                .Arg(effect->GetUUID().ToString())
                .Arg(demon->GetUUID().ToString());
          });
      ...
      if (demon->StatusEffectsCount() > 0) {
        ...
        for (int32_t i = seCount - 1; i >= 0; i--) {
          ...
          if (effect.IsNull() || !effect.Get(db) ||
            ...
            LogAccountManagerWarning([&]() {
              return libcomp::String(
                         "Removing invalid demon StatusEffect %1 saved for "
                         "account: %2\n")
                  .Arg(effect.GetUUID().ToString())
                  .Arg(state->GetAccountUID().ToString());
            });
      ...
      for (size_t i = 0; i < 4; i++) {
        ...
        if (!equipment.Get(db)) {
          LogAccountManagerWarning([&]() {
            return libcomp::String(
                       "Removing invalid demon equipment %1 saved for account: "
                       "%2\n")
        ...
        state->SetObjectID(equipment.GetUUID(), server->GetNextObjectID());
      }
    }
  }
  ...
  if (!character->GetActiveDemon().IsNull() &&
    ...
    LogAccountManagerWarning([&]() {
      return libcomp::String(
                 "Unassigning unknown active demon from character: %1\n")
          .Arg(character->GetUUID().ToString());
    });
  ...
  for (uint32_t skillID : allSkillIDs) {
    if (!definitionManager->GetSkillData(skillID)) {
      LogAccountManagerError([&]() {
        return libcomp::String(
                   "Invalid skill ID '%1' associated to the character or an "
                   "associated demon on character: %2\n")
            .Arg(skillID)
            .Arg(character->GetUUID().ToString());
      });
  ...
  for (auto hotbar : character->GetHotbars()) {
    if (!hotbar.IsNull() && !hotbar.Get(db)) {
      LogAccountManagerError([&]() {
        return libcomp::String(
                   "Hotbar %1 could not be initialized for account: %2\n")
  ...
  auto quests =
      objects::Quest::LoadQuestListByCharacter(db, character->GetUUID());
  for (auto quest : quests) {
    ...
    if (!exists && quest->GetQuestID()) {
      LogAccountManagerWarning([&]() {
        return libcomp::String("Recovered orphaned Quest %1 on character: %2\n")
  ...
  for (int16_t questID : character->GetQuestsKeys()) {
    ...
    if (!qRef.IsNull() && !qRef.Get(db)) {
      LogAccountManagerWarning([&]() {
        return libcomp::String(
                   "Removing invalid quest %1 saved for character: %2\n")
  ...
  if (!character->GetDemonQuest().IsNull()) {
    if (!character->LoadDemonQuest(db)) {
      LogAccountManagerError([&]() {
        return libcomp::String(
                   "DemonQuest %1 could not be initialized for account: %2\n")
    ...
    if (!dQuest || !demon ||
      ...
      LogAccountManagerWarning([&]() {
        return libcomp::String(
                   "Removing invalid DemonQuest saved for account: %1\n")
            .Arg(state->GetAccountUID().ToString());
      });
      ...
      if (dQuest && !dQuest->Delete(db)) {
        LogAccountManagerError([&]() {
          return libcomp::String("DemonQuest could not be deleted: %1\n")
  ...
  if (!character->GetClan().IsNull()) {
    if (!character->LoadClan(db)) {
      LogAccountManagerError([&]() {
        return libcomp::String(
                   "Clan %1 could not be initialized for account: %2\n")
  ...
  for (auto counter : objects::EventCounter::LoadEventCounterListByCharacter(
    ...
    if (state->EventCountersKeyExists(counter->GetType())) {
      LogAccountManagerError([&]() {
        return libcomp::String(
                   "Duplicate event counter encountered on character %1: %2\n")
            .Arg(counter->GetType())
            .Arg(character->GetUUID().ToString());
      });
...
bool AccountManager::InitializeNewCharacter(
  ...
  auto dCharacter = std::dynamic_pointer_cast<objects::Character>(
      (objIter != defaultObjs.end()) ? objIter->second.front() : nullptr);
  if (dCharacter) {
    // Set (selective) custom character values
    character->SetLNC(dCharacter->GetLNC());
    character->SetPoints(dCharacter->GetPoints());
    character->SetExpertiseExtension(dCharacter->GetExpertiseExtension());
  ...
  // Generate stats
  auto dStats = dCharacter ? dCharacter->GetCoreStats().Get() : nullptr;
  if (dStats) {
    // Using custom stats
  ...
  if (dProgress) {
    ...
    progress = std::make_shared<objects::CharacterProgress>(*dProgress);
    progress->Register(progress, libobjgen::UUID::Random());
  } else {
    // Using normal progress
    progress = libcomp::PersistentObject::New<objects::CharacterProgress>(true);
  }

  progress->SetCharacter(character->GetUUID());

  if (!progress->Insert(db) || !character->SetProgress(progress)) {
    return false;
  }
  ...
  for (auto equip : character->GetEquippedItems()) {
    ...
    equip->SetDurability((uint16_t)((uint16_t)poss->GetDurability() * 1000));
  ...
  if (dCharacter) {
    for (size_t i = 0; i < 15; i++) {
      ...
      auto equipCopy = characterManager->GenerateItem(dEquip->GetType(), 1);
      ...
      if (!equipCopy->Insert(db) || !box->SetItems(slot, equipCopy) ||
          !character->SetEquippedItems((size_t)def->GetBasic()->GetEquipType(),
                                       equipCopy)) {
        return false;
      }
    }
  ...
  for (auto dObj : defaultObjs["Item"]) {
    ...
    auto itemCopy =
        characterManager->GenerateItem(dItem->GetType(), dItem->GetStackSize());
  ...
  // Create the COMP
  auto comp = libcomp::PersistentObject::New<objects::DemonBox>(true);
  ...
  for (auto dObj : defaultObjs["Demon"]) {
    ...
    auto devilData = definitionManager->GetDevilData(dDemon->GetType());
    ...
    if (dStats && copyStats->GetLevel() != dStats->GetLevel()) {
      ...
      characterManager->CalculateDemonBaseStats(demonCopy, copyStats,
                                                devilData);
    }
  ...
  auto fSettings =
      libcomp::PersistentObject::New<objects::FriendSettings>(true);
  fSettings->SetCharacter(character->GetUUID());

  if (!fSettings->Insert(db) || !character->SetFriendSettings(fSettings)) {
    return false;
  }
...
bool AccountManager::LogoutCharacter(channel::ClientState* state) {
  ...
  auto character =
      state->GetAccountLogin()->GetCharacterLogin()->GetCharacter().Get();
  ...
  return mServer.lock()->GetWorldDatabase()->ProcessChangeSet(dbChanges);
}
    ...
    if (!box.IsNull() && !box->SaveWithUUID(doc, *pRoot)) {
      ...
      for (auto demon : box->GetDemons()) {
        if (!demon.IsNull() && !demon->SaveWithUUID(doc, *pRoot)) {
          ...
          for (auto iSkill : demon->GetInheritedSkills()) {
            if (!iSkill.IsNull() && !iSkill->SaveWithUUID(doc, *pRoot)) {
              return {};
            }
          }
          ...
          for (size_t i = 0; i < 4; i++) {
            ...
            if (!equipment.IsNull() && !equipment->SaveWithUUID(doc, *pRoot)) {
              return {};
            }
          }
        }
