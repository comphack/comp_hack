 ...
 * @author HACKfrost
 *
 * @brief Implementation for a window that handles event viewing and
 * modification.
 *
...
EventWindow::EventWindow(MainWindow* pMainWindow, QWidget* pParent)
  ...
  pAction->setData(to_underlying(objects::Event::EventType_t::FORK));
  connect(pAction, SIGNAL(triggered()), this, SLOT(NewEvent()));

  pAction = pMenu->addAction("Direction");
  pAction->setData(to_underlying(objects::Event::EventType_t::DIRECTION));
  connect(pAction, SIGNAL(triggered()), this, SLOT(NewEvent()));

  pAction = pMenu->addAction("EX NPC Message");
  pAction->setData(to_underlying(objects::Event::EventType_t::EX_NPC_MESSAGE));
  connect(pAction, SIGNAL(triggered()), this, SLOT(NewEvent()));

  pAction = pMenu->addAction("I-Time");
  pAction->setData(to_underlying(objects::Event::EventType_t::ITIME));
  connect(pAction, SIGNAL(triggered()), this, SLOT(NewEvent()));

  pAction = pMenu->addAction("Multitalk");
  pAction->setData(to_underlying(objects::Event::EventType_t::MULTITALK));
  connect(pAction, SIGNAL(triggered()), this, SLOT(NewEvent()));

  pAction = pMenu->addAction("NPC Message");
  pAction->setData(to_underlying(objects::Event::EventType_t::NPC_MESSAGE));
  connect(pAction, SIGNAL(triggered()), this, SLOT(NewEvent()));

  pAction = pMenu->addAction("Open Menu");
  pAction->setData(to_underlying(objects::Event::EventType_t::OPEN_MENU));
  connect(pAction, SIGNAL(triggered()), this, SLOT(NewEvent()));

  pAction = pMenu->addAction("Perform Actions");
  pAction->setData(to_underlying(objects::Event::EventType_t::PERFORM_ACTIONS));
  connect(pAction, SIGNAL(triggered()), this, SLOT(NewEvent()));

  pAction = pMenu->addAction("Play Scene");
  pAction->setData(to_underlying(objects::Event::EventType_t::PLAY_SCENE));
  connect(pAction, SIGNAL(triggered()), this, SLOT(NewEvent()));

  pAction = pMenu->addAction("Prompt");
  pAction->setData(to_underlying(objects::Event::EventType_t::PROMPT));
  connect(pAction, SIGNAL(triggered()), this, SLOT(NewEvent()));
  ...
  pAction->setData(
      to_underlying(objects::Event::EventType_t::PERFORM_ACTIONS) +
  ...
  connect(ui->actionReloadFile, SIGNAL(triggered()), this, SLOT(ReloadFile()));
  ...
  connect(ui->actionFindAction, SIGNAL(triggered()), this, SLOT(FindAction()));
  ...
  connect(findAction->next, SIGNAL(clicked()), this, SLOT(FindNextAction()));
}
...
bool EventWindow::GoToEvent(const libcomp::String& eventID) {
  ...
  if (iter == mGlobalIDMap.end()) {
    QMessageBox err;
    err.setText(
        qs(libcomp::String("Event '%1' is not currently loaded").Arg(eventID)));
    err.exec();
...
void EventWindow::ChangeEventID(const libcomp::String& currentID) {
  ...
  if (fEvent) {
    libcomp::String eventID =
        GetNewEventID(file, fEvent->Event->GetEventType());
    if (eventID.IsEmpty()) {
      return;
    }
    ...
    auto reply = QMessageBox::question(
        this, "Confirm Rename",
        QString(
            "Event ID '%1' will be changed to '%2' and all currently loaded "
            "event references will be updated automatically however, no files "
            "will be saved at this time. Only the current zone and loaded zone "
    ...
    QMessageBox err;
    err.setText(
        qs(libcomp::String("Event ID '%1' does not exist").Arg(currentID)));
    err.exec();
  }
}
...
libcomp::String EventWindow::GetCurrentFile() const { return mCurrentFileName; }
...
void EventWindow::LoadFile() {
  ...
  QString qPath = QFileDialog::getOpenFileName(
      this, tr("Load Event XML"), mMainWindow->GetDialogDirectory(),
      tr("Event XML (*.xml)"));
  if (qPath.isEmpty()) {
    return;
  }
...
void EventWindow::ReloadFile() {
  ...
  bool changeExists = file->Reordered || file->PendingRemovals.size() > 0;
  ...
  if (changeExists) {
    auto reply =
        QMessageBox::question(this, "Confirm Reload",
                              QString("%1 has pending changes that will be "
                                      "lost by reloading. Is this okay?")
                                  .arg(qs(mCurrentFileName)),
                              QMessageBox::Yes | QMessageBox::No);
    if (reply != QMessageBox::Yes) {
      return;
    }
...
void EventWindow::NewFile() {
  ...
  QString qPath = QFileDialog::getSaveFileName(
      this, tr("Create new Event file"), mMainWindow->GetDialogDirectory(),
      tr("Event XML (*.xml)"));
  if (qPath.isEmpty()) {
    return;
  }
  ...
  if (fi.exists() && fi.isFile()) {
    LogGeneralError([&]() {
      return libcomp::String(
                 "Attempted to overwrite existing file with new event file: %1")
          .Arg(cs(qPath));
    });
  ...
  if (filter.isEmpty()) {
    ...
    for (auto item : items) {
      if (display.find(item) == display.end() &&
          (item->text(0).contains(filter) || item->text(2).contains(filter))) {
        display.insert(item);
...
void EventWindow::Convert() {
  ...
  switch (currentType) {
    case objects::Event::EventType_t::EX_NPC_MESSAGE: {
      auto msg =
          std::dynamic_pointer_cast<objects::EventExNPCMessage>(current->Event);
      if (msg) {
    ...
    case objects::Event::EventType_t::NPC_MESSAGE: {
      auto msg =
          std::dynamic_pointer_cast<objects::EventNPCMessage>(current->Event);
    ...
    case objects::Event::EventType_t::PERFORM_ACTIONS: {
      ...
      if (pa) {
        for (auto act : pa->GetActions()) {
          auto dm =
              std::dynamic_pointer_cast<objects::ActionDisplayMessage>(act);
          if (dm && dm->MessageIDsCount() > 0) {
            messageID = dm->GetMessageIDs(0);
            break;
          }

          auto se = std::dynamic_pointer_cast<objects::ActionStageEffect>(act);
          if (se) {
    ...
    case objects::Event::EventType_t::PROMPT: {
      auto pr = std::dynamic_pointer_cast<objects::EventPrompt>(current->Event);
      if (pr) {
  ...
  switch (eventType) {
    case objects::Event::EventType_t::EX_NPC_MESSAGE:
      std::dynamic_pointer_cast<objects::EventExNPCMessage>(e)->SetMessageID(
          messageID);
      break;
    case objects::Event::EventType_t::NPC_MESSAGE:
      std::dynamic_pointer_cast<objects::EventNPCMessage>(e)->AppendMessageIDs(
          messageID);
      break;
    case objects::Event::EventType_t::PERFORM_ACTIONS:
      if ((dataVal / 100) ==
      ...
      break;
    case objects::Event::EventType_t::PROMPT:
      std::dynamic_pointer_cast<objects::EventPrompt>(e)->SetMessageID(
          messageID);
      break;
    default:
...
void EventWindow::GoTo() {
  QString qEventID =
      QInputDialog::getText(this, "Enter an ID", "Event ID", QLineEdit::Normal);
  if (qEventID.isEmpty()) {
    return;
  }
...
void EventWindow::FindNextAction() {
  ...
  auto actionType =
      (objects::Action::ActionType_t)findAction->action->currentData().toInt();
...
void EventWindow::Back() {
  if (mPreviousEventIDs.size() > 0) {
    ...
    if (mPreviousEventIDs.size() == 0) {
      ...
      ui->eventBack->setText(
          QString("Back (%1)").arg(mPreviousEventIDs.size()));
    }
  }
}
...
void EventWindow::CollapseAll() { ui->treeWidget->collapseAll(); }

void EventWindow::ExpandAll() { ui->treeWidget->expandAll(); }
...
void EventWindow::TreeSelectionChanged() { BindSelectedEvent(true); }
...
void EventWindow::ChangeFileEventIDs() {
  ...
  QString qPrefix =
      QInputDialog::getText(this, "Enter an ID prefix", "New ID prefix",
                            QLineEdit::Normal, qs(commonPrefix));
  if (qPrefix.isEmpty()) {
    return;
  }
  ...
  for (auto fEvent : file->Events) {
    ...
    auto typePrefix = GetEventTypePrefix(prefix, fEvent->Event->GetEventType());
    ...
    if (seq > 1000) {
      QMessageBox err;
      err.setText(
          "Event sequence exceeded 1000. Please split the file into fewer "
          "events or rename by tree branches.");
      err.exec();
...
void EventWindow::ChangeTreeBranchIDs() {
  ...
  if (secondaryRefs) {
    QMessageBox warn;
    warn.setText(
        "The selected tree branch contains at least one event that is either "
        "not part of this file or is referenced before the selected node "
        "within this file. These events will not be renamed.");
    warn.exec();
  }
  ...
  for (auto fEvent : renames) {
    for (; seq < 999;) {
      ...
      // Zero pad the number
      auto str = libcomp::String("%1%2").Arg(prefix).Arg(
          libcomp::String("%1").Arg(1000 + seq).Right(3));
      if (mGlobalIDMap.find(str) == mGlobalIDMap.end() ||
  ...
  if (seq > 1000) {
    QMessageBox err;
    err.setText(
        "Event sequence exceeded 1000. Please select fewer nodes to change.");
    err.exec();
    ...
    auto reply = QMessageBox::question(
        this, "Confirm Rename",
        QString("%1 event IDs will be changed to '%2' through '%3'%4 and all "
                "currently loaded event references will be updated "
...
bool EventWindow::LoadFileFromPath(const libcomp::String& path) {
  ...
  while (objNode) {
    ...
    if (event->GetID().IsEmpty()) {
      LogGeneralError([&]() {
        return libcomp::String("Event with no ID encountered in file: %1\n")
            .Arg(path);
      });
  ...
  if (events.size() > 0 || rootElem->FirstChild() == nullptr) {
    if (mFiles.find(path) != mFiles.end()) {
      LogGeneralInfo([&]() {
        return libcomp::String("Reloaded %1 event(s) from file: %2\n")
            .Arg(events.size())
            .Arg(path);
      });
    } else {
      LogGeneralInfo([&]() {
        return libcomp::String("Loaded %1 event(s) from file: %2\n")
            .Arg(events.size())
            .Arg(path);
      });
    }
...
void EventWindow::SaveFiles(const std::list<libcomp::String>& paths) {
  ...
  for (auto path : paths) {
    ...
    if (hasDupe) {
      QMessageBox err;
      err.setText(QString("File '%1' cannot be saved because it has at least "
                          "one duplicate event ID.")
                      .arg(qs(path)));
      err.exec();
      ...
      while (child != 0) {
        ...
        while (member != 0) {
          ...
          if (memberName == "ID") {
            ...
            if (txt &&
                existingEvents.find(txt->Value()) == existingEvents.end()) {
    ...
    for (auto fEvent : file->Events) {
      ...
      if (fEvent->Comments.size()) {
        ...
        for (auto comment : fEvent->Comments) {
          auto cNode = doc.NewComment(libcomp::String(" %1 ").Arg(comment).C());
...
void EventWindow::BindSelectedEvent(bool storePrevious) {
  ...
  if (fileIdx == -1 && selected) {
    if (file &&
        file->EventIDMap.find(selected->EventID) != file->EventIDMap.end()) {
...
void EventWindow::BindEventEditControls(QWidget* eNode) {
  ...
  for (auto ctrl : eNode->findChildren<ActionList*>()) {
    connect(ctrl, SIGNAL(rowEdit()), this, SLOT(CurrentEventEdited()));
  }
  ...
  for (auto ctrl : eNode->findChildren<QCheckBox*>()) {
    connect(ctrl, SIGNAL(toggled(bool)), this, SLOT(CurrentEventEdited()));
  }
  ...
  for (auto ctrl : eNode->findChildren<QSpinBox*>()) {
    connect(ctrl, SIGNAL(valueChanged(int)), this, SLOT(CurrentEventEdited()));
  }
}
...
void EventWindow::AddEventToTree(const libcomp::String& id,
  ...
  switch (e->GetEventType()) {
    case objects::Event::EventType_t::FORK:
      ...
      break;
    case objects::Event::EventType_t::NPC_MESSAGE: {
      auto msg = std::dynamic_pointer_cast<objects::EventNPCMessage>(e);

      auto cMessage = mMainWindow->GetEventMessage(msg->GetMessageIDs(0));
      libcomp::String moreTxt =
          msg->MessageIDsCount() > 1
              ? libcomp::String(" [+%1 More]").Arg(msg->MessageIDsCount() - 1)
              : "";
      item->setText(1, "NPC Message");
      item->setText(2, cMessage ? qs(GetInlineMessageText(libcomp::String::Join(
                                         cMessage->GetLines(), "  ")) +
                                     moreTxt)
                                : "");
    } break;
    case objects::Event::EventType_t::EX_NPC_MESSAGE: {
      auto msg = std::dynamic_pointer_cast<objects::EventExNPCMessage>(e);

      auto cMessage = mMainWindow->GetEventMessage(msg->GetMessageID());
    ...
    case objects::Event::EventType_t::ITIME: {
      ...
      bool iTime = e->GetEventType() == objects::Event::EventType_t::ITIME;
      if (iTime) {
      ...
      if (iTime) {
        ...
        for (auto& pair : messageNodes) {
          auto txt =
              dataset->GetName(dataset->GetObjectByID((uint32_t)pair.second));
          if (txt.Length() > 0) {
    ...
    case objects::Event::EventType_t::PERFORM_ACTIONS: {
      auto pa = std::dynamic_pointer_cast<objects::EventPerformActions>(e);
      ...
      for (auto action : pa->GetActions()) {
        switch (action->GetActionType()) {
          case objects::Action::ActionType_t::DISPLAY_MESSAGE: {
            auto act = std::dynamic_pointer_cast<objects::ActionDisplayMessage>(
                action);
...
void EventWindow::ChangeEventIDs(
...
bool EventWindow::ChangeActionEventIDs(
    const std::unordered_map<libcomp::String, libcomp::String>& idMap,
...
libcomp::String EventWindow::GetCommonEventPrefix(
  ...
  for (; eIter != file->Events.end(); eIter++) {
    ...
    while (commonPrefix.Length() > 0 &&
      ...
      commonPrefix = commonPrefix.Left((size_t)(commonPrefix.Length() - 1));
    }
...
libcomp::String EventWindow::GetEventTypePrefix(
    const libcomp::String& prefix, objects::Event::EventType_t eventType) {
...
libcomp::String EventWindow::GetNewEventID(
  ...
  libcomp::String eventID;
  while (true) {
    QString qEventID = QInputDialog::getText(
        this, "Enter an ID", "New ID", QLineEdit::Normal, qs(suggestedID));
    if (qEventID.isEmpty()) {
      return "";
    }
    ...
    if (globalIter != mGlobalIDMap.end()) {
      QMessageBox err;
      err.setText(qs(libcomp::String("Event ID '%1' already exists in file: %2")
                         .Arg(eventID)
                         .Arg(globalIter->second)));
      err.exec();
    } else {
      break;
    }
...
void EventWindow::UpdatePreviousEvents(const libcomp::String& last) {
  ...
  ui->eventBack->setText(QString("Back (%1)").arg(mPreviousEventIDs.size()));
  ui->eventBack->show();
}
