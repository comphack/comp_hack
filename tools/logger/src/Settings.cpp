...
Settings::Settings(LoggerServer *server, QWidget *p)
  ...
  connect(ui.addClientButton, SIGNAL(clicked(bool)), this, SLOT(addClient()));
  connect(ui.removeClientButton, SIGNAL(clicked(bool)), this,
          SLOT(removeClient()));
  ...
  ui.closeWarning->setChecked(
      !QSettings().value("noexitwarning", false).toBool());
  ...
  while (it.hasNext()) {
    ...
    QListWidgetItem *item =
        new QListWidgetItem(tr("%1 (%2)").arg(title).arg(path));
...
void Settings::addClient() {
  ...
  QMessageBox::information(
      this, "Locate Client Install",
      "You will now be asked to locate the directory that contains your client "
      "install (the directory that contains the file ImagineClient.exe).");

  // Present the user with the file dialog to select the client install
  // directory.
  QString path = QDir::toNativeSeparators(QFileDialog::getExistingDirectory(
      this, "Locate Client Install", "C:\\AeriaGames\\MegaTen"));
  ...
  if (!MainWindow::versionCheck(tr("%1/ImagineClient.exe").arg(path), &ver, 0,
                                &isUS)) {
  ...
  QString title = QInputDialog::getText(
      this, "Client Name", "Plase name this client version:", QLineEdit::Normal,
      QString::fromUtf8(buffer), &ok);

  // If the user didn't input a name or clicked cancel, abort.
  if (title.isEmpty() || !ok) return;
...
void Settings::selectedClient() {
  ...
  ui.removeClientButton->setEnabled(!ui.clientList->selectedItems().isEmpty());
}
