...
bool Parsers::EquipmentModEdit::Parse(
        ...
        if (RNG(int32_t, 1, 10000) <= successRate) {
          ...
          switch (mode) {
            ...
            case MODE_EMPTY_SOUL_TAROT: {
              ...
              if (subMode == 1) {
                if ((itemData->GetBasic()->GetFlags() & ITEM_FLAG_SOUL) == 0) {
                  // Reset to "enabled"
                  item->SetSoul(ENCHANT_ENABLE_EFFECT);
                } else {
                  item->SetSoul(0);
                ...
                if ((itemData->GetBasic()->GetFlags() & ITEM_FLAG_TAROT) == 0) {
                  // Reset to "enabled"
                  item->SetTarot(ENCHANT_ENABLE_EFFECT);
                } else {
                  item->SetTarot(0);
  ...
  if (activatedAbility) {
    if (responseCode != RESULT_CODE_ERROR) {
      if (!server->GetSkillManager()->ExecuteSkill(
              cState, skillActivationID, (int64_t)cState->GetEntityID())) {
        responseCode = RESULT_CODE_ERROR;
      }
    } else {
      uint32_t skillID = activatedAbility->GetSkillData()->GetCommon()->GetID();
      server->GetSkillManager()->SendFailure(cState, skillID, client);
    }
  }
