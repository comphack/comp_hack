...
BaseScriptEngine& BaseScriptEngine::Using<MatchManager>() {
  if (!BindingExists("MatchManager", true)) {
    ...
    Sqrat::Class<MatchManager, Sqrat::NoConstructor<MatchManager>> binding(
        mVM, "MatchManager");
    binding.Func("GetMatchEntry", &MatchManager::GetMatchEntry)
        ...
        .Func("StartUltimateBattleTimer",
              &MatchManager::StartUltimateBattleTimer)
...
MatchManager::MatchManager(const std::weak_ptr<ChannelServer>& server)
    : mServer(server) {}
...
bool MatchManager::JoinQueue(
    const std::shared_ptr<ChannelClientConnection>& client, int8_t type) {
  ...
  if (team) {
    ...
    for (int32_t memberCID : team->GetMemberIDs()) {
      auto teamClient = managerConnection->GetEntityClient(memberCID, true);
      if (!teamClient) {
        return false;
      }
  ...
  if (queued) {
    ...
    LogMatchManagerWarning([&]() {
      return libcomp::String(
                 "One or more match entries failed to queue for account: %1\n")
...
bool MatchManager::CancelQueue(
  ...
  if (match) {
    ...
    if (queued) {
      ...
      LogMatchManagerWarning([&]() {
        return libcomp::String(
                   "One or more match cancellations failed to send to the "
                   "world for account: %1\n")
            .Arg(state->GetAccountUID().ToString());
      });
...
void MatchManager::ConfirmMatch(
    const std::shared_ptr<ChannelClientConnection>& client, uint32_t matchID) {
  auto state = client->GetClientState();
  auto match = state->GetPendingMatch();
  if (!match) {
    return;
  ...
  switch (match->GetType()) {
    ...
    case objects::Match::Type_t::PVP_VALHALLA: {
      ...
      auto pvpMatch = std::dynamic_pointer_cast<objects::PvPMatch>(match);

      bool success = pvpMatch && PvPInviteReply(client, match->GetID(), true);

      libcomp::Packet reply;
      reply.WritePacketCode(ChannelToClientPacketCode_t::PACKET_PVP_CONFIRM);
    ...
    case objects::Match::Type_t::ULTIMATE_BATTLE: {
      ...
      if (ubMatch) {
        auto zoneManager = mServer.lock()->GetZoneManager();
        auto zone = zoneManager->GetGlobalZone(ubMatch->GetZoneDefinitionID(),
                                               ubMatch->GetDynamicMapID());
        ...
        if (success) {
          float x, y, rot;
          zoneManager->GetMatchStartPosition(client, zone, x, y, rot);

          zoneManager->EnterZone(client, ubMatch->GetZoneDefinitionID(),
                                 ubMatch->GetDynamicMapID(), x, y, rot);
        }
      }
...
void MatchManager::RejectPvPMatch(
    const std::shared_ptr<ChannelClientConnection>& client) {
  auto state = client->GetClientState();
  auto match = state->GetPendingMatch();
  if (!match) {
...
void MatchManager::LeaveTeam(
    const std::shared_ptr<ChannelClientConnection>& client, int32_t teamID) {
  auto state = client->GetClientState();
...
bool MatchManager::UpdateUBPoints(
    const std::shared_ptr<ChannelClientConnection>& client, int32_t adjust) {
  ...
  if (ubMatch) {
    ...
    if (ubMatch->GetState() != objects::UBMatch::State_t::COMPLETE) {
      ...
      ubMatch->SetPoints(worldCID, ubMatch->GetPoints(worldCID) + adjust);
      return true;
    }
  }
...
bool MatchManager::UpdateZiotite(const std::shared_ptr<objects::Team>& team,
  ...
  if (!worldCID) {
    ...
    libcomp::Packet request;
    request.WritePacketCode(InternalPacketCode_t::PACKET_TEAM_UPDATE);
    request.WriteU8((int8_t)InternalPacketAction_t::PACKET_ACTION_TEAM_ZIOTITE);
...
void MatchManager::SendZiotite(
  ...
  if (team) {
    ...
    if (client) {
      ...
      auto clients = mServer.lock()->GetManagerConnection()->GetEntityClients(
          team->GetMemberIDs(), true);
...
bool MatchManager::RequestTeamPvPMatch(
    const std::shared_ptr<ChannelClientConnection>& client, uint32_t variantID,
    uint32_t instanceID) {
  auto state = client->GetClientState();
  auto team = state->GetTeam();
  if (!team || team->GetLeaderCID() != state->GetWorldCID()) {
    LogMatchManagerDebug([&]() {
      return libcomp::String(
                 "Team PvP creation failed: requestor is not the team leader: "
                 "%1\n")
          .Arg(state->GetAccountUID().ToString());
    });
  ...
  for (int32_t memberCID : team->GetMemberIDs()) {
    ...
    if (!teamClient) {
      LogMatchManagerDebug([&]() {
        return libcomp::String(
                   "Team PvP creation failed: one or more team members is not "
                   "on the channel: %1\n")
            .Arg(state->GetAccountUID().ToString());
      });
  ...
  if (!ValidateMatchEntries(teamClients, (int8_t)objects::Team::Category_t::PVP,
                            true, false)) {
    LogMatchManagerDebug([&]() {
      return libcomp::String(
                 "Team PvP creation failed: one or more team members is not "
  ...
  if (!variant) {
    LogMatchManagerDebug([&]() {
      return libcomp::String(
                 "Team PvP creation failed: invalid variant specified for "
                 "instance: %1\n")
          .Arg(state->GetAccountUID().ToString());
    });
  ...
  if (variant->GetMaxPlayers() &&
    ...
    LogMatchManagerDebug([&]() {
      return libcomp::String(
                 "Team PvP creation failed: too many players are in the "
                 "current team: %1\n")
          .Arg(state->GetAccountUID().ToString());
    });
  ...
  auto instance =
      server->GetServerDataManager()->GetZoneInstanceData(instanceID);
  if (!instance) {
    LogMatchManagerDebug([&]() {
      return libcomp::String(
                 "Team PvP creation failed: invalid instance requested: %1\n")
          .Arg(state->GetAccountUID().ToString());
    });
  ...
  if (variant->GetLimitRed()) {
    ...
    if (variant->GetLimitBlue() &&
      ...
      LogMatchManagerDebug([&]() {
        return libcomp::String(
                   "Team PvP creation failed: team size restrictions could not "
                   "be met: %1\n")
            .Arg(state->GetAccountUID().ToString());
      });
  ...
  LogMatchManagerDebug([&]() {
    return libcomp::String(
               "Requesting team PvP match with variant %1 and instance %2: "
               "%3\n")
        .Arg(variantID)
        .Arg(instanceID)
  ...
  if (!server->GetChannelSyncManager()->SyncRecordUpdate(match, "PvPMatch")) {
    LogMatchManagerDebug([&]() {
      return libcomp::String(
                 "Team PvP creation failed: match could not be queued: %1\n")
          .Arg(state->GetAccountUID().ToString());
    });
...
bool MatchManager::JoinUltimateBattleQueue(int32_t worldCID,
  ...
  if (ubMatch->GetCategory() == objects::UBMatch::Category_t::UA) {
    ...
    auto client =
        mServer.lock()->GetManagerConnection()->GetEntityClient(worldCID, true);
    if (client) {
      ...
      entry->SetMatchType(objects::MatchEntry::MatchType_t::ULTIMATE_BATTLE);
...
bool MatchManager::PvPInviteReply(
    const std::shared_ptr<ChannelClientConnection>& client, uint32_t matchID,
    bool accept) {
  auto state = client->GetClientState();
  auto match = state->GetPendingMatch();
    ...
    if (!accept) {
      ...
      if (pvpData) {
        auto opChangeset = std::make_shared<libcomp::DBOperationalChangeSet>(
            state->GetAccountUID());
        auto expl = std::make_shared<libcomp::DBExplicitUpdate>(pvpData);
        ...
        if (!mServer.lock()->GetWorldDatabase()->ProcessChangeSet(
          ...
          LogMatchManagerError([&]() {
            return libcomp::String("Failed to apply PvP penalty: %1\n")
                .Arg(state->GetAccountUID().ToString());
          });
        }
      }
...
void MatchManager::StartPvPMatch(uint32_t instanceID) {
  ...
  if (MatchTeamsActive(instance)) {
    ...
    for (auto zone : instance->GetZones()) {
      zoneManager->TriggerZoneActions(zone, {}, ZoneTrigger_t::ON_PVP_START,
                                      nullptr);
    }
...
bool MatchManager::EndPvPMatch(uint32_t instanceID) {
  ...
  for (auto cids : {match->GetBlueMemberIDs(), match->GetRedMemberIDs()}) {
    for (int32_t cid : cids) {
      for (auto& pair : pvpStats->GetPlayerStats()) {
        ...
        if (stats->GetWorldCID() == cid && stats->GetCharacter().Get(db)) {
          teams[idx].push_back(stats);

          // Determine if they are still in the instance now that
          // the match has ended
          for (auto& zone : instance->GetZones()) {
            auto eState = zone->GetActiveEntity(stats->GetEntityID());
            if (eState) {
  ...
  bool noMatch = instance->GetTimerStart() == 0 || (teamQuit[0] && teamQuit[1]);
  int32_t timeLeft =
      noMatch
          ? 0
          : (int32_t)((instance->GetTimerStop() - instance->GetTimerStart()) /
                      1000000ULL);
  if (timeLeft < 0) {
    timeLeft = 0;
  }
  ...
  for (idx = 0; idx < 2; idx++) {
    for (auto stats : teams[idx]) {
      ...
      auto pvpData = character ? character->GetPvPData().Get(db) : nullptr;

      int32_t gp = pvpData ? pvpData->GetGP() : 0;
      matchGP[stats->GetEntityID()] = gp;
      matchGPAdjust[stats->GetEntityID()] =
          gp + (pvpData && pvpData->GetRanked() ? 1000 : 0);
    }
  }
  ...
  for (idx = 0; idx < 2; idx++) {
    ...
    for (auto stats : teams[idx]) {
      teamBasePoints += stats->GetBasePoints() + stats->GetBaseBonusPoints();
    }
    ...
    for (auto stats : teams[idx]) {
      ...
      if (pvpData && !noMatch) {
        int32_t kills = (int32_t)(stats->GetKills() + stats->GetDemonKills());
        int32_t deaths =
            (int32_t)(stats->GetDeaths() + stats->GetDemonDeaths());
        if (variant) {
          // Determine GP/ranked
          if (!ranked) {
            ...
            float weight = 1.f + (0.2f * ((float)teams[otherIdx].size() -
                                          (float)teams[idx].size()));
            if (weight < 0.5f) {
              weight = 0.5f;
            } else if (weight > 1.5f) {
              weight = 1.5f;
            ...
            for (auto oStats : teams[otherIdx]) {
              ...
              calc += (((double)gp * (double)weight) - (double)gpAdjust +
                       (double)gpSkew);
            }
          ...
          if (!lost || !variant->GetBPWinRequired()) {
            ...
            if (timeLeft && variant->GetRewardModifier()) {
              calc += (double)variant->GetRewardModifier() * (double)timeLeft /
                      (double)variant->GetTimePoints(0);
            }

            // BP is scaled by kill/death values
            double kdScale = (double)variant->GetBPKillDeathScale();
            if (kdScale > 0.0) {
              calc = calc *
                     (1.0 + ((double)kills * 0.10) - ((double)deaths * 0.05));
            ...
            // in the match)
            double basePoints =
                (double)(stats->GetBasePoints() + stats->GetBaseBonusPoints());
            double baseScale = (lost ? 20.0 : 6.0);
            calc += ((double)teamBasePoints + basePoints) / baseScale *
                    (1.0 + (double)stats->GetTrophyBoost());
        ...
        if ((int8_t)match->GetType() == 0 || (int8_t)match->GetType() == 1) {
          // 0: Win, 1: Lose, 2: Draw
      ...
      p.WriteU16Little(
          (uint16_t)(stats->GetDeaths() + stats->GetDemonDeaths()));
  ...
  for (auto& xpPair : xpDeltas) {
    ...
    if (client) {
      characterManager->UpdateExperience(client, xpPair.second, xpPair.first);
    }
  }
...
void MatchManager::EnterDiaspora(
    const std::shared_ptr<ChannelClientConnection>& client,
    const std::shared_ptr<Zone>& zone) {
...
void MatchManager::EnterUltimateBattle(
    const std::shared_ptr<ChannelClientConnection>& client,
    const std::shared_ptr<Zone>& zone) {
  auto state = client->GetClientState();
  auto ubMatch = zone->GetUBMatch();
  ...
  if (!ubMatch) {
    ...
    if (ubMatch->GetTimerStart() == 0 &&
      ...
      ubMatch->SetTimerExpire(
          now + (uint64_t)((uint64_t)ubMatch->GetReadyDuration() * 1000000ULL));
  ...
  if (spectating) {
    ...
    state->GetDemonState()->SetDisplayState(ActiveDisplayState_t::UB_SPECTATE);
  }
...
void MatchManager::LeaveUltimateBattle(
    const std::shared_ptr<ChannelClientConnection>& client,
    const std::shared_ptr<Zone>& zone) {
  auto state = client->GetClientState();
  auto ubMatch = zone->GetUBMatch();
...
bool MatchManager::StartPvPBaseCapture(
  ...
  int32_t errorCode = zone->OccupyPvPBase(baseID, cState->GetEntityID(), false);
  if (errorCode == 0) {
    if (!mServer.lock()->GetEventManager()->StartSystemEvent(client, baseID)) {
      // Cancel occupation
  ...
  libcomp::Packet reply;
  reply.WritePacketCode(ChannelToClientPacketCode_t::PACKET_PVP_BASE_CAPTURE);
  ...
  if (errorCode == 0) {
    ...
    server->GetTimerManager()->ScheduleEventIn(
        ...
        [](MatchManager* pMatchManager, int32_t pEntityID, int32_t pBaseID,
           uint32_t pZoneID, uint32_t pInstanceID, uint64_t pOccupyStartTime) {
          pMatchManager->CompleteBaseCapture(pEntityID, pBaseID, pZoneID,
                                             pInstanceID, pOccupyStartTime);
        },
        this, cState->GetEntityID(), baseID, zone->GetID(),
        zone->GetInstance()->GetID(), bState->GetEntity()->GetOccupyTime());
  } else {
    client->SendPacket(reply);
  }
...
bool MatchManager::LeaveBase(
  ...
  if (bState) {
    ...
    if (errorCode == 0) {
      ...
      libcomp::Packet notify;
      notify.WritePacketCode(ChannelToClientPacketCode_t::PACKET_PVP_BASE_LEFT);
...
void MatchManager::CompleteBaseCapture(int32_t entityID, int32_t baseID,
                                       uint32_t zoneID, uint32_t instanceID,
  ...
  auto bState = zone->GetPvPBase(baseID);
  int32_t errorCode =
      zone->OccupyPvPBase(baseID, entityID, true, occupyStartTime);
  if (errorCode == 0) {
    ...
    if (PvPActive(instance)) {
      ...
      points = UpdatePvPPoints(instance->GetID(), entityID, baseID, teamID,
                               points, false);
      ...
      if (points > 0) {
        ...
        if (stats) {
          stats->SetBasePoints((uint16_t)(points + stats->GetBasePoints()));
        }
      }

      // Fire base capture triggers if any
      auto triggers = zoneManager->GetZoneTriggers(
          zone, ZoneTrigger_t::ON_PVP_BASE_CAPTURE);
      if (triggers.size() > 0) {
    ...
    if (client) {
      int32_t sourceEntityID =
          client->GetClientState()->GetEventSourceEntityID();
      if (sourceEntityID == baseID) {
...
void MatchManager::IncreaseBaseBonus(int32_t baseID, uint32_t zoneID,
  ...
  if (bonus) {
    ...
    int32_t points = (int32_t)(base->GetRank() + (base->GetSpeed() * bonus));
    points = UpdatePvPPoints(instance->GetID(), occupierID, baseID, teamID,
                             points, true);
    ...
    if (points > 0 && pvpStats) {
      ...
      if (stats) {
        stats->SetBaseBonusPoints(
            (uint16_t)(points + stats->GetBaseBonusPoints()));
      }
    }
  }
}

int32_t MatchManager::UpdatePvPPoints(uint32_t instanceID,
  ...
  if (target && InPvPTeam(target)) {
    ...
    teamID = (uint8_t)(target->GetFactionGroup() == 2 ? 0 : 1);
             ...
             (!target ||
              target->GetFactionGroup() != source->GetFactionGroup()) &&
             InPvPTeam(source)) {
    ...
    for (size_t i = 0; i < 2; i++) {
      ...
      if (entity) {
        ...
        if (entity->GetEntityType() == EntityType_t::PARTNER_DEMON) {
          ...
          auto state = ClientState::GetEntityClientState(entityIDs[i]);
          if (state) {
            entityIDs[i] = state->GetCharacterState()->GetEntityID();
            altMode |= i == 1;
          }
        }
      }
...
int32_t MatchManager::UpdatePvPPoints(uint32_t instanceID, int32_t sourceID,
                                      int32_t targetID, uint8_t teamID,
                                      int32_t points, bool altMode) {
  if (teamID <= 1) {
    auto server = mServer.lock();
    auto zoneManager = server->GetZoneManager();
    ...
    if (PvPActive(instance)) {
      ...
      uint16_t newPoints = 0;
      {
        ...
        if (variant->GetMaxPoints() && oldPoints >= variant->GetMaxPoints()) {
      ...
      if (adjust) {
        libcomp::Packet notify;
        notify.WritePacketCode(ChannelToClientPacketCode_t::PACKET_PVP_POINTS);
        notify.WriteS8((int8_t)teamID);
        ...
        ChannelClientConnection::BroadcastPacket(instance->GetConnections(),
                                                 notify);

        if (variant->GetMaxPoints() && newPoints >= variant->GetMaxPoints()) {
          // Match is over
          zoneManager->StopInstanceTimer(instance);
        }
      }
...
void MatchManager::PlayerKilled(
...
bool MatchManager::ToggleDiasporaBase(const std::shared_ptr<Zone>& zone,
                                      int32_t baseID, int32_t sourceEntityID,
                                      bool capture) {
  if (!zone) {
    return false;
  }
  ...
  if (bState && variant &&
        ...
        if (duration <= (uint32_t)variant->GetTimePoints(1)) {
          mServer.lock()->GetTimerManager()->ScheduleEventIn(
              ...
              [](MatchManager* pMatchManager, int32_t pBaseID, uint32_t pZoneID,
                 uint32_t pInstanceID, uint64_t pReset) {
                pMatchManager->ResetDiasporaBase(pZoneID, pInstanceID, pBaseID,
                                                 pReset);
              },
    ...
    auto triggers = zoneManager->GetZoneTriggers(
        zone, capture ? ZoneTrigger_t::ON_DIASPORA_BASE_CAPTURE
                      : ZoneTrigger_t::ON_DIASPORA_BASE_RESET);
    triggers.remove_if(
        [def](const std::shared_ptr<objects::ServerZoneTrigger>& t) {
          return t->GetValue() != 0 && (uint32_t)t->GetValue() != def->GetID();
        });
    ...
    if (capture) {
      ...
      if (obj && obj->ActionsCount() > 0) {
        server->GetActionManager()->PerformActions(nullptr, obj->GetActions(),
                                                   sourceEntityID, zone);
      }
    }
...
bool MatchManager::StartStopMatch(
    const std::shared_ptr<Zone>& zone,
  ...
  if (zone && (match != nullptr) == (zone->GetMatch() == nullptr)) {
    ...
    std::list<std::shared_ptr<Zone>> zones = {zone};
    {
      ...
      if (instance) {
        ...
        if (match) {
          ...
          match->SetInstanceDefinitionID(instance->GetDefinition()->GetID());
    ...
    if (ubMatch) {
      ...
      if (ubMatch->GetCategory() == objects::UBMatch::Category_t::UA) {
        ...
        // Start the queue
        UltimateBattleQueue(zone->GetDefinitionID(), zone->GetDynamicMapID(),
                            zone->GetInstanceID());
      }
    } else if (!match) {
...
bool MatchManager::StartUltimateBattle(const std::shared_ptr<Zone>& zone) {
  ...
  if (ubMatch->GetCategory() == objects::UBMatch::Category_t::UA) {
    ...
    auto server = mServer.lock();
    auto clients =
        server->GetManagerConnection()->GetEntityClients(accepted, true);
    if (clients.size() > 0) {
      libcomp::Packet notify;
      notify.WritePacketCode(
    ...
    clients = server->GetManagerConnection()->GetEntityClients(rejected, true);
    if (clients.size() > 0) {
      libcomp::Packet notify;
      notify.WritePacketCode(
          ChannelToClientPacketCode_t::PACKET_UB_LOTTO_RESULT);
    ...
    if (stopMatch) {
      LogMatchManagerDebugMsg("Skipping no entry Ultimate Battle match\n");
...
void MatchManager::UltimateBattleRecruit(uint32_t zoneID, uint32_t dynamicMapID,
                                         uint32_t instanceID) {
  auto server = mServer.lock();
  auto zoneManager = server->GetZoneManager();
  ...
  auto clients =
      server->GetManagerConnection()->GetEntityClients(pendingCIDs, true);
  ...
  libcomp::Packet notify;
  notify.WritePacketCode(ChannelToClientPacketCode_t::PACKET_UB_RECRUIT);
  notify.WriteS8(1);
  ...
  server->GetTimerManager()->ScheduleEventIn(
      ...
      [](std::shared_ptr<ChannelServer> pServer,
        ...
        libcomp::Packet p;
        p.WritePacketCode(ChannelToClientPacketCode_t::PACKET_UB_RECRUIT_START);
...
void MatchManager::UltimateBattleBegin(uint32_t zoneID, uint32_t dynamicMapID,
  ...
  {
    std::lock_guard<std::mutex> lock(mLock);
    if (!ubMatch || ubMatch->GetState() > objects::UBMatch::State_t::READY) {
      // Nothing to do
      return;
    }
  ...
  if (pendingCIDs.size() > 0) {
    ...
    libcomp::Packet p;
    p.WritePacketCode(ChannelToClientPacketCode_t::PACKET_UB_RECRUIT);
    p.WriteS8(0);
...
bool MatchManager::StartUltimateBattleTimer(const std::shared_ptr<Zone>& zone,
  ...
  uint64_t expire = (uint64_t)(serverTime + (uint64_t)(duration * 1000000ULL));
  ...
  LogMatchManagerDebug([&]() {
    return libcomp::String("Starting Ultimate Battle timer: %1s (phase %2)\n")
        .Arg(duration)
        .Arg(ubMatch->GetPhase());
  });
...
void MatchManager::UltimateBattleQueue(uint32_t zoneID, uint32_t dynamicMapID,
  ...
  int32_t timeLeft = expire > now ? (int32_t)((expire - now) / 1000000ULL) : 0;
  ...
  if (!timeLeft || !start || start > expire) {
    ...
    if (ubMatch->GetCategory() == objects::UBMatch::Category_t::UB) {
      ...
      auto lobby = lobbyDef
                       ? server->GetZoneManager()->GetGlobalZone(
                             lobbyDef->GetID(), lobbyDef->GetDynamicMapID())
                       : nullptr;
...
void MatchManager::UltimateBattleTick(uint32_t zoneID, uint32_t dynamicMapID,
  ...
  if (now >= next) {
    ...
    if (ubMatch->GetState() == objects::UBMatch::State_t::ROUND) {
      ...
      p.WriteU32Little(ubMatch->GetDarkLimit() &&
                               ubMatch->GetGauge() >= ubMatch->GetDarkLimit()
                           ? 1
                           : 0);
      ...
      auto triggers =
          zoneManager->GetZoneTriggers(zone, ZoneTrigger_t::ON_UB_TICK);
      for (auto trigger : zoneManager->GetZoneTriggers(
               zone, ZoneTrigger_t::ON_UB_GAUGE_OVER)) {
      ...
      for (auto trigger : zoneManager->GetZoneTriggers(
               zone, ZoneTrigger_t::ON_UB_GAUGE_UNDER)) {
        if (trigger->GetValue() > (int32_t)pGauge &&
            trigger->GetValue() <= (int32_t)gauge) {
          triggers.push_back(trigger);
        }
  ...
  if (expire && expire <= now) {
    ...
    if (!eventID.IsEmpty()) {
      server->GetEventManager()->HandleEvent(nullptr, eventID, 0, zone, {});
    }
  ...
  server->ScheduleWork(
      next,
      [](MatchManager* pMatchManager, uint32_t pZoneID, uint32_t pDynamicMapID,
         uint32_t pInstanceID) {
        pMatchManager->UltimateBattleTick(pZoneID, pDynamicMapID, pInstanceID);
      },
      this, zoneID, dynamicMapID, instanceID);
}

void MatchManager::SendPvPLocation(
    const std::shared_ptr<ChannelClientConnection>& client, uint32_t instanceID,
    bool enter) {
  auto server = mServer.lock();
  auto zoneManager = server->GetZoneManager();
  ...
  clients.remove_if(
      [client](const std::shared_ptr<ChannelClientConnection>& c) {
        return c == client;
      });
...
void MatchManager::SendDiasporaLocation(
    const std::shared_ptr<ChannelClientConnection>& client, uint32_t instanceID,
    bool enter, bool others) {
  auto server = mServer.lock();
  auto zoneManager = server->GetZoneManager();
  ...
  clients.remove_if([client](
    ...
    return c == client || !c->GetClientState()->GetCharacterState()->Ready();
  });
    ...
    for (auto result : results) {
      auto character =
          libcomp::PersistentObject::LoadObjectByUUID<objects::Character>(
              server->GetWorldDatabase(), result->GetCharacter());
      switch (i) {
        case 0:
...
void MatchManager::UpdateUBTournament(
...
void MatchManager::UpdateUBRankings(
  ...
  for (size_t i = 2; i < 4; i++) {
    mUBRankings[i] = {{nullptr, nullptr, nullptr, nullptr, nullptr, nullptr,
  ...
  for (auto* resultSet : {&allTime, &topPoint}) {
    mUBRankings[idx] = {{nullptr, nullptr, nullptr, nullptr, nullptr, nullptr,
                         nullptr, nullptr, nullptr, nullptr}};
...
void MatchManager::UpdatePentalphaMatch(
  ...
  if (mPentalphaMatches[0] != match) {
    ...
    if (match && !match->GetPrevious().IsNull()) {
      ...
      mPentalphaMatches[1] =
          libcomp::PersistentObject::LoadObjectByUUID<objects::PentalphaMatch>(
              db, match->GetPrevious());
    }
  }
}
    ...
    if (!remove->GetMatchID()) {
      ...
      if (client) {
        if (remove->GetTeamID()) {
          ...
          libcomp::Packet reply;
          reply.WritePacketCode(ChannelToClientPacketCode_t::PACKET_PVP_CANCEL);
          reply.WriteS8(0);
    ...
    for (auto entry : pvpEntries) {
      ...
      if (client) {
        ...
        if (joinSelf.find(cid) != joinSelf.end()) {
          ...
          int8_t updateCode = it != updateCodes.end() ? it->second : 0;
...
void MatchManager::UpdatePvPMatches(
  ...
  std::unordered_map<uint32_t, int32_t> localExpire;
  {
    ...
    for (auto match : matches) {
      int32_t confirmTime =
          ChannelServer::GetExpirationInSeconds(match->GetReadyTime(), now);
      ServerTime serverConfirmTime = (ServerTime)(
          serverTime + (uint64_t)confirmTime * (uint64_t)1000000ULL);
        ...
        for (int32_t cid : cids) {
          ...
          if (client) {
            if (match->GetNoQueue()) {
              ...
              notify.WriteS8(0);
              notify.WriteS32Little(
                  (int32_t)state->ToClientTime(serverConfirmTime));
  ...
  for (auto& client : noQueueJoin) {
    ...
    // confirmation first
    libcomp::Packet request;
    request.WritePacketCode(ChannelToClientPacketCode_t::PACKET_PVP_CONFIRM);
...
std::shared_ptr<objects::UBResult> MatchManager::LoadUltimateBattleData(
  ...
  if (idxFlags & 0x01) {
    ...
    if (current && (!result || (result->GetTournament().GetUUID() !=
                                current->GetUUID()))) {
      reload = true;
    }
  }
  ...
  if (reload) {
    ...
    bool missingCurrent = state->GetUltimateBattleData(0).IsNull() && current;
    if (createMissing) {
      auto dbChanges =
          libcomp::DatabaseChangeSet::Create(state->GetAccountUID());

      if ((idxFlags & 0x01) && missingCurrent) {
        auto result = libcomp::PersistentObject::New<objects::UBResult>(true);
...
bool MatchManager::SpectatingMatch(
    const std::shared_ptr<ChannelClientConnection>& client,
    const std::shared_ptr<Zone>& zone) {
  auto state = client->GetClientState();
  auto ubMatch = zone ? zone->GetUBMatch() : nullptr;
...
bool MatchManager::CreatePvPInstance(
  ...
  if (!variantDef || !variantDef->GetDefaultInstanceID()) {
    LogMatchManagerError([&]() {
      return libcomp::String(
                 "Invalid PvP variant encountered, match creation failed: %1\n")
  ...
  auto instance =
      server->GetZoneManager()->GetInstance(instAccess->GetInstanceID());
  if (!instance) {
    LogMatchManagerError([&]() {
      return libcomp::String("Failed to create PvP instance variant: %1\n")
          .Arg(match->GetVariantID());
    });
  ...
  int32_t startTime = ChannelServer::GetExpirationInSeconds(matchStart, now);

  instance->SetTimerExpire(
      (ServerTime)(serverTime + ((uint64_t)startTime +
                                 instance->GetVariant()->GetTimePoints(0)) *
                                    1000000ULL));
...
void MatchManager::QueuePendingPvPMatch(uint8_t type, uint32_t readyTime) {
  ...
  if (variantIDs.size() == 0) {
    LogMatchManagerError([&]() {
      return libcomp::String(
                 "PvP match queuing failed due to undefined match type "
                 "variants: %1\n")
          .Arg(type);
    });
  ...
  auto instDef = serverDataManager->GetZoneInstanceData(
      variantDef->GetDefaultInstanceID());
  ...
  pvpMatch->SetChannelID(server->GetWorldSharedConfig()->GetChannelDistribution(
      instDef->GetGroupID()));
...
void MatchManager::GetPvPTrophies(
  ...
  std::set<int8_t> validTrophies = {DAMAGE_FIRST,
                                    DAMAGE_TAKEN_FIRST,
                                    DAMAGE_MAX,
                                    DAMAGE_TAKEN_MAX,
                                    BASE_FIRST,
  ...
  for (auto cids : {match->GetBlueMemberIDs(), match->GetRedMemberIDs()}) {
    ...
    bool lost = pvpStats->GetPoints(idx) < pvpStats->GetPoints(otherIdx);
    ...
    switch (trophy) {
      ...
      case DAMAGE_MAX:
        ...
        for (auto stats : allPlayers) {
          maxMap[stats->GetEntityID()] = (double)(stats->GetDamageMax());
        }
        break;
      case DAMAGE_TAKEN_MAX:
        // Gather damage taken max
        for (auto stats : allPlayers) {
          maxMap[stats->GetEntityID()] = (double)(stats->GetDamageMaxTaken());
        }
        break;
      case BASE_FIRST:
      ...
      case KILL_MAX:
        // Gather kills
        for (auto stats : allPlayers) {
          maxMap[stats->GetEntityID()] =
              (double)(stats->GetKills() + stats->GetDemonKills());
        }
        break;
      case DEATH_MIN:
        ...
        for (auto stats : allPlayers) {
          maxMap[stats->GetEntityID()] =
              -(double)(stats->GetDeaths() + stats->GetDemonDeaths());
        }
        allowZero = true;
        break;
      case BSTATUS_MAX:
        ...
        for (auto stats : allPlayers) {
          maxMap[stats->GetEntityID()] = (double)(stats->GetBadStatus());
        }
        break;
      case GSTATUS_MAX:
        // Gather good status count
        for (auto stats : allPlayers) {
          maxMap[stats->GetEntityID()] = (double)(stats->GetGoodStatus());
        }
        break;
      case DAMAGE_SUM_MAX:
        // Gather damage dealt
        for (auto stats : allPlayers) {
          maxMap[stats->GetEntityID()] = (double)(stats->GetDamageSum());
        }
        break;
        ...
        for (auto stats : pList) {
          ...
          switch (trophy) {
            case BSTATUS_TAKEN_DRATE:
              ...
              key = (int32_t)(stats->GetDeaths() + stats->GetDemonDeaths());
            ...
            case WIN_MVP:
              ...
              key = (int32_t)(stats->GetDeaths() + stats->GetDemonDeaths());
              val = (int32_t)(stats->GetKills() + stats->GetDemonKills() +
                              stats->GetBasePoints() +
            ...
            default:
              // Gather deaths / kills
              key = (int32_t)(stats->GetKills() + stats->GetDemonKills());
              val = (int32_t)(stats->GetDeaths() + stats->GetDemonDeaths());
              break;
          }
        ...
        if (m.find(0) == m.end()) {
          ...
          for (auto& mPair : m) {
            for (auto& pair : mPair.second) {
              maxMap[pair.first] = (double)pair.second / (double)mPair.first;
            }
          }
        }
      } break;
      case BASE_POINT_MAX:
        // Gather base points
        for (auto stats : allPlayers) {
          maxMap[stats->GetEntityID()] = (double)(stats->GetBasePoints());
        }
        break;
      case BASE_BONUS_POINT_MAX:
        // Gather bonus points
        for (auto stats : allPlayers) {
          maxMap[stats->GetEntityID()] = (double)(stats->GetBaseBonusPoints());
        }
        break;
      ...
      case WIN_GP_MIN: {
        ...
        for (auto stats : losers) {
          ...
          if (pvpData) {
            maxMap[stats->GetEntityID()] = -(double)(pvpData->GetGP());
          }
        }
        allowZero = true;
      } break;
...
bool MatchManager::ValidateMatchEntries(
  ...
  if (!isTeam) {
    ...
    if (clients.size() < 2) {
      LogMatchManagerDebugMsg(
          "Match entry validation failed: teams must have at least 2 "
          "members\n");
    ...
    if (memberIDs.size() != clients.size()) {
      LogMatchManagerDebugMsg(
          "Match entry validation failed: one or more team members is missing "
          "from validation\n");
    ...
    for (auto client : clients) {
      ...
      if (memberIDs.find(worldCID) == memberIDs.end()) {
        LogMatchManagerDebug([&]() {
          return libcomp::String(
                     "Match entry validation failed: invalid team member "
                     "requested: %1\n")
              .Arg(client->GetClientState()->GetAccountUID().ToString());
        });
      ...
      if (client->GetClientState()->GetZone() != zone) {
        LogMatchManagerDebug([&]() {
          return libcomp::String(
                     "Match entry validation failed: team members must all be "
                     "in the same zone: %1\n")
              .Arg(client->GetClientState()->GetAccountUID().ToString());
        });
  ...
  for (auto client : clients) {
    if (client->GetClientState()->GetPendingMatch()) {
      LogMatchManagerDebug([&]() {
        return libcomp::String(
                   "Match entry validation failed: pending match exists: %1\n")
            .Arg(client->GetClientState()->GetAccountUID().ToString());
      });
    ...
    if (GetMatchEntry(client->GetClientState()->GetWorldCID())) {
      LogMatchManagerDebug([&]() {
        return libcomp::String(
                   "Match entry validation failed: existing queue match entry "
                   "exists: %1\n")
            .Arg(client->GetClientState()->GetAccountUID().ToString());
      });
    ...
    if (teamCategory == (int8_t)objects::Team::Category_t::PVP) {
      ...
      if (!pvpData) {
        LogMatchManagerError([&]() {
          return libcomp::String(
                     "PvP entry validation failed: PvPData could not be "
                     "created: %1\n")
              .Arg(client->GetClientState()->GetAccountUID().ToString());
        });
      ...
      if (checkPenalties && pvpData->GetPenaltyCount() >= 3) {
        LogMatchManagerDebug([&]() {
          return libcomp::String(
                     "PvP entry validation failed: too many penalties exist: "
                     "%1\n")
              .Arg(client->GetClientState()->GetAccountUID().ToString());
        });
...
bool MatchManager::EndUltimateBattlePhase(const std::shared_ptr<Zone>& zone,
  ...
  for (auto client : clients) {
    ...
    if (tournament && matchOver) {
      ...
      if (current && allTime) {
        ...
        LogMatchManagerError([&]() {
          return libcomp::String(
                     "Failed to load Ultimate Battle data for character when "
                     "ending match: %1\n")
  ...
  players.sort([ubMatch](const std::shared_ptr<ChannelClientConnection>& a,
  ...
  for (auto client : players) {
    ...
    notify.WriteU32Little((uint32_t)ubMatch->GetCoins(state->GetWorldCID()));
  }
...
void MatchManager::QueueNextBaseBonus(int32_t baseID,
  ...
  if (PvPActive(instance)) {
    ...
    mServer.lock()->GetTimerManager()->ScheduleEventIn(
        ...
        this, baseID, zone->GetID(), instance->GetID(), occupyStartTime);
  }
}
...
void MatchManager::FirePhaseTriggers(const std::shared_ptr<Zone>& zone,
  ...
  for (auto z : zones) {
    for (auto trigger :
         zoneManager->GetZoneTriggers(z, ZoneTrigger_t::ON_PHASE)) {
...
void MatchManager::SendPhase(
  ...
  switch (match->GetType()) {
    case objects::Match::Type_t::DIASPORA: {
      libcomp::Packet p;
      p.WritePacketCode(ChannelToClientPacketCode_t::PACKET_DIASPORA_PHASE);
    ...
    case objects::Match::Type_t::ULTIMATE_BATTLE: {
      ...
      if (timerStart) {
        ...
        uint64_t start = ubMatch->GetTimerStart();
        timeLeft =
            (start < stop) ? (float)((double)(stop - start) / 1000000.0) : 0.f;
      } else {
        timeLeft =
            (now < stop) ? (float)((double)(stop - now) / 1000000.0) : 0.f;
      }
      ...
      libcomp::Packet p;
      p.WritePacketCode(ChannelToClientPacketCode_t::PACKET_UB_PHASE);
      p.WriteU32Little(ubMatch->GetSubType());
...
void MatchManager::SendUltimateBattleMembers(
    const std::shared_ptr<Zone>& zone,
    const std::shared_ptr<ChannelClientConnection>& client) {
  auto ubMatch = zone ? zone->GetUBMatch() : nullptr;
  if (!ubMatch) {
  ...
  auto playerClients = mServer.lock()->GetManagerConnection()->GetEntityClients(
      ubMatch->GetMemberIDs(), true);

  playerClients.remove_if(
      [zone](const std::shared_ptr<ChannelClientConnection>& c) {
        return c->GetClientState()->GetZone() != zone;
      });

  libcomp::Packet notify;
  notify.WritePacketCode(ChannelToClientPacketCode_t::PACKET_UB_MEMBERS);
  notify.WriteU32Little(ubMatch->GetSubType());
...
bool MatchManager::MatchTeamsActive(
    const std::shared_ptr<ZoneInstance>& instance) {
  auto clients = instance->GetConnections();
  if (clients.size() == 0) {
    // No one in zone
...
bool MatchManager::IsPvPMatchEntry(
    const std::shared_ptr<objects::MatchEntry>& entry) {
  return entry->GetMatchType() == objects::MatchEntry::MatchType_t::PVP_FATE ||
         entry->GetMatchType() ==
