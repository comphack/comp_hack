...
SpawnList::~SpawnList() { delete prop; }
...
QString SpawnList::GetObjectName(
    const std::shared_ptr<libcomp::Object>& obj) const {
  auto spawn = std::dynamic_pointer_cast<objects::Spawn>(obj);
...
void SpawnList::LoadProperties(const std::shared_ptr<libcomp::Object>& obj) {
  ...
  if (spawn) {
    ...
    prop->killValueType->setCurrentIndex(
        to_underlying(spawn->GetKillValueType()));
    prop->bossGroup->setValue((int32_t)spawn->GetBossGroup());
    prop->factionGroup->setValue(spawn->GetFactionGroup());
    prop->chkValidDQuestTarget->setChecked(spawn->GetValidDemonQuestTarget());
  } else {
...
void SpawnList::SaveProperties(const std::shared_ptr<libcomp::Object>& obj) {
  ...
  if (spawn) {
    ...
    spawn->SetCategory(
        (objects::Spawn::Category_t)prop->category->currentIndex());
    ...
    spawn->SetKillValueType(
        (objects::Spawn::KillValueType_t)prop->killValueType->currentIndex());
    spawn->SetBossGroup((uint8_t)prop->bossGroup->value());
    spawn->SetFactionGroup(prop->factionGroup->value());
    spawn->SetValidDemonQuestTarget(prop->chkValidDQuestTarget->isChecked());
  }
}
...
void SpawnList::UpdateAIDisplay() {
  ...
  if (demonType && mMainWindow) {
    auto devilData = std::dynamic_pointer_cast<objects::MiDevilData>(
        mMainWindow->GetBinaryDataSet("DevilData")->GetObjectByID(demonType));
    ...
    if (aiData) {
      if (aiData->GetAggroLimit()) {
        ...
        prop->aiAggroLimit->setText(qs(libcomp::String("%1 (Rank %2)")
                                           .Arg(aggroMax)
                                           .Arg(aiData->GetAggroLimit())));
      } else {
        prop->aiAggroLimit->setText("1 (Rank 0)");
      }

      prop->aiLevelLimit->setText(
          qs(libcomp::String("+%1").Arg(aiData->GetAggroLevelLimit())));
      prop->aiAggroDay->setText(
          qs(libcomp::String("%1 (%2)")
      ...
      prop->aiDeaggroScale->setText(
          qs(libcomp::String("x%1").Arg(aiData->GetDeaggroScale())));
      prop->aiThinkSpeed->setText(
          qs(libcomp::String("%1 ms").Arg(aiData->GetThinkSpeed())));
      prop->chkAINormalSkillUse->setChecked(aiData->GetNormalSkillUse());
      prop->chkAIStrikeFirst->setChecked(aiData->GetStrikeFirst());
    } else {
      reset = true;
    }
  } else {
