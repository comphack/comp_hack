/**
 * @file libhack/src/ServerConstants.h
 * @ingroup libhack
 *
 * @author HACKfrost
 *
...
#ifndef LIBHACK_SRC_SERVERCONSTANTS_H
#define LIBHACK_SRC_SERVERCONSTANTS_H
...
class ServerConstants {
 ...
 public:
   ...
   */
  static bool Initialize(const libcomp::String& filePath);
 ...
 private:
   ...
   */
  static bool LoadString(const std::string& value, libcomp::String& prop);
  ...
  static bool ToIntegerArray(std::array<T, SIZE>& prop,
                             std::list<libcomp::String> values) {
    if (SIZE != values.size()) {
      return false;
    }
...
#define SVR_CONST libhack::ServerConstants::GetConstants()

#endif  // LIBHACK_SRC_SERVERCONSTANTS_H
