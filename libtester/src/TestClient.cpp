...
TestClient::TestClient(const TestClient& other)
    : mTimer(mService),
      mMessageQueue(new libcomp::MessageQueue<libcomp::Message::Message*>()) {
  (void)other;
...
void TestClient::Disconnect() { mConnection->Close(); }
...
bool TestClient::WaitForDisconnect(double& waitTime,
  ...
  return WaitForMessage(
      [](const MessageList& msgs) {
        for (auto msg : msgs) {
          if (nullptr !=
              dynamic_cast<libcomp::Message::ConnectionClosed*>(msg)) {
            return WaitStatus::Success;
          }
        }
...
bool TestClient::WaitForPacket(uint16_t code, libcomp::ReadOnlyPacket& p,
                               double& waitTime,
                               asio::steady_timer::duration timeout) {
  bool result = WaitForMessage(
      [&](const MessageList& msgs) {
        for (auto msg : msgs) {
  ...
  if (result) {
    ...
    for (auto msg : mReceivedMessages) {
      ...
      if (nullptr != pmsg) {
        ...
        HandlePacket(static_cast<ChannelToClientPacketCode_t>(cmdCode), copy);
    ...
    badMessages =
        static_cast<int>(mReceivedMessages.size()) - (foundPacket ? 1 : 0);
...
bool TestClient::WaitForMessage(
    std::function<WaitStatus(const MessageList&)> eventFilter, double& waitTime,
    asio::steady_timer::duration timeout) {
  bool result = true;
