...
EventCondition::EventCondition(MainWindow* pMainWindow, QWidget* pParent)
  ...
  ui->typeNormal->addItem(
      "Instance Access", (int)objects::EventCondition::Type_t::INSTANCE_ACCESS);
  ui->typeNormal->addItem("Item", (int)objects::EventCondition::Type_t::ITEM);
  ui->typeNormal->addItem("Inventory Free",
                          (int)objects::EventCondition::Type_t::INVENTORY_FREE);
  ui->typeNormal->addItem("Level", (int)objects::EventCondition::Type_t::LEVEL);
  ui->typeNormal->addItem("LNC", (int)objects::EventCondition::Type_t::LNC);
  ui->typeNormal->addItem("LNC Type",
                          (int)objects::EventCondition::Type_t::LNC_TYPE);
  ui->typeNormal->addItem("Map", (int)objects::EventCondition::Type_t::MAP);
  ...
  ui->typeNormal->addItem(
      "Quest Available", (int)objects::EventCondition::Type_t::QUEST_AVAILABLE);
  ...
  ui->typeNormal->addItem(
      "Title Available", (int)objects::EventCondition::Type_t::TITLE_AVAILABLE);
...
EventCondition::~EventCondition() { delete ui; }
...
void EventCondition::Load(const std::shared_ptr<objects::EventCondition>& e) {
  ...
  ui->value1Selector->SetValue(e->GetValue1() >= 0 ? (uint32_t)e->GetValue1()
                                                   : 0);
  ...
  auto flagCondition =
      std::dynamic_pointer_cast<objects::EventFlagCondition>(e);
  auto scriptCondition =
      std::dynamic_pointer_cast<objects::EventScriptCondition>(e);
...
std::shared_ptr<objects::EventCondition> EventCondition::Save() const {
  ...
  condition->SetCompareMode(
      (objects::EventCondition::CompareMode_t)ui->compareMode->currentData()
          .value<int>());
...
void EventCondition::RadioToggle() {
  ...
  if (ui->typeFlags->isEnabled() && !ui->radFlags->isChecked()) {
...
void EventCondition::TypeSelectionChanged() { RefreshTypeContext(); }

objects::EventCondition::Type_t EventCondition::GetCurrentType() const {
  if (ui->radFlags->isChecked()) {
    return (objects::EventCondition::Type_t)ui->typeFlags->currentData()
        .value<int>();
  } else if (ui->radScript->isChecked()) {
    return objects::EventCondition::Type_t::SCRIPT;
  } else {
    return (objects::EventCondition::Type_t)ui->typeNormal->currentData()
        .value<int>();
  }
}
  ...
  switch (GetCurrentType()) {
    ...
    case objects::EventCondition::Type_t::DEMON_BOOK:
      if (objects::EventCondition::CompareMode_t::EXISTS ==
          (objects::EventCondition::CompareMode_t)currentData.value<int>()) {
        ui->lblValue1->setText("Demon Type:");
...
void EventCondition::ResetCompareMode(
  ...
  ui->compareMode->addItem(
      qs(defaultCompareTxt.IsEmpty()
             ? "Default"
             : libcomp::String("Default (%1)").Arg(defaultCompareTxt)),
  ...
  ui->compareMode->addItem("Exists",
                           (int)objects::EventCondition::CompareMode_t::EXISTS);
  ui->compareMode->addItem(
      "LT (or NaN)", (int)objects::EventCondition::CompareMode_t::LT_OR_NAN);
  ui->compareMode->addItem("LT",
                           (int)objects::EventCondition::CompareMode_t::LT);
  ui->compareMode->addItem("GTE",
                           (int)objects::EventCondition::CompareMode_t::GTE);
  ui->compareMode->addItem(
      "Between", (int)objects::EventCondition::CompareMode_t::BETWEEN);
}
