...
int main(int argc, char *argv[]) {
...
#ifdef Q_OS_WIN32
    QString defaultPath =
        QString("%1/My Documents/Captures").arg(QDir::homePath());
#else   // Q_OS_UNIX
    QString defaultPath = QString("%1/Captures").arg(QDir::homePath());
#endif  // Q_OS_WIN32

    capturePath = settings.value("General/CapturePath", defaultPath).toString();
  ...
  std::cout << "----------------------------------------"
            << "----------------------------------------" << std::endl;
  std::cout << "Capture Directory: " << capturePath.toLocal8Bit().data()
            << std::endl;
  std::cout << "----------------------------------------"
            << "----------------------------------------" << std::endl;
