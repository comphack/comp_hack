...
const uint16_t EVENT_COMPARE_NUMERIC2 =
    EVENT_COMPARE_NUMERIC | (uint16_t)EventCompareMode::BETWEEN;
...
bool EventManager::HandleEvent(
  ...
  if (instance) {
    ...
    ctx.CurrentZone =
        client ? client->GetClientState()->GetCharacterState()->GetZone()
               : zone;
...
bool EventManager::StartSystemEvent(
    const std::shared_ptr<ChannelClientConnection>& client,
...
int32_t EventManager::InterruptEvent(
    const std::shared_ptr<ChannelClientConnection>& client) {
  if (!client) {
    return 0;
  }
...
bool EventManager::RequestMenu(
    const std::shared_ptr<ChannelClientConnection>& client, int32_t menuType,
    int32_t shopID, int32_t sourceEntityID, bool allowInsert) {
  auto state = client->GetClientState();
  auto eState = state->GetEventState();
  auto current = eState->GetCurrent();
  ...
  if (!allowInsert && current) {
    if (state->GetCurrentMenuShopID(menuType) == shopID) {
      ...
      LogEventManagerDebug([&]() {
        return libcomp::String(
                   "Attempted to open menu type '%1' for character that was "
                   "already in menu on account: %2\n")
            .Arg(menuType)
    ...
    LogEventManagerError([&]() {
      return libcomp::String(
                 "Attempted to open menu type '%1' for character already in an "
                 "event on account: %2\n")
          .Arg(menuType)
          .Arg(state->GetAccountUID().ToString());
    });
  ...
  menu->SetID(libcomp::String("SYSTEM:MENU_%1_%2").Arg(menuType).Arg(shopID));
...
bool EventManager::HandleResponse(
    const std::shared_ptr<ChannelClientConnection>& client, int32_t responseID,
    int64_t objectID) {
  auto state = client->GetClientState();
  auto eState = state->GetEventState();
  ...
  switch (eventType) {
    case objects::Event::EventType_t::NPC_MESSAGE:
      if (responseID != 0) {
        ...
        // If there are still more messages, increment and continue the same
        // event
    ...
    case objects::Event::EventType_t::PROMPT: {
      ...
      for (size_t i = 0;
           i < e->ChoicesCount() && i <= (size_t)adjustedResponseID; i++) {
      ...
      if (choice == nullptr) {
        LogEventManagerError([&]() {
          return libcomp::String("Invalid choice %1 selected for event %2\n")
              .Arg(responseID)
              .Arg(e->GetID());
        });
      } else {
    ...
    case objects::Event::EventType_t::ITIME: {
      ...
      if (e->GiftIDsCount() > 0) {
        ...
        if (item) {
          ...
          items[itemType] = 1;
          if (!mServer.lock()->GetCharacterManager()->AddRemoveItems(
                  client, items, false, objectID)) {
        ...
        if (next.IsEmpty() ||
            !HandleEvent(client, next, current->GetSourceEntityID())) {
          EndEvent(client);
        }
        ...
        // Normal interaction
        bool doNext =
            e->ChoicesCount() == 0 || responseID < 0 || responseID >= 4;
        if (!doNext && current->DisabledChoicesContains((uint8_t)responseID)) {
        ...
        if (!doNext) {
          ...
          if (choice == nullptr) {
            LogEventManagerError([&]() {
              return libcomp::String(
                         "Invalid choice %1 selected for event %2\n")
                  .Arg(responseID)
                  .Arg(e->GetID());
            });
          } else {
    ...
    case objects::Event::EventType_t::OPEN_MENU:
      if (responseID == -1) {
        ...
        LogEventManagerError([&]() {
          return libcomp::String("Non-zero response %1 received for menu %2\n")
              .Arg(responseID)
              .Arg(event->GetID());
        });
      }
    ...
    case objects::Event::EventType_t::MULTITALK:
      if (responseID != 0) {
        LogEventManagerError([&]() {
          return libcomp::String("Non-zero response %1 received for event %2\n")
              .Arg(responseID)
              .Arg(event->GetID());
        });
      }
    ...
    default:
      LogEventManagerError([&]() {
        return libcomp::String(
                   "Response received for invalid event of type %1\n")
...
bool EventManager::ContinueChannelChangeEvent(
  ...
  if (!instance) {
    LogEventManagerError([&]() {
      return libcomp::String(
                 "Unable to continue event '%1' after channel change for "
                 "acount: %1\n")
  ...
  if (instance->GetEvent()->GetEventType() ==
    ...
    if (actions.size() > 0) {
      ...
      mServer.lock()->GetActionManager()->PerformActions(
          client, actions, 0, state->GetZone(), options);
    }
  }
...
bool EventManager::UpdateQuest(
    const std::shared_ptr<ChannelClientConnection>& client, int16_t questID,
    int8_t phase, bool forceUpdate,
  ...
  if (!questData) {
    LogEventManagerError([&]() {
      return libcomp::String("Invalid quest ID supplied for UpdateQuest: %1\n")
          .Arg(questID);
    });
  ...
  if (phase == -1) {
    ...
    if (!quest && completed && !forceUpdate) {
      LogEventManagerError([&]() {
        return libcomp::String("Quest '%1' has already been completed\n")
            .Arg(questID);
      });
    ...
    recalcCharacter =
        cState->UpdateQuestState(definitionManager, (uint32_t)questID);
    ...
    if (!forceUpdate && completed &&
      ...
      LogEventManagerError([questID]() {
        return libcomp::String(
                   "Already completed non-repeatable quest '%1' cannot be "
                   "started again\n")
            .Arg(questID);
      });
...
void EventManager::UpdateQuestKillCount(
    const std::shared_ptr<ChannelClientConnection>& client,
  ...
  for (auto qPair : character->GetQuests()) {
    ...
    for (uint32_t i = 0; i < phaseData->GetRequirementCount(); i++) {
      ...
      if ((req->GetType() == objects::QuestPhaseRequirement::Type_t::KILL ||
           req->GetType() ==
               objects::QuestPhaseRequirement::Type_t::KILL_HIDDEN) &&
          it != kills.end()) {
  ...
  if (dQuest) {
    for (auto& targetPair : dQuest->GetTargets()) {
      ...
      if (it != kills.end() &&
        ...
        UpdateDemonQuestCount(client, dQuest->GetType(), it->first, it->second);
      }
    }
  }
}
...
bool EventManager::EvaluateQuestConditions(EventContext& ctx, int16_t questID) {
  ...
  if (!questData) {
    LogEventManagerError([&]() {
      return libcomp::String(
                 "Invalid quest ID supplied for EvaluateQuestConditions: %1\n")
          .Arg(questID);
    });
...
bool EventManager::EvaluateEventCondition(
    EventContext& ctx,
  ...
  switch (condition->GetType()) {
    case objects::EventCondition::Type_t::SCRIPT: {
      auto scriptCondition =
          std::dynamic_pointer_cast<objects::EventScriptCondition>(condition);
      if (!scriptCondition) {
      ...
      auto script =
          serverDataManager->GetScript(scriptCondition->GetScriptID());
      if (ctx.Client && !ctx.CurrentZone) {
        LogEventManagerError([&]() {
          return libcomp::String(
                     "Attempted to execute a client targeted condition script "
                     "ID outside of a zone: %1\n")
          ...
          auto scriptResult =
              !f.IsNull()
                  ? f.Evaluate<int32_t>(
                        ...
                        ctx.CurrentZone, scriptCondition->GetValue1(),
                        scriptCondition->GetValue2(), sqParams)
                  : 0;
          if (scriptResult) {
        ...
        LogEventManagerError([&]() {
          return libcomp::String("Invalid event condition script ID: %1\n")
              .Arg(scriptCondition->GetScriptID());
        });
      }
    } break;
    ...
    case objects::EventCondition::Type_t::ZONE_INSTANCE_CHARACTER_FLAGS: {
      ...
      switch (condition->GetType()) {
        ...
        case objects::EventCondition::Type_t::ZONE_CHARACTER_FLAGS:
          if (client) {
            ...
            LogEventManagerError([eventID]() {
              return libcomp::String(
                         "Attempted to check zone character flags with no "
                         "associated client: %1\n")
                  .Arg(eventID);
            });
        ...
        case objects::EventCondition::Type_t::ZONE_INSTANCE_CHARACTER_FLAGS:
          if (client) {
            ...
            LogEventManagerError([eventID]() {
              return libcomp::String(
                         "Attempted to check zone instance character flags "
                         "with no associated client: %1\n")
                  .Arg(eventID);
            });
      ...
      auto flagCon =
          std::dynamic_pointer_cast<objects::EventFlagCondition>(condition);
      if (zone && flagCon) {
...
bool EventManager::EvaluatePartnerCondition(
    const std::shared_ptr<ChannelClientConnection>& client,
    const std::shared_ptr<objects::EventCondition>& condition) {
  auto state = client->GetClientState();
  ...
  switch (condition->GetType()) {
    ...
    case objects::EventCondition::Type_t::PARTNER_STAT_VALUE: {
      ...
      return Compare(
          (int32_t)dState->GetCorrectValue((CorrectTbl)condition->GetValue1()),
...
bool EventManager::EvaluateQuestCondition(
    EventContext& ctx,
    const std::shared_ptr<objects::EventCondition>& condition) {
  if (!ctx.Client) {
    return false;
  ...
  switch (condition->GetType()) {
    case objects::EventCondition::Type_t::QUEST_AVAILABLE: {
      // If the quest is active or completed and not-repeatable, it is not
      // available If neither of those are true, eveluate its starting
      // conditions
      auto progress = character->GetProgress();

      size_t index;
      uint8_t shiftVal;
      CharacterManager::ConvertIDToMaskValues((uint16_t)questID, index,
                                              shiftVal);
      ...
      return quest == nullptr &&
             (!completed || questData->GetType() ==
                                objects::MiQuestData::Type_t::REPEATABLE) &&
             EvaluateQuestConditions(ctx, questID);
    }
    case objects::EventCondition::Type_t::QUEST_PHASE:
      if (quest) {
        return Compare((int32_t)quest->GetPhase(), condition->GetValue2(), 0,
                       compareMode, EventCompareMode::EQUAL,
                       EVENT_COMPARE_NUMERIC);
    ...
    case objects::EventCondition::Type_t::QUEST_PHASE_REQUIREMENTS:
      return quest && EvaluateQuestPhaseRequirements(
                          ctx.Client, questID, (int8_t)condition->GetValue2());
    case objects::EventCondition::Type_t::QUEST_FLAGS:
...
bool EventManager::EvaluateFlagStates(
    const std::unordered_map<int32_t, int32_t>& flagStates,
...
bool EventManager::Compare(int32_t value1, int32_t value2, int32_t value3,
                           ...
                           EventCompareMode defaultCompare,
  ...
  if (compareMode == EventCompareMode::DEFAULT_COMPARE) {
    if (defaultCompare == EventCompareMode::DEFAULT_COMPARE) {
      LogEventManagerErrorMsg(
          "Default comparison specified for non-defaulted comparison\n");
...
bool EventManager::EvaluateCondition(
    EventContext& ctx, const std::shared_ptr<ActiveEntityState>& eState,
  ...
  switch (condition->GetType()) {
    case objects::EventConditionData::Type_t::LEVEL:
      if (!eState) {
        ...
        return Compare(eState->GetLevel(), condition->GetValue1(),
                       condition->GetValue2(), compareMode,
                       EventCompareMode::GTE, EVENT_COMPARE_NUMERIC2);
      }
    ...
    case objects::EventConditionData::Type_t::TIMESPAN_WEEK:
      if (compareMode != EventCompareMode::BETWEEN &&
        ...
        int8_t minMinutes =
            (int8_t)(val1 - (minDays * 10000) - (minHours * 100));

        int8_t maxDays = (int8_t)(val2 / 10000);
        int8_t maxHours = (int8_t)((val2 - (maxDays * 10000)) / 100);
        int8_t maxMinutes =
            (int8_t)(val2 - (maxDays * 10000) - (maxHours * 100));

        bool skipDay = minDays == 7 && maxDays == 7;

        uint16_t systemSum =
            (uint16_t)(((skipDay ? 0 : (clock.WeekDay - 1)) * 24 * 60 * 60) +
    ...
    case objects::EventConditionData::Type_t::QUEST_ACTIVE:
      if (!client || (compareMode != EventCompareMode::EQUAL &&
        ...
        // Quest ID [value 1] active check = [value 2] (1 for active, 0 for not
        // active)
        auto character =
            client->GetClientState()->GetCharacterState()->GetEntity();

        return character->GetQuests((int16_t)condition->GetValue1()).IsNull() ==
               (condition->GetValue2() == 0);
      }
    case objects::EventConditionData::Type_t::QUEST_SEQUENCE:
      if (!client || (compareMode != EventCompareMode::EQUAL &&
        ...
        // Quest ID [value 1] is on its final phase (since this will progress
        // the story)
        ...
        auto prevQuestData =
            definitionManager->GetQuestData((uint32_t)prevQuestID);

        if (prevQuestData == nullptr) {
          LogEventManagerError([&]() {
            return libcomp::String(
                       "Invalid previous quest ID supplied for "
                       "EvaluateCondition: %1\n")
                .Arg(prevQuestID);
          });
        ...
        return prevQuestData->GetPhaseCount() ==
               (uint32_t)(prevQuest->GetPhase() + 1);
      }
    ...
    case objects::EventConditionData::Type_t::EXPERTISE_CLASS_OBTAINABLE:
      if (!client) {
        ...
        int32_t maxTotalPoints =
            server->GetCharacterManager()->GetMaxExpertisePoints(character);
        if (maxTotalPoints < 0) {
          // No (functional) limit
        ...
        switch (condition->GetType()) {
          case objects::EventConditionData::Type_t::EXPERTISE_POINTS_OBTAINABLE:
            ...
            if ((compareMode != EventCompareMode::EQUAL &&
              ...
              int32_t points = cState->GetExpertisePoints(
                  (uint32_t)condition->GetValue1(), definitionManager);
              int32_t required = condition->GetValue2();
              return required > points && (points + remaining) >= required;
            }
          case objects::EventConditionData::Type_t::EXPERTISE_CLASS_OBTAINABLE:
        ...
        if (condition->GetValue1() > -1 && remaining) {
          ...
          // total left
          auto expDef = definitionManager->GetExpertClassData(
        ...
        return Compare(remaining, condition->GetValue2(), 0, compareMode,
                       EventCompareMode::GTE, EVENT_COMPARE_NUMERIC);
      }
    case objects::EventConditionData::Type_t::EXPERTISE:
      if (!client) {
        ...
        int32_t compareTo =
            cState->GetExpertisePoints((uint32_t)condition->GetValue1(),
    ...
    case objects::EventCondition::Type_t::BETHEL:
      if (!client) {
        ...
        int32_t bethel =
            progress ? progress->GetBethel((size_t)condition->GetValue1()) : 0;

        return Compare(bethel, condition->GetValue2(), 0, compareMode,
                       EventCompareMode::GTE, EVENT_COMPARE_NUMERIC);
      }
    ...
    case objects::EventConditionData::Type_t::COMP_FREE:
      if (!client) {
        ...
        return Compare(freeCount, condition->GetValue1(),
                       condition->GetValue2(), compareMode,
    ...
    case objects::EventConditionData::Type_t::COWRIE:
      if (!client) {
        ...
        return Compare(cowrie, condition->GetValue1(), condition->GetValue2(),
                       compareMode, EventCompareMode::GTE,
                       EVENT_COMPARE_NUMERIC2);
      }
    case objects::EventCondition::Type_t::DEMON_BOOK:
      if (!client) {
        ...
        // Demon ID ([value 2] = 0) or base demon ID ([value 2] != 0) matching
        // [value 1] exists in the compendium
        auto server = mServer.lock();
        auto definitionManager = server->GetDefinitionManager();
        ...
        return Compare((int32_t)dState->GetCompendiumCount(),
      ...
      if (!ctx.CurrentZone ||
        ...
        for (auto bState : ctx.CurrentZone->GetDiasporaBases()) {
          ...
          if ((int32_t)def->GetLetter() == condition->GetValue1()) {
            return base->GetCaptured() == (condition->GetValue2() == 1);
          }
        }
    ...
    case objects::EventCondition::Type_t::EXPERTISE_ACTIVE:
      if (!client || (compareMode != EventCompareMode::EQUAL &&
        ...
        // Expertise ID [value 1] is active ([value 2] != 1) or locked ([value
        // 2] = 1)
        auto character =
            client->GetClientState()->GetCharacterState()->GetEntity();

        auto exp =
            character->GetExpertises((size_t)condition->GetValue1()).Get();
        return condition->GetValue2() == 1 ? (!exp || exp->GetDisabled())
                                           : (exp && !exp->GetDisabled());
      }
    ...
    case objects::EventCondition::Type_t::EVENT_COUNTER:
      if (!client) {
        ...
        return Compare(counter ? counter->GetCounter() : 0,
                       condition->GetValue2(), 0, compareMode,
                       EventCompareMode::GTE, EVENT_COMPARE_NUMERIC);
      }
    case objects::EventCondition::Type_t::EVENT_WORLD_COUNTER: {
      // World event counter [value 1] compares to [value 2]
      auto counter =
          mServer.lock()->GetChannelSyncManager()->GetWorldEventCounter(
              condition->GetValue1());
      if (compareMode == EventCompareMode::EXISTS) {
        return counter != nullptr;
      }
    ...
    case objects::EventCondition::Type_t::FACTION_GROUP:
      if (!eState) {
        ...
        return Compare(eState->GetFactionGroup(), condition->GetValue1(),
                       condition->GetValue2(), compareMode,
                       EventCompareMode::EQUAL, EVENT_COMPARE_NUMERIC2);
      }
        ...
        if (compareMode == EventCompareMode::EXISTS) {
          ...
          switch (condition->GetValue2()) {
            case 1:
              ...
              {
                auto instVar = access && access->GetVariantID()
                                   ? server->GetServerDataManager()
                                         ->GetZoneInstanceVariantData(
                                             access->GetVariantID())
                                   : nullptr;
                return instVar &&
                       (int32_t)instVar->GetID() == condition->GetValue1();
              }
              break;
            case 2:
            ...
            default:
              // Current zone is part of the instance they have access to
              {
                ...
                auto currentInstance = zone ? zone->GetInstance() : nullptr;

                auto def =
                    access
                        ? server->GetServerDataManager()->GetZoneInstanceData(
                              access->GetDefinitionID())
                        : nullptr;
                auto currentDef = currentInstance
                                      ? currentInstance->GetDefinition()
                                      : nullptr;
                auto currentZoneDef = zone ? zone->GetDefinition() : nullptr;

                // true if the instance is the same, the lobby is the
                // same or they are in the lobby
                return (currentInstance && !access) ||
                       (def && ((currentDef && def->GetLobbyID() ==
                                                   currentDef->GetLobbyID()) ||
    ...
    case objects::EventConditionData::Type_t::MATERIAL:
      if (!client) {
        ...
        return Compare(
            (int32_t)character->GetMaterials((uint32_t)condition->GetValue1()),
            condition->GetValue2(), 0, compareMode, EventCompareMode::GTE,
            EVENT_COMPARE_NUMERIC);
      }
      ...
      if (!ctx.CurrentZone) {
        ...
        if (condition->GetValue2() == -1) {
          ...
          for (auto c : ctx.CurrentZone->GetConnectionList()) {
            ...
            std::list<std::shared_ptr<objects::EventInstance>> eInstances;
            if (eventState) {
            ...
            for (auto eInst : eInstances) {
              if (eInst && eInst->GetSourceEntityID() == npc->GetEntityID()) {
                return true;
              }
            }
          }
          ...
          return Compare((int32_t)npcState, condition->GetValue2(), 0,
                         compareMode, EventCompareMode::EQUAL,
                         EVENT_COMPARE_NUMERIC);
        }
      }
    case objects::EventCondition::Type_t::PARTY_SIZE:
      if (!client) {
        ...
        return Compare((int32_t)(party ? party->MemberIDsCount() : 0),
                       condition->GetValue1(), condition->GetValue2(),
                       compareMode, EventCompareMode::BETWEEN,
                       EVENT_COMPARE_NUMERIC2);
      }
    case objects::EventConditionData::Type_t::PENTALPHA_TEAM:
      if (!client) {
        ...
        return pEntry &&
               Compare((int32_t)pEntry->GetTeam(), condition->GetValue1(),
                       condition->GetValue2(), compareMode,
                       EventCompareMode::BETWEEN, EVENT_COMPARE_NUMERIC2);
      }
    ...
    case objects::EventCondition::Type_t::STAT_VALUE:
      if (!eState) {
        ...
        return Compare((int32_t)eState->GetCorrectValue(
                           (CorrectTbl)condition->GetValue1()),
                       condition->GetValue2(), 0, compareMode,
                       EventCompareMode::GTE, EVENT_COMPARE_NUMERIC);
      }
    case objects::EventCondition::Type_t::STATUS_ACTIVE:
      if (!eState || (compareMode != EventCompareMode::EXISTS &&
        ...
        return aState &&
               aState->StatusEffectActive((uint32_t)condition->GetValue1());
      }
    case objects::EventCondition::Type_t::TEAM_CATEGORY:
      if (!client || (compareMode != EventCompareMode::EQUAL &&
        ...
        return team && (int32_t)team->GetCategory() == condition->GetValue1();
      }
    case objects::EventCondition::Type_t::TEAM_LEADER:
      if (!client || (compareMode != EventCompareMode::EQUAL &&
        ...
        return team && (int32_t)team->GetLeaderCID() == state->GetWorldCID();
      }
    ...
    case objects::EventCondition::Type_t::TIMESPAN_DATETIME:
      if (compareMode != EventCompareMode::BETWEEN &&
        ...
        int32_t systemSum = (clock.Month * 1000000) + (clock.Day * 10000) +
                            (clock.SystemHour * 100) + clock.SystemMin;
    ...
    case objects::EventCondition::Type_t::TITLE_AVAILABLE:
      if (!client || (compareMode != EventCompareMode::EQUAL &&
        ...
        if (titleID < (int16_t)(progress->SpecialTitlesCount() * 8)) {
          ...
          size_t index;
          uint8_t shiftVal;
          CharacterManager::ConvertIDToMaskValues((uint16_t)titleID, index,
                                                  shiftVal);
    ...
    case objects::EventCondition::Type_t::QUESTS_ACTIVE:
      if (!client) {
        ...
        for (auto qPair : questMap) {
          auto quest = definitionManager->GetQuestData((uint32_t)qPair.first);
          if (quest &&
        ...
        return Compare(count, condition->GetValue1(), condition->GetValue2(),
                       compareMode, EventCompareMode::EQUAL,
                       EVENT_COMPARE_NUMERIC2);
      }
    ...
    default:
      LogEventManagerError([&]() {
        return libcomp::String(
                   "Invalid condition type supplied for EvaluateCondition: "
                   "%1\n")
...
bool EventManager::EvaluateQuestPhaseRequirements(
    const std::shared_ptr<ChannelClientConnection>& client, int16_t questID,
    int8_t phase) {
  auto server = mServer.lock();
  auto definitionManager = server->GetDefinitionManager();
  ...
  for (uint32_t i = 0; i < phaseData->GetRequirementCount(); i++) {
    ...
    switch (req->GetType()) {
      case objects::QuestPhaseRequirement::Type_t::ITEM: {
        uint32_t count = server->GetCharacterManager()->GetExistingItemCount(
            character, req->GetObjectID());
      ...
      default:
        LogEventManagerError([&]() {
          return libcomp::String(
                     "Invalid requirement type encountered for "
                     "EvaluateQuestPhaseRequirements in quest '%1' phase '%2': "
                     "%3\n")
              .Arg(questID)
              .Arg(currentPhase)
...
void EventManager::SendActiveQuestList(
  ...
  for (auto kv : questMap) {
    ...
    reply.WriteArray(&customData,
                     (uint32_t)customData.size() * sizeof(int32_t));
  }
...
void EventManager::SendCompletedQuestList(
  ...
  reply.WritePacketCode(
      ChannelToClientPacketCode_t::PACKET_QUEST_COMPLETED_LIST);
...
std::shared_ptr<objects::DemonQuest> EventManager::GenerateDemonQuest(
  ...
  uint16_t enabledTypeFlags =
      server->GetWorldSharedConfig()->GetEnabledDemonQuests();
  ...
  uint8_t ssRank =
      cState->GetExpertiseRank(EXPERTISE_CHAIN_SWORDSMITH, definitionManager);
  uint8_t amRank =
      cState->GetExpertiseRank(EXPERTISE_CHAIN_ARMS_MAKER, definitionManager);
    ...
    if (demonTraits.find((uint32_t)pair.first) != demonTraits.end()) {
      switch (pair.second[0]) {
        case 1:
          ...
          if (enabledTypes.find(
                  (uint16_t)objects::DemonQuest::Type_t::CRYSTALLIZE) !=
              enabledTypes.end()) {
            validTypes.insert(
                (uint16_t)objects::DemonQuest::Type_t::CRYSTALLIZE);
          }
          ...
          if (enabledTypes.find(
                  (uint16_t)objects::DemonQuest::Type_t::ENCHANT_TAROT) !=
              enabledTypes.end()) {
            validTypes.insert(
                (uint16_t)objects::DemonQuest::Type_t::ENCHANT_TAROT);
          }
          ...
          if (enabledTypes.find(
                  (uint16_t)objects::DemonQuest::Type_t::ENCHANT_SOUL) !=
              enabledTypes.end()) {
            validTypes.insert(
                (uint16_t)objects::DemonQuest::Type_t::ENCHANT_SOUL);
          }
        ...
        case 2:
          ...
          if (enabledTypes.find(
                  (uint16_t)objects::DemonQuest::Type_t::SYNTH_MELEE) !=
                  enabledTypes.end() &&
              ssRank >= 10) {
            validTypes.insert(
                (uint16_t)objects::DemonQuest::Type_t::SYNTH_MELEE);
        ...
        case 3:
          ...
          if (enabledTypes.find(
                  (uint16_t)objects::DemonQuest::Type_t::SYNTH_GUN) !=
                  enabledTypes.end() &&
              amRank >= 10) {
            validTypes.insert((uint16_t)objects::DemonQuest::Type_t::SYNTH_GUN);
          }
          break;
        default:
          break;
  ...
  if (validTypes.find((uint16_t)objects::DemonQuest::Type_t::EQUIPMENT_MOD) !=
      validTypes.end()) {
    for (auto item : character->GetItemBoxes(0)->GetItems()) {
      auto itemData = !item.IsNull()
                          ? definitionManager->GetItemData(item->GetType())
                          : nullptr;
      if (itemData) {
    ...
    if (lvl < 30) {
      equipment.remove_if([](const std::shared_ptr<objects::Item>& item) {
        return item->GetModSlots(0) == 0;
      });
    }

    if (equipment.size() == 0) {
      validTypes.erase((uint16_t)objects::DemonQuest::Type_t::EQUIPMENT_MOD);
    }
  }
  ...
  if (validTypes.size() > 0) {
    ...
    LogEventManagerError([&]() {
      return libcomp::String(
                 "No valid demon quest could be generated for demon type '%1' "
                 "on character: %2\n")
          .Arg(demon->GetType())
  ...
  switch (dQuest->GetType()) {
    ...
    case objects::DemonQuest::Type_t::ENCHANT_SOUL: {
      bool isKill = dQuest->GetType() == objects::DemonQuest::Type_t::KILL;
      bool ignoreSelf =
          isKill || dQuest->GetType() == objects::DemonQuest::Type_t::CONTRACT;
      ...
      for (auto& pair : serverDataManager->GetFieldZoneIDs()) {
        auto zoneDef = serverDataManager->GetZoneData(pair.first, pair.second);
        if (!zoneDef) continue;

        std::unordered_map<uint32_t, std::shared_ptr<objects::Spawn>> spawns;
        ...
        for (auto& sgPair : zoneDef->GetSpawnGroups()) {
          ...
          for (auto& spawnPair : sg->GetSpawns()) {
            ...
            if (spawns.find(spawnID) != spawns.end() &&
                ...
                (!restriction ||
                 Zone::TimeRestrictionActive(worldClock, restriction))) {
        ...
        for (auto& spawnPair : spawns) {
          ...
          if (validSpawns.find(spawnPair.first) != validSpawns.end()) {
            fieldEnemyMap[spawn->GetLevel()].insert(spawn->GetEnemyType());
          }
        }
      }
      ...
      for (auto& pair : fieldEnemyMap) {
        if (abs(pair.first - lvlAdjust) <= 10) {
          for (uint32_t enemyType : pair.second) {
            ...
            if (!ignoreSelf ||
                definitionManager->GetDevilData(enemyType)
                        ->GetUnionData()
                        ->GetBaseDemonID() !=
    ...
    case objects::DemonQuest::Type_t::ENCHANT_SOUL: {
      ...
      for (uint32_t demonType : demons) {
        ...
        if (def) {
          ...
          auto enchantData = definitionManager->GetEnchantDataByDemonID(baseID);
          if (enchantData) {
  ...
  switch (dQuest->GetType()) {
    case objects::DemonQuest::Type_t::KILL:
      // Kill a randomly chosen field demon
      {
        ...
        if (left > 3 && RNG(int32_t, 1, lvlAdjust + 2) != 1) {
          while (left) {
            uint16_t count = RNG(uint16_t, counts.size() > 0 ? 1 : 2, left);
        ...
        for (uint16_t count : counts) {
          uint32_t enemyType = libcomp::Randomizer::GetEntry(demons);
          if (enemyType) {
    ...
    case objects::DemonQuest::Type_t::CRYSTALLIZE:
      // Random crystal from a specific demon
      {
        ...
        if (enchantData) {
          dQuest->SetTargets(enchantData->GetDevilCrystal()->GetItemID(), 1);
        }
      }
      break;
    case objects::DemonQuest::Type_t::ENCHANT_TAROT:
    ...
    case objects::DemonQuest::Type_t::SYNTH_GUN:
      ...
      {
        bool isSS =
            dQuest->GetType() == objects::DemonQuest::Type_t::SYNTH_MELEE;
    ...
    case objects::DemonQuest::Type_t::PLASMA:
      // Random color, count between 10 and 30
      {
        ...
        dQuest->SetTargets(
            (uint32_t)floor((float)RNG(uint32_t, min, max) / 10.f),
            RNG(int32_t, 10, 30));
      }
      break;
    default:
...
void EventManager::AddDemonQuestRewards(
  ...
  uint32_t nextRaceSeq =
      (uint32_t)(progress->GetDemonQuestsCompleted(raceID) + 1);

  std::unordered_map<uint32_t,
                     std::list<std::shared_ptr<objects::DemonQuestReward>>>
      rewardGroups;
  for (auto& pair : serverDataManager->GetDemonQuestRewardData()) {
    ...
    if (reward->GetSequenceStart()) {
      ...
      if (seq < start || (end && seq >= end) || (!repeat && seq != start) ||
  ...
  for (auto& pair : rewardGroups) {
    ...
    // Sort by ID
    rewards.sort([](const std::shared_ptr<objects::DemonQuestReward>& a,
    ...
    for (auto& reward : rewards) {
      ...
      for (uint32_t dropSetID : reward->GetNormalDropSets()) {
        ...
        for (auto drop :
             characterManager->DetermineDrops(dropSet->GetDrops(), 0)) {
          dQuest->SetRewardItems(
              drop->GetItemType(),
              RNG(uint16_t, drop->GetMinStack(), drop->GetMaxStack()));
        }
      ...
      for (uint16_t title : reward->GetBonusTitles()) {
        size_t index;
        uint8_t shiftVal;
        CharacterManager::ConvertIDToMaskValues(title, index, shiftVal);
  ...
  for (auto iter = SVR_CONST.DEMON_QUEST_XP.begin();
    ...
    if (nextSeq == 5 && idx == 0) {
      ...
      bool onFinal = (size_t)(idx + 1) == SVR_CONST.DEMON_QUEST_XP.size();
...
bool EventManager::UpdateDemonQuestCount(
  ...
  if (dQuest && dQuest->GetType() == questType &&
    ...
    for (auto& targetPair : dQuest->GetTargets()) {
      ...
      int32_t newCount = 0;
      int32_t currentCount = dQuest->GetTargetCurrentCounts(targetPair.first);
      if (itemMode) {
        // Ignore increment, set to current
        newCount = (int32_t)server->GetCharacterManager()->GetExistingItemCount(
            character, targetPair.first);
      } else {
      ...
      if (newCount != currentCount) {
        ...
        libcomp::Packet p;
        p.WritePacketCode(
            ChannelToClientPacketCode_t::PACKET_DEMON_QUEST_COUNT_UPDATE);
...
int8_t EventManager::EndDemonQuest(
  ...
  if (failCode) {
    ...
    if (dQuest->GetType() == objects::DemonQuest::Type_t::ITEM) {
      ...
      if (!server->GetCharacterManager()->AddRemoveItems(client, removeItems,
                                                         false)) {
        return -1;
      }
    }
    ...
    character->SetDemonQuest(NULLUUID);
    progress->SetDemonQuestSequence(
        (uint16_t)(progress->GetDemonQuestSequence() + 1));
    demon->SetHasQuest(false);
  ...
  if (!dQuest->GetUUID().IsNull() && failCode != 3) {
    ...
    notify.WritePacketCode(ChannelToClientPacketCode_t::PACKET_DEMON_QUEST_END);
  ...
  effects[SVR_CONST.STATUS_DEMON_QUEST_ACTIVE] =
      StatusEffectChange(SVR_CONST.STATUS_DEMON_QUEST_ACTIVE, 0, true);
...
void EventManager::StartWebGame(
    const std::shared_ptr<ChannelClientConnection>& client,
  ...
  if (gameSession) {
    if (!gameSession->GetSessionID().IsEmpty()) {
      LogEventManagerError([&]() {
        return libcomp::String(
                   "Second web-game session requested"
                   " for account: %1")
            .Arg(state->GetAccountUID().ToString());
      });
    ...
    auto e = current ? std::dynamic_pointer_cast<objects::EventOpenMenu>(
                           current->GetEvent())
                     : nullptr;
...
bool EventManager::HandleEvent(EventContext& ctx) {
  ...
  if (conditions.size() > 0 && !EvaluateEventConditions(ctx, conditions)) {
    ...
    switch (eventType) {
      ...
      default:
        LogEventManagerError([&]() {
          return libcomp::String("Failed to handle event of type %1\n")
              .Arg(to_underlying(eventType));
        });
...
void EventManager::SetEventStatus(
    const std::shared_ptr<ChannelClientConnection>& client) {
...
void EventManager::HandleNext(EventContext& ctx) {
  ...
  if (skipInvalid) {
    ...
    branches.remove_if(
        [serverDataManager](const std::shared_ptr<objects::EventBase>& b) {
          // Leave empty next event branches
          return !b->GetNext().IsEmpty() &&
                 serverDataManager->GetEventData(b->GetNext()) == nullptr;
        });
  }
  ...
  if (event->GetEventType() == objects::Event::EventType_t::OPEN_MENU &&
    ...
    if (!branchScriptID.IsEmpty()) {
      ...
      if (ctx.Client && !ctx.CurrentZone) {
        LogEventManagerError([&]() {
          return libcomp::String(
                     "Attempted to execute a client targeted branch script ID "
                     "outside of a zone: %1\n")
              .Arg(branchScriptID);
        });
        ...
        if (skipInvalid) {
          LogEventManagerError([&]() {
            return libcomp::String(
                       "Ignoring skip invalid setting on branch script event: "
                       "%1\n")
                .Arg(event->GetID());
          });
        ...
        LogEventManagerError([&]() {
          return libcomp::String("Invalid event branch script ID: %1\n")
              .Arg(branchScriptID);
        });
  ...
  if (!queueEventID.IsEmpty() && eState && !ctx.AutoOnly) {
    auto queue =
        PrepareEvent(queueEventID, ctx.EventInstance->GetSourceEntityID());
    if (queue) {
...
bool EventManager::Prompt(EventContext& ctx) {
  ...
  p.WriteS32Little(
      ctx.EventInstance->GetSourceEntityID() == 0
          ? ctx.Client->GetClientState()->GetCharacterState()->GetEntityID()
          : ctx.EventInstance->GetSourceEntityID());
  p.WriteS32Little(e->GetMessageID());
    ...
    if (!skip && (e->GetSkipInvalid() || choice->GetSkipInvalid())) {
      ...
      if (!choice->GetBranchScriptID().IsEmpty()) {
        ...
        for (auto branch : choice->GetBranches()) {
          ...
          valid |=
              branch->GetNext().IsEmpty() ||
              serverDataManager->GetEventData(branch->GetNext()) != nullptr;
        }
      }
...
bool EventManager::OpenMenu(EventContext& ctx) {
  ...
  p.WriteString16Little(state->GetClientStringEncoding(), sessionID, true);
  ...
  if (menuType == (int32_t)SVR_CONST.MENU_BAZAAR) {
    ...
    if (bState && bState->GetEntityID() == bazaarEntityID && zone) {
      ...
      if (market && market->GetAccount().GetUUID() == state->GetAccountUID()) {
        market->SetState(objects::BazaarData::State_t::BAZAAR_PREPARING);
        mServer.lock()->GetZoneManager()->SendBazaarMarketData(
            zone, bState, (uint32_t)market->GetMarketID());
      }
    }
  } else if (menuType == (int32_t)SVR_CONST.MENU_UB_RANKING) {
    // Send UB rankings for the menu
    mServer.lock()->GetMatchManager()->SendUltimateBattleRankings(ctx.Client);
  }
...
bool EventManager::ITime(EventContext& ctx) {
  ...
  if (!startActionsID.IsEmpty()) {
    auto saInst =
        PrepareEvent(startActionsID, ctx.EventInstance->GetSourceEntityID());
    if (saInst) {
      EventContext ctx2;
    ...
    for (size_t i = 0; i < 5; i++) {
      ...
      if (choice) {
        ...
        if (choice->GetMessageID() == 0 ||
            (conditions.size() > 0 &&
             !EvaluateEventConditions(ctx, conditions))) {
          ctx.EventInstance->InsertDisabledChoices((uint8_t)i);
          choice = nullptr;
        }
      }
...
bool EventManager::HandleTriFusion(
    const std::shared_ptr<ChannelClientConnection>& client) {
  auto state = client->GetClientState();
  ...
  auto partyClients =
      mServer.lock()->GetManagerConnection()->GetPartyConnections(client, true,
                                                                  true);
  ...
  if (tfSessionOwner) {
    ...
    tfSession->SetSourceEntityID(state->GetCharacterState()->GetEntityID());
...
bool EventManager::HandleWebGame(
    const std::shared_ptr<ChannelClientConnection>& client) {
  auto state = client->GetClientState();
  auto eState = state->GetEventState();
  auto current = eState ? eState->GetCurrent() : nullptr;
  ...
  if (!gameSession) {
    ...
    auto progress =
        character->GetProgress().Get(server->GetWorldDatabase(), true);
...
bool EventManager::PrepareTransformScript(
  ...
  auto script =
      e ? serverDataManager->GetScript(e->GetTransformScriptID()) : nullptr;
  ...
  auto scriptResult =
      !f.IsNull()
          ? f.Evaluate<int32_t>(
                ...
                state ? state->GetDemonState() : nullptr, zone, sqParams)
          : 0;
