...
if [ ! -f external-0.1.1-${PLATFORM}.zip ]; then
    wget -q https://github.com/comphack/external/releases/download/external-25/external-0.1.1-${PLATFORM}.zip
fi

# For a re-download of the libcomp builds
rm -f libcomp-*-${PLATFORM}.tar.bz2
wget -q https://github.com/comphack/libcomp/releases/download/v4.1.2/libcomp-4.1.2-${PLATFORM}.zip
