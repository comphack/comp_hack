...
PacketListModel::PacketListModel(QObject *p)
    : QAbstractListModel(p), mPacketLimit(0) {
  if (mPacketInfo.isEmpty()) loadPacketInfo();
...
QVariant PacketListModel::data(const QModelIndex &idx, int role) const {
  ...
  switch (role) {
    ...
    case Qt::DecorationRole: {
      if (d->client >= 0 && d->client < 6) return mIcons.at(d->client);
...
PacketData *PacketListModel::packetBefore(int idx) const {
  ...
  if (idx < 0 || idx >= mPacketData.count()) return 0;
  ...
  for (int i = idx; i > 0; i--) {
    ...
    if (prev && prev->cmd == first->cmd) return prev;
  }
...
void PacketListModel::addPacketData(const QList<PacketData *> &packetData) {
  ...
  if (mPacketLimit && newSize > mPacketLimit) {
    ...
    while (removeCount-- > 0) delete mPacketData.takeFirst();
...
QModelIndex PacketListModel::modelIndex(int idx) const {
  if (idx < 0 || idx >= mPacketData.count()) return QModelIndex();
...
void PacketListModel::loadPacketInfo() {
  ...
  if (!infoDOM.setContent(&infoFile)) return;
  ...
  for (int i = 0; i < infoList.count(); i++) {
    ...
    if (mPacketInfo.contains(code)) continue;
    ...
    info->name =
        QString("%1 (0x%2)")
            .arg(name)
            .arg(QString("%1").arg(code, 4, 16, QLatin1Char('0')).toUpper());
...
const PacketInfo *PacketListModel::getPacketInfo(uint16_t code) {
  if (!mPacketInfo.contains(code)) return 0;

  return mPacketInfo.value(code, 0);
}

int32_t PacketListModel::packetLimit() const { return mPacketLimit; }
