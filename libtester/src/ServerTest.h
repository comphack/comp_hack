...
#define UPHOLD_EQ(a, b)                          \
  {                                              \
    bool _didUpholdCondition = false;            \
    ([&](bool& _didUpholdConditionRef) -> void { \
      ASSERT_EQ(a, b);                           \
      _didUpholdConditionRef = true;             \
    })(_didUpholdCondition);                     \
    if (!_didUpholdCondition) {                  \
      throw libtester::TestFailure();            \
    }                                            \
  }

#define UPHOLD_GT(a, b)                          \
  {                                              \
    bool _didUpholdCondition = false;            \
    ([&](bool& _didUpholdConditionRef) -> void { \
      ASSERT_GT(a, b);                           \
...
class ServerTestConfig {
 public:
  ServerTestConfig(const std::chrono::milliseconds& testTime,
                   ...
                   const std::string& programsPath, bool debug = false);
...
static inline void EXPECT_COMPLETE(
    const std::chrono::duration<Rep, Period>& dur, Function&& f,
    Args&&... args) {
  std::promise<bool> promisedFinished;
  auto futureResult = promisedFinished.get_future();

  std::shared_ptr<libtester::TimedTask> task(
      new libtester::TimedTaskImpl<Args...>(std::forward<Function>(f),
...
template <class Rep, class Period, typename Function, typename... Args>
static inline void EXPECT_TIMEOUT(const std::chrono::duration<Rep, Period>& dur,
...
static inline void EXPECT_SERVER(const libtester::ServerTestConfig& config,
  ...
  std::thread(
      [](std::promise<bool>& finished,
         ...
         const std::shared_ptr<libtester::TimedTask>& _task, bool _debug) {
      ...
      config.GetProgramsPath(), task, config.GetDebug())
      .detach();
...
static inline ServerTestConfig LobbyOnly() {
  return ServerTestConfig(std::chrono::milliseconds(60000),  // 60 seconds
                          std::chrono::milliseconds(20000),  // 20 seconds
                          "bin/testing/programs-lobby.xml", false /* debug */);
}

static inline ServerTestConfig SingleChannel() {
  return ServerTestConfig(std::chrono::milliseconds(3 * 60000),  // 3 minutes
                          std::chrono::milliseconds(60000),      // 60 seconds
                          "bin/testing/programs.xml", false /* debug */);
}
