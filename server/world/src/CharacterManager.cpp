...
bool CharacterManager::UnregisterCharacter(
    std::shared_ptr<objects::CharacterLogin> cLogin) {
  bool removed = false;
  if (cLogin) {
    std::lock_guard<std::mutex> lock(mLock);
...
std::shared_ptr<objects::CharacterLogin> CharacterManager::GetCharacterLogin(
  ...
  auto character =
      objects::Character::LoadCharacterByName(worldDB, characterName);
...
bool CharacterManager::RequestChannelDisconnect(int32_t worldCID) {
  ...
  if (cLogin && cLogin->GetChannelID() >= 0) {
    auto channel =
        mServer.lock()->GetChannelConnectionByID(cLogin->GetChannelID());
    if (channel) {
      libcomp::Packet p;
...
bool CharacterManager::SendToCharacters(
    libcomp::Packet& p,
...
void CharacterManager::ConvertToTargetCIDPacket(libcomp::Packet& p,
                                                uint32_t cidOffset,
                                                size_t cidCount) {
...
bool CharacterManager::SendToRelatedCharacters(
    libcomp::Packet& p, int32_t worldCID, uint32_t cidOffset,
    uint8_t relatedTypes, bool includeSelf, bool zoneRestrict) {
  auto server = mServer.lock();
  auto cLogin = GetCharacterLogin(worldCID);
  if (!cLogin) {
  ...
  if (zoneRestrict) {
    cLogins.remove_if(
        [cLogin](const std::shared_ptr<objects::CharacterLogin>& l) {
...
CharacterManager::GetRelatedCharacterLogins(
  ...
  if (relatedTypes & RELATED_FRIENDS) {
    ...
    if (character &&
        cLogin->GetStatus() != objects::CharacterLogin::Status_t::OFFLINE) {
...
void CharacterManager::SendStatusToRelatedCharacters(
    const std::list<std::shared_ptr<objects::CharacterLogin>>& cLogins,
    uint8_t updateFlags, bool zoneRestrict) {
  for (auto cLogin : cLogins) {
    ...
    if (GetStatusPacket(reply, cLogin, outFlags)) {
      bool clanUpdate =
          0 != (outFlags & (uint8_t)CharacterLoginStateFlag_t::CHARLOGIN_BASIC);
      bool friendUpdate =
          0 != (outFlags &
      ...
      uint8_t relatedTypes = (uint8_t)((clanUpdate ? RELATED_CLAN : 0) |
                                       (friendUpdate ? RELATED_FRIENDS : 0) |
                                       (partyUpdate ? RELATED_PARTY : 0));

      // If all that is being sent is zone visible stats, restrict to the same
      // zone If the zone is contained in the change, relay it to the player
      bool partyStatsOnly =
          zoneRestrict &&
          (0 ==
           (outFlags &
            (uint8_t)(
                ...
                (uint8_t)
                    CharacterLoginStateFlag_t::CHARLOGIN_PARTY_DEMON_INFO)));
      bool containsZone =
          0 != (outFlags & (uint8_t)CharacterLoginStateFlag_t::CHARLOGIN_ZONE);
      SendToRelatedCharacters(reply, cLogin->GetWorldCID(), 1, relatedTypes,
                              containsZone, partyStatsOnly);
    }
  }
}

bool CharacterManager::GetStatusPacket(
    libcomp::Packet& p, const std::shared_ptr<objects::CharacterLogin>& cLogin,
    uint8_t& updateFlags) {
...
bool CharacterManager::AddToParty(
...
bool CharacterManager::PartyJoin(
    std::shared_ptr<objects::PartyCharacter> member,
    ...
    if (targetLogin && targetLogin->GetChannelID() >= 0) {
      ...
      if (targetMember) {
        ...
        if (partyID == 0) {
          ...
          newParty = true;
        } else if (GetCharacterLogin(targetMember->GetWorldCID())
                       ->GetPartyID() != partyID) {
  ...
  if (responseCode == (uint16_t)PartyErrorCodes_t::SUCCESS) {
    SendPartyMember(member, partyID, newParty, false, sourceConnection);
  }
...
bool CharacterManager::PartyRecruit(
    ...
    if (targetLogin && targetLogin->GetChannelID() >= 0) {
      ...
      if (targetMember) {
        ...
        if (partyID == 0) {
          ...
          newParty = true;
        } else if (GetCharacterLogin(member->GetWorldCID())->GetPartyID() !=
                   partyID) {
  ...
  if (responseCode == (uint16_t)PartyErrorCodes_t::SUCCESS) {
    SendPartyMember(targetMember, partyID, newParty, false, sourceConnection);
  }
...
void CharacterManager::PartyLeave(
  ...
  if (responseCode == (uint16_t)PartyErrorCodes_t::SUCCESS) {
    ...
    if (memberIDs.size() <= 1) {
      ...
      PartyLeaderUpdate(party->GetID(), cLogin->GetWorldCID(), nullptr,
                        *memberIDs.begin());
    }
  }
...
void CharacterManager::PartyDisband(
    uint32_t partyID, int32_t sourceCID,
    std::shared_ptr<libcomp::TcpConnection> requestConnection) {
  auto party = GetParty(partyID);
...
void CharacterManager::PartyLeaderUpdate(
    uint32_t partyID, int32_t sourceCID,
    std::shared_ptr<libcomp::TcpConnection> requestConnection,
    int32_t targetCID) {
  auto party = GetParty(partyID);
  ...
  if (requestConnection) {
    ...
    WorldServer::GetRelayPacket(relay, sourceCID);
    relay.WritePacketCode(
        ChannelToClientPacketCode_t::PACKET_PARTY_LEADER_UPDATE);
    relay.WriteU16Little(responseCode);
  ...
  if (responseCode == (uint16_t)PartyErrorCodes_t::SUCCESS) {
    ...
    request.WritePacketCode(InternalPacketCode_t::PACKET_PARTY_UPDATE);
    request.WriteU8(
        (uint8_t)InternalPacketAction_t::PACKET_ACTION_GROUP_LEADER_UPDATE);
    request.WriteU8(0);  // Not a response
...
void CharacterManager::PartyKick(
    std::shared_ptr<objects::CharacterLogin> cLogin, int32_t targetCID) {
...
void CharacterManager::SendPartyInfo(uint32_t partyID,
...
std::shared_ptr<objects::ClanInfo> CharacterManager::GetClan(
    const libobjgen::UUID& uuid) {
...
bool CharacterManager::ClanJoin(std::shared_ptr<objects::CharacterLogin> cLogin,
  ...
  auto character =
      libcomp::PersistentObject::LoadObjectByUUID<objects::Character>(
          db, cLogin->GetCharacter().GetUUID(), true);
  ...
  SendToRelatedCharacters(relay, cLogin->GetWorldCID(), cidOffset, RELATED_CLAN,
                          true);

  SendClanMemberInfo(cLogin, 0x30);
  RecalculateClanLevel(clanID);
  SendClanMemberInfo(cLogin,
                     (uint8_t)CharacterLoginStateFlag_t::CHARLOGIN_BASIC);

  return true;
}

bool CharacterManager::ClanLeave(
    std::shared_ptr<objects::CharacterLogin> cLogin, int32_t clanID,
    std::shared_ptr<libcomp::TcpConnection> requestConnection) {
  ...
  if (RemoveFromClan(cLogin, clanID)) {
    ...
    if (member &&
        member->GetMemberType() == objects::ClanMember::MemberType_t::MASTER) {
      ...
      if (newMaster) {
        ...
        relay.WritePacketCode(
            ChannelToClientPacketCode_t::PACKET_CLAN_MASTER_UPDATED);
        relay.WriteS32Little(clanID);
...
void CharacterManager::ClanDisband(
    int32_t clanID, int32_t sourceCID,
    std::shared_ptr<libcomp::TcpConnection> requestConnection) {
  auto clanInfo = GetClan(clanID);
  ...
  if (responseCode == 0) {
    ...
    for (auto member : clan->GetMembers()) {
      auto character =
          libcomp::PersistentObject::LoadObjectByUUID<objects::Character>(
              worldDB, member->GetCharacter(), true);
      character->SetClan(NULLUUID);
      dbChanges->Update(character);
...
void CharacterManager::ClanKick(
    std::shared_ptr<objects::CharacterLogin> cLogin, int32_t clanID,
    int32_t targetCID,
    std::shared_ptr<libcomp::TcpConnection> requestConnection) {
  if (requestConnection) {
  ...
  if (clanInfo) {
    ...
    for (auto memberRef : clan->GetMembers()) {
      ...
      auto character =
          member
              ? libcomp::PersistentObject::LoadObjectByUUID<objects::Character>(
                    db, member->GetCharacter())
              : nullptr;

      if (!character) {
        LogCharacterManagerWarning([&]() {
          return libcomp::String(
                     "Invalid clan member encountered on clan '%1' with UID: "
                     "%2\n")
              .Arg(clan->GetName())
      ...
      totalPoints =
          (uint64_t)(totalPoints + (uint64_t)character->GetLoginPoints());
    }
...
void CharacterManager::SendClanDetails(
    std::shared_ptr<objects::CharacterLogin> cLogin,
    std::shared_ptr<libcomp::TcpConnection> requestConnection,
  ...
  if (memberIDs.size() > 0) {
    ...
    for (auto mPair : clanInfo->GetMemberMap()) {
      ...
      if (memberLogin) {
        ...
        relay.WriteU8(memberLogin->GetWorldCID() == cLogin->GetWorldCID() ? 1
                                                                          : 0);
        relay.WriteS8(memberLogin->GetChannelID());
        relay.WriteS32Little(
            memberLogin->GetZoneID() ? (int32_t)memberLogin->GetZoneID() : -1);
        relay.WriteS32Little(memberChar ? (int32_t)memberChar->GetLastLogin()
                                        : 0);
      } else {
        relay.WriteBlank(11);
      }
      ...
      relay.WriteS32Little(memberChar ? memberChar->GetLoginPoints() : 0);
    }
  } else {
    // Clan level info
...
void CharacterManager::SendClanMemberInfo(
    std::shared_ptr<objects::CharacterLogin> cLogin, uint8_t updateFlags) {
  auto clanInfo = GetClan(cLogin->GetClanID());
  auto clan = clanInfo->GetClan().Get();
  auto member = clanInfo->GetMemberMap(cLogin->GetWorldCID()).Get();
  ...
  if (member) {
    ...
    if (updateFlags & (uint8_t)CharacterLoginStateFlag_t::CHARLOGIN_ZONE) {
      relay.WriteS32Little(cLogin->GetZoneID() ? (int32_t)cLogin->GetZoneID()
                                               : -1);
    }
...
int32_t CharacterManager::AddToTeam(int32_t worldCID, int32_t teamID) {
  ...
  if (teamID) {
    ...
    std::lock_guard<std::mutex> lock(mLock);
    if (!team ||
        team->MemberIDsCount() >= GetTeamMaxSize((int8_t)team->GetCategory())) {
      // Cannot add to team
      return 0;
    }
...
bool CharacterManager::TeamJoin(
    int32_t worldCID, int32_t teamID,
    std::shared_ptr<libcomp::TcpConnection> sourceConnection) {
...
void CharacterManager::TeamLeaderUpdate(
    int32_t teamID, int32_t sourceCID,
    std::shared_ptr<libcomp::TcpConnection> requestConnection,
    int32_t targetCID) {
  auto team = GetTeam(teamID);
...
void CharacterManager::TeamKick(std::shared_ptr<objects::CharacterLogin> cLogin,
  ...
  libcomp::Packet relay;
  auto cidOffset = WorldServer::GetRelayPacket(relay);
  relay.WritePacketCode(ChannelToClientPacketCode_t::PACKET_TEAM_KICK);
  relay.WriteS32Little(teamID);
  relay.WriteS8(errorCode);
  relay.WriteS32Little(targetCID);
...
bool CharacterManager::TeamZiotiteUpdate(
    int32_t teamID, const std::shared_ptr<objects::CharacterLogin>& source,
    int32_t sZiotite, int8_t lZiotite) {
  auto team = GetTeam(teamID);
  if (!team || (source && source->GetTeamID() != teamID)) {
    LogCharacterManagerError([&]() {
      return libcomp::String(
                 "Ziotite could not be updated for invalid team from "
                 "character: %1\n")
          .Arg(source ? source->GetCharacter().GetUUID().ToString() : "NONE");
    });
  ...
  int8_t newLAmount = 0;
  {
    ...
    if ((sZiotite || lZiotite) && newSAmount == team->GetSmallZiotite() &&
  ...
  request.WriteU8((uint8_t)InternalPacketAction_t::PACKET_ACTION_TEAM_ZIOTITE);
...
void CharacterManager::SendTeamInfo(int32_t teamID,
                                    const std::list<int32_t>& cids) {
  libcomp::Packet request;
  request.WritePacketCode(InternalPacketCode_t::PACKET_TEAM_UPDATE);
  request.WriteU8((uint8_t)InternalPacketAction_t::PACKET_ACTION_UPDATE);
...
uint32_t CharacterManager::CreateParty(
    std::shared_ptr<objects::PartyCharacter> member) {
  auto cid = member->GetWorldCID();
  auto login = GetCharacterLogin(cid);
...
void CharacterManager::SendPartyMember(
  ...
  if (newParty) {
    if (!isRefresh) {
      ...
      SendToRelatedCharacters(request, member->GetWorldCID(), 1, RELATED_PARTY,
                              true);
    }
  } else {
  ...
  SendToRelatedCharacters(relay, member->GetWorldCID(), cidOffset,
                          newParty ? RELATED_PARTY : 0, true);
}

bool CharacterManager::RemoveFromParty(
    std::shared_ptr<objects::CharacterLogin> cLogin, uint32_t partyID) {
  bool success = false;
  {
    std::lock_guard<std::mutex> lock(mLock);
...
bool CharacterManager::RemoveFromClan(
    std::shared_ptr<objects::CharacterLogin> cLogin, int32_t clanID) {
  auto clanInfo = GetClan(clanID);
  std::lock_guard<std::mutex> lock(mLock);
  if (clanInfo && cLogin->GetClanID() == clanID) {
    ...
    if (clan) {
      for (auto mRef : clan->GetMembers()) {
        ...
        if (m) {
          ...
          LogCharacterManagerError([&]() {
            return libcomp::String(
                       "Invalid clan member %1 encountered on clan %1\n")
    ...
    if (character &&
        character->GetClan().GetUUID() == clanInfo->GetClan().GetUUID()) {
      character->SetClan(NULLUUID);
      dbChanges->Update(character);
    }
    ...
    if (worldDB->ProcessChangeSet(dbChanges)) {
      LogClanDebug([clan, character]() {
        return libcomp::String(
                   "Successfully removed member from clan '%1': %2\n")
            .Arg(clan->GetName())
            .Arg(character->GetUUID().ToString());
      });
...
bool CharacterManager::RemoveFromTeam(
