...
ActionManager::ActionManager(const std::weak_ptr<ChannelServer>& server)
  ...
  mActionHandlers[objects::Action::ActionType_t::SPAWN] = &ActionManager::Spawn;
  mActionHandlers[objects::Action::ActionType_t::CREATE_LOOT] =
      &ActionManager::CreateLoot;
  mActionHandlers[objects::Action::ActionType_t::DELAY] = &ActionManager::Delay;
...
void ActionManager::PerformActions(
  ...
  if (zone) {
    ...
    ctx.CurrentZone =
        mServer.lock()->GetZoneManager()->GetCurrentZone(ctx.Client);
  }

  if (!ctx.Client && ctx.CurrentZone && sourceEntityID) {
    // Add the client of the source if they are still in the zone
    auto sourceClient =
        mServer.lock()->GetManagerConnection()->GetEntityClient(sourceEntityID);
  ...
  for (auto action : actions) {
      ...
      if (action->GetSourceContext() !=
        ...
        LogActionManagerError([&]() {
          return libcomp::String(
                     "Non-source context encountered for action set that "
                     "resulted in a channel change: %1\n")
              .Arg(ctx.Client->GetClientState()->GetAccountUID().ToString());
        });
      }
    ...
    if (mActionHandlers.end() == it) {
      ...
      if (srcCtx == objects::Action::SourceContext_t::ENEMIES) {
        ...
        auto worldCIDs =
            GetActionContextCIDs(action, ctx, failure, preFiltered);
        if (!failure) {
          ...
          if (!preFiltered) {
            ...
            switch (action->GetLocation()) {
              case objects::Action::Location_t::INSTANCE:
                cStates.remove_if(
                    [ctxInst](const std::shared_ptr<CharacterState>& cState) {
                      auto z = cState->GetZone();
                      return z == nullptr || z->GetInstance() != ctxInst;
                    });
                break;
              case objects::Action::Location_t::ZONE:
                cStates.remove_if(
                    [ctxZone](const std::shared_ptr<CharacterState>& cState) {
          ...
          for (auto cState : cStates) {
            auto charClient = connectionManager->GetEntityClient(
        ...
        if (ctx.Client) {
          ...
          if (options.IncrementEventIndex) {
            ...
            if (current) {
              current->SetIndex((uint16_t)(current->GetIndex() + 1));
            }
          }
        }
      }
      ...
      if (failure && action->GetStopOnFailure()) {
        if (!action->GetOnFailureEvent().IsEmpty()) {
          mServer.lock()->GetEventManager()->HandleEvent(
              ctx.Client, action->GetOnFailureEvent(), ctx.SourceEntityID);
        } else {
          LogActionManagerDebug([&]() {
            return libcomp::String(
                       "Quitting mid-action execution following the result of "
                       "action type: %1.\n")
                .Arg(ActionManager::GetActionName(action->GetActionType()));
          });
        }
...
void ActionManager::SendStageEffect(
    const std::shared_ptr<ChannelClientConnection>& client, int32_t messageID,
    int8_t effectType, bool includeMessage, int32_t messageValue) {
  libcomp::Packet p;
...
std::set<int32_t> ActionManager::GetActionContextCIDs(
  ...
  switch (action->GetSourceContext()) {
    ...
    case objects::Action::SourceContext_t::TEAM: {
      auto sourceClient = ctx.Client ? ctx.Client
                                     : connectionManager->GetEntityClient(
    ...
    case objects::Action::SourceContext_t::MATCH_TEAM: {
      ...
      auto match = ctx.CurrentZone ? ctx.CurrentZone->GetMatch() : nullptr;
      if (match && match->MemberIDsContains(state->GetWorldCID())) {
        auto pvpMatch = std::dynamic_pointer_cast<objects::PvPMatch>(match);
        if (pvpMatch) {
          // Add PvP team participants
          for (auto& team :
               {pvpMatch->GetBlueMemberIDs(), pvpMatch->GetRedMemberIDs()}) {
            bool inTeam = false;
...
bool ActionManager::ZoneChange(ActionContext& ctx) {
    ...
    if (!spotID) {
      ...
      if (!zoneID) {
        LogActionManagerErrorMsg(
            "Attempted to move to the homepoint but no homepoint is set\n");
  ...
  if (spotID > 0) {
    ...
    if (zoneDef) {
      ...
      if (spotIter != spots.end()) {
        Point p = zoneManager->GetRandomSpotPoint(spotIter->second, zoneData);
        x = p.x;
        y = p.y;
      ...
      LogActionManagerError([&]() {
        return libcomp::String(
                   "Invalid zone requested for spot ID move %1 (%2), #3.\n")
            .Arg(zoneID)
            .Arg(dynamicMapID)
            .Arg(spotID);
      });
  ...
  if (!zoneManager->EnterZone(ctx.Client, zoneID, dynamicMapID, x, y, rotation,
                              true)) {
    LogActionManagerError([&]() {
      return libcomp::String("Failed to add client to zone %1 (%2).\n")
          .Arg(zoneID)
          .Arg(dynamicMapID);
    });
...
bool ActionManager::SetHomepoint(ActionContext& ctx) {
  ...
  if (zoneID == 0 || !zoneDef) {
    LogActionManagerErrorMsg(
        "Attempted to execute a set homepoint action with an invalid zone ID "
        "specified\n");
  ...
  float yCoord = 0.f;
  float rot = 0.f;
  if (!mServer.lock()->GetZoneManager()->GetSpotPosition(
          zoneDef->GetDynamicMapID(), spotID, xCoord, yCoord, rot)) {
    LogActionManagerErrorMsg(
        "Attempted to execute a set homepoint action with an invalid spot ID "
        "specified\n");
...
bool ActionManager::AddRemoveItems(ActionContext& ctx) {
  ...
  if (act->GetFromDropSet()) {
    ...
    for (auto& pair : adds) {
      ...
      if (dropSet) {
        ...
        auto drops = characterManager->DetermineDrops(dropSet->GetDrops(), 0,
                                                      pair.second != 0);
        ...
        for (auto l : loot) {
          if (dropItems.find(l->GetType()) != dropItems.end()) {
            dropItems[l->GetType()] =
                (uint32_t)(dropItems[l->GetType()] + l->GetCount());
          } else {
  ...
  switch (act->GetMode()) {
    ...
    case objects::ActionAddRemoveItems::Mode_t::TIME_TRIAL_REWARD: {
      ...
      if (timeTrialReward) {
        ...
        if (!progress || progress->GetTimeTrialID() <= 0) {
          LogActionManagerError([&]() {
            return libcomp::String(
                       "Attempted to grant time trial rewards when no complete "
                       "time trial exists: %1\n")
      ...
      if (timeTrialReward) {
        ...
        uint32_t rewardItem = adds.size() > 0 ? adds.begin()->first : 0;
        uint16_t rewardItemCount =
            adds.size() > 0 ? (uint16_t)adds.begin()->second : (uint16_t)0;
    ...
    case objects::ActionAddRemoveItems::Mode_t::MATERIAL_TANK: {
      ...
      for (auto& itemSet : {adds, removes}) {
        for (auto pair : itemSet) {
          ...
          auto categoryData =
              itemData ? itemData->GetCommon()->GetCategory() : nullptr;
          if (!categoryData ||
              categoryData->GetMainCategory() != ITEM_CATEGORY_ACTIVE ||
              categoryData->GetSubCategory() != ITEM_SUBCATEGORY_ELEMENT) {
            LogActionManagerError([&]() {
              return libcomp::String(
                         "Attempted to add or remove non-material item in the "
                         "material tank: %1\n")
                  .Arg(pair.first);
            });
      ...
      if (character) {
        ...
        for (auto pair : adds) {
          ...
          int32_t newStack =
              (int32_t)((it != materials.end() ? it->second : 0) + pair.second);
    ...
    case objects::ActionAddRemoveItems::Mode_t::POST:
      if (removes.size() > 0) {
        LogActionManagerErrorMsg(
            "Attempted to remove one or more items from a post which is not "
            "allowed.\n");
        ...
        for (auto pair : adds) {
          ...
          if (!product) {
            LogActionManagerError([&]() {
              return libcomp::String(
                         "Attempted to add an invalid product to a post: %1\n")
                  .Arg(pair.first);
            });
        ...
        for (auto pair : adds) {
          for (uint32_t i = 0; i < pair.second; i++) {
            ...
            auto postItem =
                libcomp::PersistentObject::New<objects::PostItem>(true);
      ...
      // player's box and put in inventory
      {
        ...
        for (auto& pair : removes) {
          ...
          if (pair.first == 0) {
            ...
            startingSlot =
                (size_t)((dBox->GetNextPosition() + boxSize - 1) % boxSize);
            removing = true;
          } else {
          ...
          for (uint32_t i = 0;
               i < pair.second && (i == 0 || slot != startingSlot); i++) {
            auto l = dBox->GetLoot(slot);
            if (l && !removing) {
              if (toInventory.find(l->GetType()) != toInventory.end()) {
                toInventory[l->GetType()] =
                    (uint32_t)(toInventory[l->GetType()] + l->GetCount());
              } else {
        ...
        if (toInventory.size() > 0) {
          ...
          characterManager->AddRemoveItems(ctx.Client, toInventory, true);
        }

        // Adds must succeed
        return server->GetZoneManager()->UpdateDestinyBox(instance, worldCID,
                                                          loot, removeSlots) ||
               loot.size() == 0;
      }
      break;
...
bool ActionManager::AddRemoveStatus(ActionContext& ctx) {
  ...
  for (auto pair : act->GetStatusStacks()) {
    effects[pair.first] =
        StatusEffectChange(pair.first, pair.second, act->GetIsReplace());
  ...
  if (effects.size() > 0) {
    ...
    switch (act->GetTargetType()) {
      ...
      case objects::ActionAddRemoveStatus::TargetType_t::SOURCE: {
        ...
        playerEntities =
            eState && (eState->GetEntityType() == EntityType_t::CHARACTER ||
    ...
    for (auto eState : entities) {
      if (act->GetAllowNull()) {
        auto statusNulls = tokuseiManager->GetAspectMap(
            eState, TokuseiAspectType::STATUS_NULL);

        // Copy the effects that are not nullified
        StatusEffectChanges activeEffects;
        for (auto& pair : effects) {
          auto statusDef = definitionManager->GetStatusData(pair.first);
          ...
          if (statusNulls.size() > 0) {
            if (statusNulls.find((int32_t)pair.first) != statusNulls.end()) {
            ...
            if (statusNulls.find(adjustCategory) != statusNulls.end()) {
          ...
          if (server->GetWorldSharedConfig()->GetNRAStatusNull()) {
            ...
            CorrectTbl nraType =
                (CorrectTbl)(affinity + (uint8_t)CorrectTbl::NRA_DEFAULT);
    ...
    if (playerEntities) {
      ...
      tokuseiManager->Recalculate(state->GetCharacterState(), true, entityIDs);

      for (auto eState : entities) {
        server->GetCharacterManager()->RecalculateStats(eState, ctx.Client);
      }
    }
  }
...
bool ActionManager::UpdateCOMP(ActionContext& ctx) {
  ...
  std::unordered_map<uint32_t, std::list<std::shared_ptr<objects::Demon>>>
      remove;
  if (act->RemoveDemonsCount() > 0) {
    if (act->RemoveDemonsKeyExists(0)) {
      ...
      if (d) {
        if (d->GetLocked()) {
          LogActionManagerErrorMsg(
              "Attempted to remove partner demon that is locked\n");
        ...
        LogActionManagerErrorMsg(
            "Attempted to remove partner demon but no demon was summoned for "
            "COMP removal request\n");
        ...
        if (act->RemoveDemonsKeyExists(type)) {
          if (act->GetRemoveDemons(type) == 0) {
            ...
            if (remove.find(0) != remove.end()) {
              LogActionManagerErrorMsg(
                  "Attempted to remove partner demon twice for COMP removal "
                  "request\n");
              ...
              LogActionManagerErrorMsg(
                  "Attempted to remove specific partner demon that was not "
                  "summoned for COMP removal request\n");

              return false;
            }
          } else if (act->GetRemoveDemons(type) >
                     (uint8_t)remove[type].size()) {
            remove[type].push_back(slot.Get());
          }
        }
      }
    ...
    for (auto pair : act->GetRemoveDemons()) {
      if ((pair.second == 0 && remove[pair.first].size() != 1) ||
          (pair.second != 0 &&
           (uint8_t)remove[pair.first].size() < pair.second)) {
        LogActionManagerErrorMsg(
            "One or more demons does not exist or is locked for COMP removal "
            "request\n");
  ...
  if (act->AddDemonsCount() > 0) {
    ...
    for (auto pair : act->GetAddDemons()) {
      ...
      if (freeCount < pair.second) {
        LogActionManagerErrorMsg(
            "Not enough slots free for COMP add request\n");
...
bool ActionManager::GrantXP(ActionContext& ctx) {
  ...
  if (act->GetTargetType() == objects::ActionGrantXP::TargetType_t::CHARACTER ||
      act->GetTargetType() ==
          objects::ActionGrantXP::TargetType_t::CHARACTER_AND_PARTNER) {
  ...
  for (auto eState : entityStates) {
    if (eState->Ready(true)) {
      ...
      characterManager->UpdateExperience(ctx.Client, xp, eState->GetEntityID());
    }
  }
...
bool ActionManager::GrantSkills(ActionContext& ctx) {
  ...
  switch (act->GetTargetType()) {
    case objects::ActionGrantSkills::TargetType_t::CHARACTER: {
      ...
      if (act->GetSkillPoints() > 0) {
        characterManager->UpdateSkillPoints(ctx.Client, act->GetSkillPoints());
      }
      ...
      if (act->GetExpertiseMax()) {
        ...
        if (val == -1) {
          ...
          newVal = (int16_t)(character->GetExpertiseExtension() + val);
        }
      ...
      if (act->ExpertisePointsCount() > 0) {
        ...
        for (auto pair : act->GetExpertisePoints()) {
          ...
          if (expSet) {
            ...
            auto exp = character->GetExpertises((size_t)pair.first).Get();
            if (exp) {
              points = points - exp->GetPoints();
            }
          }

          expPoints.push_back(std::pair<uint8_t, int32_t>(pair.first, points));
        }
    ...
    case objects::ActionGrantSkills::TargetType_t::PARTNER:
      ...
      if (act->GetSkillPoints() > 0) {
        LogActionManagerErrorMsg(
            "Attempted to grant skill points to a partner demon\n");
      ...
      if (act->ExpertisePointsCount() > 0) {
        LogActionManagerErrorMsg(
            "Attempted to grant expertise points to a partner demon\n");
      ...
      if (act->GetExpertiseMax()) {
        LogActionManagerErrorMsg(
            "Attempted to extend max expertise for a partner demon\n");
  ...
  if (eState->Ready(true)) {
    for (uint32_t skillID : act->GetSkillIDs()) {
      characterManager->LearnSkill(ctx.Client, eState->GetEntityID(), skillID);
    }
  }
...
bool ActionManager::SpecialDirection(ActionContext& ctx) {
  ...
  libcomp::Packet p;
  p.WritePacketCode(
      ChannelToClientPacketCode_t::PACKET_EVENT_SPECIAL_DIRECTION);
...
bool ActionManager::PlaySoundEffect(ActionContext& ctx) {
  ...
  if (act->GetWaitTime()) {
    ...
    state->SetSoundDelayTime((uint64_t)(
        startFrom +
...
bool ActionManager::SetNPCState(ActionContext& ctx) {
  ...
  if (!act) {
    ...
    LogActionManagerErrorMsg(
        "Source client NPC state change requested but no source client exists "
  ...
  if (!oNPCState) {
    LogActionManagerError([&]() {
      return libcomp::String(
                 "SetNPCState attempted on invalid target in zone %1: %2\n")
  ...
  switch (oNPCState->GetEntityType()) {
    ...
    case EntityType_t::OBJECT:
      oNPC =
          std::dynamic_pointer_cast<ServerObjectState>(oNPCState)->GetEntity();
      break;
    default:
      break;
  }

  if (oNPC && act->GetFrom() >= 0 &&
      oNPC->GetState() != (uint8_t)act->GetFrom()) {
    // Stop all actions past this point
    return false;
  }

  if (oNPC &&
      (act->GetSourceClientOnly() || act->GetState() != oNPC->GetState())) {
    ...
    if (npcState) {
      if (act->GetState() == HNPC_STATE_SHOW) {
        zoneManager->ShowNPC(ctx.CurrentZone, clients, npcState, false);
      } else {
        zoneManager->RemoveEntities(clients, {npcState->GetEntityID()});
      }
    } else {
      if (!act->GetSourceClientOnly()) {
        // Update collisions
      ...
      if (act->GetState() == ONPC_STATE_HIDE) {
        zoneManager->RemoveEntities(clients, {oNPCState->GetEntityID()});
      } else if (from == ONPC_STATE_HIDE) {
        auto objState = std::dynamic_pointer_cast<ServerObjectState>(oNPCState);
        zoneManager->ShowObject(ctx.CurrentZone, clients, objState, false);
      } else {
        libcomp::Packet p;
  ...
  switch (act->GetFlagType()) {
    ...
    case objects::ActionUpdateFlag::FlagType_t::TIME_TRIAL:
      if (act->GetRemove()) {
        LogActionManagerError([&]() {
          return libcomp::String(
                     "UpdateFlag attempted to remove a time trial record: %1\n")
              .Arg(act->GetID());
        });
    ...
    case objects::ActionUpdateFlag::FlagType_t::TITLE:
      if (act->GetRemove()) {
        LogActionManagerError([&]() {
          return libcomp::String("UpdateFlag attempted to remove a title: %1\n")
              .Arg(act->GetID());
        });
...
bool ActionManager::UpdateLNC(ActionContext& ctx) {
  ...
  auto character =
      ctx.Client->GetClientState()->GetCharacterState()->GetEntity();
  auto characterManager = mServer.lock()->GetCharacterManager();
  ...
  switch (act->GetPointType()) {
    case objects::ActionUpdatePoints::PointType_t::CP:
      if (act->GetIsSet() || act->GetValue() < 0) {
        LogActionManagerErrorMsg(
            "Attempts to explicitly set or decrease the player's CP are not "
            "allowed!\n");
    ...
    case objects::ActionUpdatePoints::PointType_t::SOUL_POINTS: {
      mServer.lock()->GetCharacterManager()->UpdateSoulPoints(
          ctx.Client, (int32_t)act->GetValue(), false, !act->GetIsSet());
    } break;
      ...
      if (act->GetModifier() > 0) {
        ...
        if (progress) {
          ...
          if (oldVal != val) {
            ...
            libcomp::Packet p;
            p.WritePacketCode(ChannelToClientPacketCode_t::PACKET_ITIME_UPDATE);
            p.WriteS8(iTimeID);
        ...
        LogActionManagerErrorMsg(
            "Invalid I-Time ID specified for UpdatePoints action\n");
    ...
    case objects::ActionUpdatePoints::PointType_t::PVP_POINTS: {
      ...
      if (MatchManager::InPvPTeam(cState)) {
        ...
        if (!matchManager->UpdatePvPPoints(
                instance->GetID(), cState->GetEntityID(), -1,
                (uint8_t)(factionGroup - 1), (int32_t)act->GetValue(), false)) {
          return false;
        }
      }
    } break;
    ...
    case objects::ActionUpdatePoints::PointType_t::BETHEL: {
      ...
      auto pEntry =
          server->GetMatchManager()->LoadPentalphaData(ctx.Client, 0x01);
      if (act->GetIsSet()) {
        ...
        return server->GetCharacterManager()->UpdateCowrieBethel(ctx.Client, 0,
                                                                 bethel);
      } else if (!pEntry) {
        // Everything past this point requires an active entry
        return false;
      } else {
        auto currentMatch = server->GetMatchManager()->GetPentalphaMatch(false);
        ...
        if (act->GetValue()) {
          ...
          newVals[pEntry->GetTeam()] = bethel + oldVals[pEntry->GetTeam()];
        } else if (!instance ||
          ...
          size_t otherIdx = (size_t)instance->GetVariant()->GetSubID();

          pEntry->SetBethel(oldBethel + bethel);
          newVals[pEntry->GetTeam()] = oldVals[pEntry->GetTeam()] + bethel;
          newVals[otherIdx] = oldVals[otherIdx] - bethel;
          state->SetInstanceBethel(0);

          // Get the final amount
          bethel = server->GetCharacterManager()->UpdateBethel(ctx.Client,
                                                               bethel, true);

          libcomp::Packet p;
          p.WritePacketCode(ChannelToClientPacketCode_t::PACKET_PENTALPHA_END);
    ...
    case objects::ActionUpdatePoints::PointType_t::ZIOTITE: {
      ...
      return mServer.lock()->GetMatchManager()->UpdateZiotite(
          team, sZiotite, lZiotite, state->GetWorldCID());
    } break;
    case objects::ActionUpdatePoints::PointType_t::REUNION_POINTS:
      if (act->GetIsSet() || act->GetValue() < 0) {
        LogActionManagerErrorMsg(
            "Attempts to explicitly set or decrease reunion points are not "
            "allowed!\n");
    ...
    case objects::ActionUpdatePoints::PointType_t::DURABILITY: {
      ...
      auto equip =
          character->GetEquippedItems((size_t)act->GetModifier()).Get();
      if (equip) {
        mServer.lock()->GetCharacterManager()->UpdateDurability(
            ctx.Client, equip, (int32_t)act->GetValue(), !act->GetIsSet());
      }
    } break;
...
bool ActionManager::UpdateQuest(ActionContext& ctx) {
  ...
  if (flagStates.size() > 0 &&
      act->GetFlagSetMode() !=
          objects::ActionUpdateQuest::FlagSetMode_t::UPDATE) {
    auto character =
        ctx.Client->GetClientState()->GetCharacterState()->GetEntity();
    auto quest = character->GetQuests(act->GetQuestID()).Get();
    auto existing =
        quest ? quest->GetFlagStates() : std::unordered_map<int32_t, int32_t>();
  ...
  return eventManager->UpdateQuest(ctx.Client, act->GetQuestID(),
                                   act->GetPhase(), act->GetForceUpdate(),
                                   flagStates);
}
  ...
  switch (act->GetType()) {
    ...
    case objects::ActionUpdateZoneFlags::Type_t::ZONE_INSTANCE_CHARACTER:
      if (ctx.Client) {
        ...
        LogActionManagerErrorMsg(
            "Attempted to update a zone character flag with no associated "
            "client!\n");
          ...
          case objects::ActionUpdateZoneFlags::SetMode_t::UPDATE:
            for (auto pair : act->GetFlagStates()) {
              if (pair.second == 0) {
                ...
                eState->SetAdditionalTokusei(pair.first, (uint16_t)pair.second);
              }
            }
            break;
          case objects::ActionUpdateZoneFlags::SetMode_t::INCREMENT:
          case objects::ActionUpdateZoneFlags::SetMode_t::DECREMENT: {
            ...
            for (auto pair : act->GetFlagStates()) {
              ...
              if (val <= 0) {
                ...
                eState->SetAdditionalTokusei(pair.first, (uint16_t)val);
              }
            }
          } break;
          default:
    ...
    for (int32_t triggerFlag : triggerFlags) {
      for (auto trigger : ctx.CurrentZone->GetFlagSetTriggers()) {
        if (trigger->GetValue() == triggerFlag) {
          PerformActions(ctx.Client, trigger->GetActions(), ctx.SourceEntityID,
                         ctx.CurrentZone);
        }
      }
    }
...
bool ActionManager::UpdateZoneInstance(ActionContext& ctx) {
  ...
  switch (act->GetMode()) {
    ...
    case objects::ActionZoneInstance::Mode_t::CLAN_JOIN:
      if (!ctx.Client) {
        LogActionManagerErrorMsg(
            "Attempted to create an instance with no client context\n");

        return false;
      } else {
        auto serverDataManager = server->GetServerDataManager();
        auto instDef =
            serverDataManager->GetZoneInstanceData(act->GetInstanceID());
        if (!instDef) {
          LogActionManagerError([&]() {
            return libcomp::String(
                       "Invalid zone instance ID could not be created: %1\n")
        ...
        switch (act->GetMode()) {
          ...
          case objects::ActionZoneInstance::Mode_t::CLAN_JOIN:
            // Grant access to the clan in the zone and join right away
            {
              auto character = state->GetCharacterState()->GetEntity();
              auto clanUID =
                  character ? character->GetClan().GetUUID() : NULLUUID;
              if (clanUID != NULLUUID) {
                for (auto& cPair : ctx.CurrentZone->GetConnections()) {
                  auto oState = cPair.second->GetClientState();
                  character = oState->GetCharacterState()->GetEntity();
                  if (character && character->GetClan().GetUUID() == clanUID) {
        ...
        if (moveNow) {
          ...
          for (int32_t cid : accessCIDs) {
            auto otherState = ClientState::GetEntityClientState(cid, true);
            if (cid != state->GetWorldCID() &&
                (!otherState || otherState->GetZone() != ctx.CurrentZone)) {
              remove.insert(cid);
            }
          }
        ...
        instAccess->SetCreateTimerExpirationEventID(
        ...
        if (resultCode && moveNow) {
          ...
          auto clients = managerConnection->GetEntityClients(accessCIDs, true);
          ...
          if (team) {
            for (auto client : managerConnection->GetEntityClients(
                     team->GetMemberIDs(), true)) {
              auto oState = client->GetClientState();
              if (accessCIDs.find(oState->GetWorldCID()) == accessCIDs.end()) {
    ...
    case objects::ActionZoneInstance::Mode_t::JOIN:
      if (!ctx.Client) {
        LogActionManagerErrorMsg(
            "Attempted to join an instance with no client context\n");

        return false;
      } else {
        auto instAccess = zoneManager->GetInstanceAccess(
            ctx.Client->GetClientState()->GetWorldCID());
        bool success =
            instAccess && zoneManager->MoveToInstance(ctx.Client, instAccess);
    ...
    case objects::ActionZoneInstance::Mode_t::START_TIMER: {
      ...
      if (!instance ||
          (act->GetInstanceID() && def->GetID() != act->GetInstanceID())) {
        return false;
      }
      ...
      if (timerID) {
        switch (ctx.CurrentZone->GetInstanceType()) {
          ...
          case InstanceType_t::DEMON_ONLY:
            LogActionManagerErrorMsg(
                "Attempted to start a non-default timer on an implicit timer "
                "instance type.\n");
        ...
        if (instance->GetTimerStart() && !instance->GetTimerStop() &&
          ...
          LogActionManagerError([&]() {
            return libcomp::String(
                       "Attempted to start an instance timer but the previous "
                       "timer could not be stopped first for instance %1\n")
                .Arg(instance->GetID());
          });
        ...
        auto definitionManager = server->GetDefinitionManager();
        auto timeLimitDef = definitionManager->GetTimeLimitData(timerID);
        if (timeLimitDef) {
          instance->SetTimerID(timerID);
          instance->SetTimeLimitData(timeLimitDef);
          instance->SetTimerExpirationEventID(act->GetTimerExpirationEventID());
          ...
          LogActionManagerError([&]() {
            return libcomp::String(
                       "Attempted to start an invalid instance timer: %1\n")
                .Arg(timerID);
          });
    ...
    case objects::ActionZoneInstance::Mode_t::STOP_TIMER: {
      ...
      if (timerID) {
        ...
        if (!timeLimitData || timeLimitData->GetID() != timerID) {
          LogActionManagerError([&]() {
            return libcomp::String(
                       "Attempted to stop an instance timer that did not match "
                       "the supplied timer ID: %1\n")
                .Arg(timerID);
          });
    ...
    case objects::ActionZoneInstance::Mode_t::TEAM_PVP:
      if (!ctx.Client) {
        LogActionManagerErrorMsg(
            "Attempted to start a team PvP match with no client context\n");

        return false;
      } else {
        auto matchManager = server->GetMatchManager();
        return matchManager->RequestTeamPvPMatch(
            ctx.Client, act->GetVariantID(), act->GetInstanceID());
      }
      break;
    default:
      break;
...
bool ActionManager::CreateLoot(ActionContext& ctx) {
  ...
  switch (act->GetPosition()) {
    ...
    case objects::ActionCreateLoot::Position_t::SOURCE_RELATIVE: {
      ...
      if (!source) {
        LogActionManagerErrorMsg(
            "Attempted to create source relative loot without a valid source "
            "entity\n");
  ...
  if (act->GetExpirationTime() > 0.f) {
    ...
    lootTime = (uint64_t)(
        now + (uint64_t)((double)act->GetExpirationTime() * 1000000.0));
  }
  ...
  for (auto loc : locations) {
    ...
    for (auto dropSet : characterManager->DetermineDropSets(
             act->GetDropSetIDs(), ctx.CurrentZone, ctx.Client)) {
    ...
    if (loc->GetSpotID()) {
      ...
      if (spotIter != zoneSpots.end()) {
        Point p = zoneManager->GetRandomSpotPoint(spotIter->second->Definition);
        x = p.x;
        y = p.y;
      } else {
        LogActionManagerError([zone, loc]() {
          return libcomp::String(
                     "Attempted to create loot in zone %1 at an invalid spot: "
                     "%2\n")
    ...
    if (firstClient) {
      zoneManager->SendLootBoxData(firstClient, lState, nullptr, true, true);
    }
  }
...
bool ActionManager::Delay(ActionContext& ctx) {
  ...
  if (act->GetType() == objects::ActionDelay::Type_t::ACTION_DELAY) {
    ...
    if (act->GetDuration()) {
      ...
      int32_t worldCID =
          ctx.Client ? ctx.Client->GetClientState()->GetWorldCID() : 0;
      ...
      server->ScheduleWork(
          ...
          [](const std::shared_ptr<ChannelServer> pServer,
             ...
             const std::shared_ptr<Zone> pZone, int32_t pSourceEntityID,
             int32_t pWorldCID, uint32_t pGroupID) {
            ...
            if (actionManager && pZone && !pZone->GetInvalid()) {
              ...
              auto client =
                  pWorldCID ? pZone->GetConnections()[pWorldCID] : nullptr;

              ActionOptions options;
              options.GroupID = pGroupID;

              actionManager->PerformActions(client, pAct->GetActions(),
                                            pSourceEntityID, pZone, options);
            }
          },
      ...
      case objects::ActionDelay::Type_t::CHARACTER_COOLDOWN:
        // Set the context character's ActionCooldowns value
        {
          ...
          if (act->GetDelayID() == COOLDOWN_INVOKE_LAW ||
            ...
            mServer.lock()->GetCharacterManager()->SendInvokeStatus(ctx.Client,
                                                                    true);
          }
        }
        break;
...
bool ActionManager::RunScript(ActionContext& ctx) {
  ...
  if (script && script->Type.ToLower() == "actioncustom") {
    ...
    auto scriptResult =
        !f.IsNull() ? f.Evaluate<int32_t>(
                          source, state ? state->GetCharacterState() : nullptr,
                          state ? state->GetDemonState() : nullptr, zone,
                          server, sqParams)
                    : 0;
...
bool ActionManager::RecordTimeTrial(ActionContext& ctx, uint32_t rewardItem,
  ...
  if (progress && progress->GetTimeTrialID()) {
    ...
    libcomp::Packet p;
    p.WritePacketCode(ChannelToClientPacketCode_t::PACKET_TIME_TRIAL_REPORT);
    ...
    LogActionManagerError([&]() {
      return libcomp::String(
                 "Attempted to update an active time trial record but one does "
                 "not exist: %1\n")
          .Arg(state->GetAccountUID().ToString());
    });
...
bool ActionManager::VerifyClient(ActionContext& ctx,
  ...
  if (!ctx.Client) {
    LogActionManagerError([&]() {
      return libcomp::String(
                 "Attempted to execute a %1 with no associated client "
                 "connection\n")
          .Arg(typeName);
    });
...
bool ActionManager::VerifyZone(ActionContext& ctx,
  ...
  if (!ctx.CurrentZone) {
    LogActionManagerError([&]() {
      return libcomp::String("Attempted to execute a %1 with no current zone\n")
          .Arg(typeName);
    });
...
bool ActionManager::PrepareTransformScript(
  ...
  auto act = ctx.Action;
  auto script =
      act ? serverDataManager->GetScript(act->GetTransformScriptID()) : nullptr;
