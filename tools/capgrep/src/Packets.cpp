...
void action0015(PacketData* d, libcomp::Packet& p,
  ...
  QString xml = QString(
                    "<oNPC id=\"%1\" uid=\"%2\" state=\"%3\" convo=\"0\" "
                    "action=\"0\"\n\tx=\"%4\" y=\"%5\" rot=\"%6\"/>")
                    .arg(id)
                    .arg(uid)
                    .arg(state)
...
void action0023(PacketData* d, libcomp::Packet& p,
  ...
  QString xml = QString(
                    ...
                    "global=\"0\" dropZone=\"0\"\n\t\tx=\"%4\" y=\"%5\" "
                    "rot=\"%6\" actionOnZoneIn=\"0\" actionOnZoneOut=\"0\"/>\n")
                    .arg(zone)
                    .arg(set)
                    .arg(uid)
                    .arg(x)
  ...
  for (int i = 0; i < model->rowCount(); i++) {
    ...
    if (d2->cmd < 0x0014 || d2->cmd > 0x0015) continue;

    libcomp::Packet p2;
    p2.WriteArray(d2->data.constData(), static_cast<uint32_t>(d2->data.size()));
    p2.Rewind();
    ...
    if (d2->cmd == 0x0014)  // hNPC
    {
      ...
      if (npcs.contains(uid2)) continue;

      uint32_t zone_set = p2.ReadU32Little();
      uint32_t zone_id = p2.ReadU32Little();

      if (zone_id != zone || zone_set != set) continue;
...
void action00B9(PacketData* d, libcomp::Packet& p,
  ...
  if (valid) valid &= numBytes == packetBefore.ReadU16Little();
