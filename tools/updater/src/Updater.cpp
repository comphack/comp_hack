...
Updater::Updater(QWidget *p) : QWidget(p), mDone(false) {
  ...
  setWindowTitle(tr("COMP_hack Updater v%1.%2.%3 (%4)")
                     .arg(VERSION_MAJOR)
                     .arg(VERSION_MINOR)
                     .arg(VERSION_PATCH)
  ...
  connect(mDL, SIGNAL(statusChanged(const QString &)), ui.statusLabel,
          SLOT(setText(const QString &)));
  connect(mDL, SIGNAL(totalFilesChanged(int)), ui.totalProgress,
          SLOT(setMaximum(int)));
  connect(mDL, SIGNAL(currentFileChanged(int)), ui.totalProgress,
          SLOT(setValue(int)));
  connect(mDL, SIGNAL(downloadSizeChanged(int)), ui.fileProgress,
          SLOT(setMaximum(int)));
  connect(mDL, SIGNAL(downloadProgressChanged(int)), ui.fileProgress,
          SLOT(setValue(int)));
  connect(mDL, SIGNAL(errorMessage(const QString &)), this,
          SLOT(errorMessage(const QString &)));

  connect(ui.settingsButton, SIGNAL(clicked(bool)), this, SLOT(showSettings()));
  connect(ui.screenshotsButton, SIGNAL(clicked(bool)), this,
          SLOT(showScreenshots()));
  connect(ui.diagButton, SIGNAL(clicked(bool)), this, SLOT(showDXDiag()));
  connect(ui.checkButton, SIGNAL(clicked(bool)), this, SLOT(recheck()));
  connect(ui.retryButton, SIGNAL(clicked(bool)), this, SLOT(retry()));
...
void Updater::unlock() {
  ...
  if (aprilFools.month() == 4 && aprilFools.day() == 1) {
    ...
    QMessageBox::information(
        this, "April Fools",
        "OK, no error actually occured, but do you remember when you used to "
        "get these damn errors? Pathetic they never figured it out, isn't it. "
        "Enjoy your april fools day! -COMP_hack Team");
  }
  ...
  if (versionMap.open(QIODevice::ReadOnly | QIODevice::Text)) {
    ...
    while (!versionMap.atEnd()) {
      ...
      if (versionLine.count() < 1 || versionLine.left(1) == "#") continue;
      if (versionLine.toLower() == "[versions]") break;

      QMessageBox::critical(
          this, tr("VersionData.txt Error"),
    ...
    while (!versionMap.atEnd()) {
      ...
      if (versionLine.count() < 1 || versionLine.left(1) == "#") continue;
      if (versionLine.toLower().left(1) == "[") break;
      ...
      if (!version.title.isEmpty() && !version.server.isEmpty() &&
        ...
        connect(action, SIGNAL(triggered(bool)), this, SLOT(startGame()));
      }
    }
    ...
    while (true) {
      ...
      if (line.count() > 1) tag = line.at(1).trimmed();

      if (d->files.contains(file)) {
        QMessageBox::critical(
            this, tr("VersionData.txt Error"),
            tr("Duplicate file '%1' found for tag '%2'").arg(file).arg(d->tag));
...
bool Updater::copyFile(const QString &src, const QString &dest) {
  ...
  if (out.write(in.readAll()) != QFileInfo(src).size()) return false;
...
void Updater::startGame() {
  ...
  if (action) {
    ...
    if (!mVersionMap.contains(tag)) return;
  ...
  while (it.hasNext()) {
    ...
    QString source =
        tr("%1/%2.%3").arg(qApp->applicationDirPath()).arg(file).arg(suffix);
    QString dest = tr("%1/%2").arg(qApp->applicationDirPath()).arg(file);
...
void Updater::showSettings() {
  ...
  QProcess::startDetached(
      "env WINEPREFIX=\"/home/erikku/.wine\" wine "
...
void Updater::recheck() {
  ...
  QString path =
      QString("%1/ImagineUpdate2.dat").arg(qApp->applicationDirPath());

  QFile(path).remove();

  path = QString("%1/ImagineUpdate2.ver").arg(qApp->applicationDirPath());
