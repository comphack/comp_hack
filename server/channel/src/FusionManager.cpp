...
FusionManager::FusionManager(const std::weak_ptr<ChannelServer>& server)
    : mServer(server) {}
...
bool FusionManager::HandleFusion(
    ...
    int64_t demonID2, uint32_t costItemType) {
  std::shared_ptr<objects::Demon> resultDemon;
  int8_t result =
      ProcessFusion(client, demonID1, demonID2, -1, costItemType, resultDemon);
...
bool FusionManager::HandleTriFusion(
    const std::shared_ptr<ChannelClientConnection>& client, int64_t demonID1,
    int64_t demonID2, int64_t demonID3, bool soloFusion) {
  ...
  for (auto demon : {demon1, demon2, demon3}) {
    if (demon) {
      ...
      auto dClient =
          account
              ? managerConnection->GetClientConnection(account->GetUsername())
              : nullptr;
      if (dClient) {
  ...
  if (soloFusion) {
    ...
    if (result == 0) {
      ...
      std::unordered_map<
          std::shared_ptr<ChannelClientConnection>,
          std::unordered_map<int8_t, std::shared_ptr<objects::Item>>>
          newItemMap;
      ...
      for (auto dPair : dClientMap) {
        ...
        auto dEnchantData =
            definitionManager->GetEnchantDataByDemonID(dPair.first->GetType());
        if (freeSlots[pClient].size() > 0 && dEnchantData &&
            RNG(int16_t, 1, 10) == 1) {
          uint32_t crystalItem = dEnchantData->GetDevilCrystal()->GetItemID();
          if (crystalItem) {
      ...
      if (newItemMap.size() > 0) {
        for (auto clientPair : freeSlots) {
          ...
          if (newItemMap[pClient].size() > 0) {
            auto pCharacter =
                pClient->GetClientState()->GetCharacterState()->GetEntity();
...
uint32_t FusionManager::GetResultDemon(
    const std::shared_ptr<ChannelClientConnection>& client, int64_t demonID1,
...
uint32_t FusionManager::GetResultDemon(
  ...
  uint32_t baseDemonType3 =
      def3.second ? def3.second->GetUnionData()->GetBaseDemonID() : 0;
  ...
  for (auto special : specialFusions) {
    ...
    for (size_t i = 0; i < 3; i++) {
      ...
      if (sourcePair.second) {
        ...
        auto sourceBaseDemonType = specialDef->GetUnionData()->GetBaseDemonID();
  ...
  if (triFusion) {
    ...
    std::list<std::pair<uint8_t, std::shared_ptr<objects::MiDevilData>>> defs =
        {def1, def2, def3};
    ...
    if (darkCount > 0) {
      if (darkCount == 1) {
        ...
        auto darkDef =
            (race1 == dRace1 || race1 == dRace2)
                ? def1
                : ((race2 == dRace1 || race2 == dRace2) ? def2 : def3);

        bool found1, found2;
        size_t race1Idx = GetRaceIndex(
            (uint8_t)otherDef1.second->GetCategory()->GetRace(), found1);
        size_t race2Idx = GetRaceIndex(
            (uint8_t)otherDef2.second->GetCategory()->GetRace(), found2);

        if (!found1 || !found2) {
          LogFusionManagerErrorMsg(
              "Invalid single dark, dual fusion race encountered for "
              "trifusion\n");
        ...
        race2Idx = GetRaceIndex(
            (uint8_t)darkDef.second->GetCategory()->GetRace(), found2);

        if (!found1 || !found2) {
          LogFusionManagerErrorMsg(
              "Invalid single dark, 2nd dual fusion race encountered for "
              "trifusion\n");
        ...
        bool found1, found2;
        size_t race1Idx = GetRaceIndex(
            (uint8_t)darkDef1.second->GetCategory()->GetRace(), found1);
        size_t race2Idx = GetRaceIndex(
            (uint8_t)otherDef.second->GetCategory()->GetRace(), found2);

        if (!found1 || !found2) {
          LogFusionManagerErrorMsg(
              "Invalid double dark, dual fusion race encountered for "
              "trifusion\n");
        ...
        uint8_t resultRace = FUSION_RACE_MAP[race1Idx + 1][race2Idx];
        resultDef = GetResultDemon(
            resultRace, GetAdjustedLevelSum(darkDef1.first, otherDef.first));

        race1Idx = GetRaceIndex(resultRace, found1);
        race2Idx = GetRaceIndex(
            (uint8_t)darkDef2.second->GetCategory()->GetRace(), found2);

        if (!found1 || !found2 || !resultDef) {
          LogFusionManagerErrorMsg(
              "Invalid double dark, 2nd dual fusion race encountered for "
              "trifusion\n");
      ...
      for (auto elemSpecial : SVR_CONST.TRIFUSION_SPECIAL_ELEMENTAL) {
        ...
        if ((elemSpecial[0] == elemType1 && elemSpecial[1] == elemType2) ||
          ...
          // Check valid races
          if (otherRace == elemSpecial[2] || otherRace == elemSpecial[3] ||
      ...
      if (!found1 || !found2) {
        LogFusionManagerErrorMsg(
            "Invalid single element, dual fusion race encountered for "
            "trifusion\n");
      ...
      if ((lnc1 < 5000 && lnc1 > -5000) && (lnc2 < 5000 && lnc2 > -5000) &&
          (lnc3 < 5000 && lnc3 > -5000)) {
      ...
      int8_t adjustedLevelSum =
          (int8_t)(((float)levelSum / 3.f) + 1.f + (float)finalLevelAdjust);
      ...
      if (race1 == race2) {
        ...
        if (!found1 || !found2) {
          LogFusionManagerErrorMsg(
              "Invalid dual fusion race encountered for trifusion\n");
        ...
        if (elemIdx) {
          ...
          uint32_t result = GetElementalFuseResult(
              elemType, race3, def3.second->GetBasic()->GetID());
          if (result == 0) {
            LogFusionManagerError([&]() {
              return libcomp::String(
                         "Invalid elemental fusion request during TriFusion "
                         "mid-point fusion: %1, %2, %3\n")
                  .Arg(demonType1)
                  .Arg(demonType2)
                  .Arg(demonType3);
            });
          ...
          LogFusionManagerError([&]() {
            return libcomp::String(
                       "Attempted TriFusion on same race highest level demons "
                       "that did not result in an elemental midpoint result: "
                       "%1, %2, %3\n")
                .Arg(demonType1)
                .Arg(demonType2)
                .Arg(demonType3);
      ...
      } else if (f1 >= 2 && f1 <= 9 && f2 >= 2 && f2 <= 9 && f3 >= 2 &&
                 f3 <= 9) {
  ...
  if (race1 == mRace || race2 == mRace) {
    ...
    if (race1 == race2) {
      ...
      return GetMitamaType((size_t)FUSION_ELEMENTAL_MITAMA[eIdx1][eIdx2]);
    }
    ...
    uint32_t result = GetElementalFuseResult(elementalType, race, demonType);
    if (result == 0) {
      LogFusionManagerError([&]() {
        return libcomp::String(
                   "Invalid elemental fusion request of demon IDs  %1 and %2 "
    ...
    if (!found1 || !found2) {
      LogFusionManagerError([&]() {
        return libcomp::String(
                   "Invalid fusion request of demon IDs %1 and %2 received "
                   "from account: %3\n")
            .Arg(demonType1)
            .Arg(demonType2)
            .Arg(state->GetAccountUID().ToString());
    ...
    if (resultRace == 0) {
      LogFusionManagerError([&]() {
        return libcomp::String(
                   "Invalid fusion result of demon IDs %1 and %2 requested "
                   "from account: %3\n")
            .Arg(demonType1)
            .Arg(demonType2)
            .Arg(state->GetAccountUID().ToString());
...
uint32_t FusionManager::GetMistakeResultType(
  ...
  mistakeDefs.remove_if([&](const std::shared_ptr<objects::FusionMistake>& m) {
    if (!triFusion &&
    ...
    if (m->SourceTypesCount() > 0) {
      ...
      for (size_t i = 0; i < 3; i++) {
        ...
        if (def) {
          for (uint8_t k = 0; k < 3; k++) {
            ...
            types.insert(def->GetUnionData()->GetBaseDemonID());
      ...
      return !FusionManager::TypesMatch(matches, triFusion, m->GetSourceAny());
    }
  ...
  mistakeDefs.remove_if([targetDef, targetLevel](
                            const std::shared_ptr<objects::FusionMistake>& m) {
    if (m->TargetRaceIDsCount() > 0 &&
        !m->TargetRaceIDsContains(
            (uint8_t)targetDef->GetCategory()->GetRace())) {
      // Target race not contained
      return true;
    } else if (m->GetTargetLevelMin() && m->GetTargetLevelMin() > targetLevel) {
      // Target level too low
      return true;
    } else if (m->GetTargetLevelMax() && m->GetTargetLevelMax() < targetLevel) {
  ...
  mistakeDefs.remove_if(
      [&validTypes](const std::shared_ptr<objects::FusionMistake>& m) {
  ...
  mistakeDefs.remove_if(
      [worldClock](const std::shared_ptr<objects::FusionMistake>& m) {
        // Active rate is 0%
        float moonRate = m->GetMoonRates((size_t)worldClock.MoonPhase);
        return moonRate == 0.f || (moonRate < 0.f && m->GetBaseRate() == 0.f);
      });
  ...
  for (auto& mistakeDef : mistakeDefs) {
    float moonRate = mistakeDef->GetMoonRates((size_t)worldClock.MoonPhase);
  ...
  if (RNG_DEC(float, 0.01f, 100.f, 2) <= maxRate) {
    ...
    for (auto& mistakeDef : mistakeDefs) {
      ...
      if (hit <= rate) {
        return libcomp::Randomizer::GetEntry(validTypes[mistakeDef->GetID()]);
      }
...
bool FusionManager::TypesMatch(const std::array<std::set<uint8_t>, 3>& matches,
  ...
  if (any) {
    ...
    return matches[0].size() > 0 || matches[1].size() > 0 ||
           matches[2].size() > 0;
  } else {
...
void FusionManager::EndExchange(
  ...
  if (exchange) {
    ...
    switch (exchange->GetType()) {
      ...
      case objects::PlayerExchangeSession::Type_t::TRIFUSION_HOST: {
        ...
        auto partyClients =
            managerConnection->GetPartyConnections(client, true, true);
        ...
        if (exchange->GetType() ==
          ...
          ChannelClientConnection::BroadcastPacket(partyClients, request);
        } else {
          // End for self
          ...
          auto otherClient = otherCState
                                 ? managerConnection->GetEntityClient(
                                       otherCState->GetEntityID(), false)
                                 : nullptr;
          auto otherState =
              otherClient ? otherClient->GetClientState() : nullptr;
          auto tfSession =
              otherState
...
int8_t FusionManager::ProcessFusion(
  ...
  uint32_t resultDemonType =
      GetResultDemon(client, demonID1, demonID2, demonID3, specialResult);
  ...
  if (costItemType == 0 || costItemType == SVR_CONST.ITEM_MACCA) {
    ...
    LogFusionManagerError([&]() {
      return libcomp::String(
                 "Invalid cost item type supplied for demon fusion: %1\n")
          .Arg(resultDemonType);
    });
  ...
  // Tri-Fusion success uses the same formula as normal fusion but gets a flat
  // 12% boost
  double successRate =
      ceil((140.0 - (difficulty * 2.5)) + (difficulty - (baseLevel * 1.5)) +
           (difficulty * 0.5) + (demonID3 > 0 ? 12.0 : 0.0));
  if (demonID3 > 0) {
    // Tri-Fusion success rate gets adjusted by familiarity sum / 4000
    double adjust =
        floor((double)(demon1->GetFamiliarity() + demon2->GetFamiliarity() +
                       demon3->GetFamiliarity()) /
              4000.0);
    successRate += adjust;
  } else if (costItemType == SVR_CONST.ITEM_KREUZ) {
  ...
  for (auto dPair : dMap) {
    ...
    if (expertiseBoost.find(entityID) == expertiseBoost.end()) {
      ...
      if (client == dClient) {
        uint8_t dRank = dCState->GetExpertiseRank(EXPERTISE_DEMONOLOGY);
        if (demonID3 > 0) {
    ...
    if (mistakeType) {
      if (demon3) {
        LogFusionManagerDebug([&]() {
          return libcomp::String(
                     "Tri-fusion %1 for result %2 (%3 x %4 x %5) with success "
                     "rate %6 resulted in fusion mistake to type %7 for "
                     "character: %8\n")
        ...
        LogFusionManagerDebug([&]() {
          return libcomp::String(
                     "Fusion %1 for result %2 (%3 x %4) with success rate %5 "
                     "resulted in fusion mistake to type %6 for character: "
                     "%7\n")
              .Arg(failed ? "failure" : "success")
              .Arg(resultDemonType)
              .Arg(demon1->GetType())
  ...
  if (failed) {
    ...
    int32_t ePoints = characterManager->CalculateExpertiseGain(
        cState, EXPERTISE_FUSION, 0.5f);
    expPoints.push_back(
        std::pair<uint8_t, int32_t>(EXPERTISE_FUSION, ePoints + 10));

    ePoints = characterManager->CalculateExpertiseGain(
        cState, EXPERTISE_DEMONOLOGY, 0.25f);
    expPoints.push_back(
        std::pair<uint8_t, int32_t>(EXPERTISE_DEMONOLOGY, ePoints + 10));
    ...
    if (demon3) {
      LogFusionManagerDebug([&]() {
        return libcomp::String(
                   "Tri-fusion failed for result %1 (%2 x %3 x %4) with "
                   "success rate %5 for character: %6\n")
            .Arg(resultDemonType)
            .Arg(demon1->GetType())
            .Arg(demon2->GetType())
            .Arg(demon3->GetType())
      ...
      LogFusionManagerDebug([&]() {
        return libcomp::String(
                   "Fusion failed for result %1 (%2 x %3) with success rate %4 "
                   "for character: %5\n")
            .Arg(resultDemonType)
            .Arg(demon1->GetType())
            .Arg(demon2->GetType())
  ...
  if (!mistake) {
    if (demon3) {
      LogFusionManagerDebug([&]() {
        return libcomp::String(
                   "Tri-fusion succeeded with result %1 (%2 x %3 x %4) with "
                   "success rate %5 for character: %6\n")
            .Arg(resultDemonType)
            .Arg(demon1->GetType())
            .Arg(demon2->GetType())
      ...
      LogFusionManagerDebug([&]() {
        return libcomp::String(
                   "Fusion succeeded with result %1 (%2 x %3) with success "
                   "rate %4 for character: %5\n")
            .Arg(resultDemonType)
            .Arg(demon1->GetType())
            .Arg(demon2->GetType())
            .Arg(successRate)
  ...
  for (auto demon : {demon1, demon2, demon3}) {
    if (demon) {
      ...
      familiarity = (uint16_t)(familiarity + ((float)demon->GetFamiliarity() *
                                              (demon3 ? 0.2f : 0.25f)));
      ...
      if (it != dMap.end()) {
        ...
        if (dState->GetEntity() == demon) {
          characterManager->StoreDemon(dClient, true, demonID3 > 0 ? 16 : 12);
        }
      }
  ...
  if (mitamaFusion) {
    ...
    bool found = false;
    size_t mitamaIdx =
        GetMitamaIndex(mitamaDef->GetUnionData()->GetBaseDemonID(), found);
    if (!found) {
      mitama = demon2;
    ...
    if (!characterManager->MitamaDemon(
            client, state->GetObjectID(nonMitama->GetUUID()),
            growthData->GetGrowthType(), (uint8_t)(mitamaIdx + 1))) {
      return -1;
    }
    ...
    auto inheritRestrictions =
        demonData->GetGrowth()->GetInheritanceRestrictions();
    ...
    for (auto source : {demon1, demon2, demon3}) {
      if (source) {
        for (auto learned : source->GetLearnedSkills()) {
          ...
          if (inheritedSkillCounts.find(learned) ==
              inheritedSkillCounts.end()) {
    ...
    if (iType <= 21) {
      for (auto iPair : inherited) {
        ...
        uint8_t baseValue =
            INHERITENCE_SKILL_MAP[(size_t)(affinity - 2)][iType];
  ...
  state->SetObjectID(resultDemon->GetUUID(), server->GetNextObjectID(), true);
  ...
  for (auto demon : {demon1, demon2, demon3}) {
    if (demon) {
      ...
      if (it != dMap.end()) {
        auto dClient = it->second;
        characterManager->SendDemonBoxData(dClient, 0, {demon->GetBoxSlot()});
      }
    }
  }
  ...
  server->GetEventManager()->UpdateDemonQuestCount(
      client, objects::DemonQuest::Type_t::FUSE, resultDemonType, 1);

  // Update expertise for success
  std::list<std::pair<uint8_t, int32_t>> expPoints;

  int32_t ePoints =
      characterManager->CalculateExpertiseGain(cState, EXPERTISE_FUSION, 2.f);
  expPoints.push_back(
      std::pair<uint8_t, int32_t>(EXPERTISE_FUSION, ePoints + 10));
...
std::shared_ptr<objects::MiDevilData> FusionManager::GetResultDemon(
  ...
  if (fusionRanges.size() == 0) {
    LogFusionManagerError([&]() {
      return libcomp::String("No valid fusion range found for race ID: %1\n")
          .Arg(race);
    });
...
uint32_t FusionManager::GetElementalType(size_t elementalIndex) const {
  const static uint32_t FUSION_ELEMENTAL_TYPES[] = {
      SVR_CONST.ELEMENTAL_1_FLAEMIS, SVR_CONST.ELEMENTAL_2_AQUANS,
      SVR_CONST.ELEMENTAL_3_AEROS, SVR_CONST.ELEMENTAL_4_ERTHYS};
...
uint32_t FusionManager::GetMitamaType(size_t mitamaIndex) const {
  const static uint32_t FUSION_MITAMA_TYPES[] = {
      SVR_CONST.MITAMA_1_ARAMITAMA, SVR_CONST.MITAMA_2_NIGIMITAMA,
      SVR_CONST.MITAMA_3_KUSHIMITAMA, SVR_CONST.MITAMA_4_SAKIMITAMA};
...
bool FusionManager::IsTriFusionValid(
...
uint32_t FusionManager::RankUpDown(uint8_t raceID, uint32_t demonType,
                                   bool up) {
  auto fusionRanges =
      mServer.lock()->GetDefinitionManager()->GetFusionRanges(raceID);
