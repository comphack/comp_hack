...
void TeamInvite(std::shared_ptr<WorldServer> server,
  ...
  if (cLogin && targetLogin && targetChar && targetLogin->GetChannelID() >= 0) {
    auto team = cLogin->GetTeamID()
                    ? characterManager->GetTeam(cLogin->GetTeamID())
                    : nullptr;
    if (!team) {
...
void TeamCancel(std::shared_ptr<WorldServer> server,
  ...
  auto team = characterManager->GetTeam(teamID);
  auto leaderLogin =
      team ? characterManager->GetCharacterLogin(team->GetLeaderCID())
           : nullptr;

  if (leaderLogin && leaderLogin->GetChannelID() >= 0 &&
      teamID == leaderLogin->GetTeamID()) {
    auto channel =
        server->GetChannelConnectionByID(leaderLogin->GetChannelID());
    if (channel) {
...
bool Parsers::TeamUpdate::Parse(
  ...
  switch ((InternalPacketAction_t)mode) {
    case InternalPacketAction_t::PACKET_ACTION_ADD: {
      ...
      if (p.Left() < 1) {
        LogTeamErrorMsg(
            "Team form request encountered without type specified\n");
    ...
    case InternalPacketAction_t::PACKET_ACTION_GROUP_LIST: {
      ...
      if (team && team->GetID() == teamID) {
        ...
        for (int32_t worldCID : memberIDs) {
          auto memberLogin = characterManager->GetCharacterLogin(worldCID);
          auto memberChar =
    ...
    case InternalPacketAction_t::PACKET_ACTION_GROUP_LEADER_UPDATE: {
      ...
      if (p.Left() < 4) {
        LogTeamError([&]() {
          return libcomp::String(
                     "Missing target CID parameter for command %1\n")
              .Arg(mode);
        });
    ...
    case InternalPacketAction_t::PACKET_ACTION_TEAM_ZIOTITE: {
      ...
      if (p.Left() < 5) {
        LogTeamError([&]() {
          return libcomp::String(
                     "Missing ziotite amount parameters for command %1\n")
              .Arg(mode);
        });
      ...
      characterManager->TeamZiotiteUpdate(teamID, cLogin, sZiotite, lZiotite);
    } break;
    default:
      break;
  }
