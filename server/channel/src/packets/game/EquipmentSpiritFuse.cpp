...
bool Parsers::EquipmentSpiritFuse::Parse(
  ...
  if (!error) {
    ...
    if (mainDef) {
      ...
      subCategory = mainDef->GetCommon()->GetCategory()->GetSubCategory();
    }
    ...
    if (!mainDef || !basicDef || !specialDef ||
      ...
      LogItemError([&]() {
        return libcomp::String(
                   "EquipmentSpiritFuse request received with one or more "
                   "invalid item type(s): %1, %2, %3\n")
            .Arg(mainItem->GetType())
            .Arg(basicItem->GetType())
      ...
      LogItemError([&]() {
        return libcomp::String(
                   "EquipmentSpiritFuse request received with invalid "
                   "equipment type item: %1\n")
      ...
      error = true;
               ...
               subCategory !=
                   basicDef->GetCommon()->GetCategory()->GetSubCategory() ||
               subCategory !=
                   specialDef->GetCommon()->GetCategory()->GetSubCategory()) {
      LogItemError([&]() {
        return libcomp::String(
                   "EquipmentSpiritFuse request received with equipment types "
                   "that do not match: %1\n")
            .Arg(state->GetAccountUID().ToString());
      });
      ...
      if (genders.size() > 1) {
        LogItemError([&]() {
          return libcomp::String(
                     "EquipmentSpiritFuse request received with differing "
                     "gender armor: %1\n")
              .Arg(state->GetAccountUID().ToString());
        });
    ...
    if (!error) {
      ...
      for (auto& itemDef : {mainDef, basicDef, specialDef}) {
        if (CharacterManager::IsCPItem(itemDef)) {
          ...
          costSum = costSum + (uint32_t)itemDef->GetBasic()->GetBuyPrice();
        }
      }
      ...
      cost =
          (uint32_t)floor(floor((double)costSum / 3.0) * 0.8 * bonusMultiplier);
    }
  }
  ...
  if (!error) {
    ...
    double chainBoost =
        (double)(2.5 * (floor(0.1 * cState->GetExpertiseRank(
    ...
    double expBoost =
        (double)(5.0 / 3.0 *
                 (floor(0.1 *
                        cState->GetExpertiseRank(EXPERTISE_WEAPON_KNOWLEDGE)) +
                  floor(0.1 *
                        cState->GetExpertiseRank(EXPERTISE_GUN_KNOWLEDGE)) +
    ...
    if (chainBoost > 1.0 || expBoost > 1.0) {
      successRate =
          successRate + (chainBoost + expBoost) * cpBoost * demonBoost;
    }
    ...
    for (uint32_t effectItem : effectItems) {
      ...
      if (it != SVR_CONST.SPIRIT_FUSION_BOOST.end()) {
        ...
        if (itemData && CharacterManager::IsCPItem(itemData) &&
            ((itemData->GetBasic()->GetFlags() & ITEM_FLAG_CRYSTAL) != 0)) {
    ...
    mainItem->SetBasicEffect(basicEffect ? basicEffect : basicItem->GetType());
    ...
    mainItem->SetSpecialEffect(spEffect ? spEffect : specialItem->GetType());
    ...
    if (result == RESULT_FAILURE) {
      ...
      mainItem->SetRentalExpiration(
          (uint32_t)((int32_t)std::time(0) + expirationDays * 24 * 3600));
    } else {
        ...
        if (bonusSlots.size() > 0) {
          ...
          for (size_t slot : bonusSlots) {
            if (slots.find(slot) == slots.end()) {
              int32_t slotChance = (int32_t)(
                  100.0 / (double)(5 + mainItem->GetFuseBonuses(slot)) +
                  gSuccessBoost);
              if (slotChance >= 100 || RNG(int32_t, 1, 100) <= slotChance) {
                slots.insert(slot);
              }
            }
          }
          ...
          for (size_t slot : slots) {
            mainItem->SetFuseBonuses(
                slot, (int8_t)(mainItem->GetFuseBonuses(slot) + 1));
          }
        }
      }
    }
    ...
    if (assistItem) {
      updateItems[assistItem] = (uint16_t)(assistItem->GetStackSize() - 1);
    }
    ...
    if (characterManager->UpdateItems(client, false, insertItems,
      ...
      LogItemError([&]() {
        return libcomp::String(
                   "EquipmentSpiritFuse failed to update items: %1\n")
            .Arg(state->GetAccountUID().ToString());
      });
