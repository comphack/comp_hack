...
bool Downloader::checkFile(const FileData *info) {
  ...
  if (compMatch.exactMatch(info->path)) path = compMatch.cap(1);

  if (path == "ImagineUpdate.dat") return true;

  if (QFileInfo(path).size() != info->uncompressed_size) return false;
  ...
  if (checksum != info->uncompressed_hash) return false;
...
void Downloader::requestError(QNetworkReply::NetworkError code) {
  ...
  if (!errorString.isEmpty()) {
#ifdef COMP_HACK_LOGSTDOUT
    std::cout << "Download failed: " << errorString.toLocal8Bit().data()
              << std::endl;
#endif  // COMP_HACK_LOGSTDOUT
    ...
    mStatusCode =
        mCurrentReq->attribute(QNetworkRequest::HttpStatusCodeAttribute)
            .toInt();
    QString phrase =
        mCurrentReq->attribute(QNetworkRequest::HttpReasonPhraseAttribute)
            .toString();

#ifdef COMP_HACK_LOGSTDOUT
    std::cout << "Download failed: Server returned status code " << mStatusCode
              << " " << phrase.toLocal8Bit().data() << std::endl;
#endif  // COMP_HACK_LOGSTDOUT

    expressFinish(tr("Download failed: Server returned status code %1 %2")
                      .arg(mStatusCode)
                      .arg(phrase));
  }
}
...
int Downloader::uncompressChunk(const void *src, void *dest, int in_size,
...
void Downloader::requestFinished() {
  ...
  if (mPath.isEmpty()) {
...
#ifdef COMP_HACK_LOGSTDOUT
      std::cout << "Download failed: Compressed size does not match"
                << std::endl;
#endif  // COMP_HACK_LOGSTDOUT

      return expressFinish(
          "Download failed: Compressed "
          "size does not match");
    }

    if (checksum != mCurrentFile->compressed_hash) {
#ifdef COMP_HACK_LOGSTDOUT
      std::cout << "Download failed: Invalid compressed hash detected"
                << std::endl;
#endif  // COMP_HACK_LOGSTDOUT

      return expressFinish("Download failed: Invalid compressed hash detected");
    }
    ...
    if (compMatch.exactMatch(mPath)) {
      ...
      QByteArray uncomp = uncompress(mData, mCurrentFile->uncompressed_size);

      if (uncomp.size() != mCurrentFile->uncompressed_size) {
#ifdef COMP_HACK_LOGSTDOUT
        std::cout << "Download failed: Uncompressed size does not match"
                  << std::endl;
#endif  // COMP_HACK_LOGSTDOUT

        return expressFinish(
            "Download failed: Uncompressed size does not match");
      }

      checksum =
          QCryptographicHash::hash(uncomp, QCryptographicHash::Md5).toHex();

      if (checksum != mCurrentFile->uncompressed_hash) {
#ifdef COMP_HACK_LOGSTDOUT
        std::cout << "Download failed: Invalid uncompressed hash detected"
                  << std::endl;
#endif  // COMP_HACK_LOGSTDOUT

        return expressFinish(
            "Download failed: Invalid uncompressed hash detected");
      }
    ...
    if (mSaveFiles) {
      ...
      last.write(QString("FILE : ./%1,%2,%3,%4,%5\n")
                     .arg(mCurrentFile->path)
                     .arg(mCurrentFile->compressed_hash)
                     .arg(mCurrentFile->compressed_size)
                     .arg(mCurrentFile->uncompressed_hash)
                     .arg(mCurrentFile->uncompressed_size)
                     .toUtf8());
      last.close();
    }
  }
...
void Downloader::expressFinish(const QString &msg) {
  ...
  QFile last("ImagineUpdate2.dat");
  if (mSaveFiles) last.open(QIODevice::WriteOnly | QIODevice::Append);
  ...
  while (!mFiles.isEmpty()) {
    ...
    if (!mWhiteList.isEmpty()) {
      ...
      foreach (QString pattern, mWhiteList) {
        if (info->path.left(pattern.length()) != pattern) continue;
    ...
    if (!mOldFiles.contains(info->path)) continue;

    FileData *old = mOldFiles.value(info->path);
    if (old->uncompressed_hash != info->uncompressed_hash) continue;

    if (mSaveFiles) {
      last.write(QString("FILE : ./%1,%2,%3,%4,%5\n")
                     .arg(info->path)
                     .arg(info->compressed_hash)
                     .arg(info->compressed_size)
                     .arg(info->uncompressed_hash)
                     .arg(info->uncompressed_size)
                     .toUtf8());
    }
  }
...
void Downloader::setURL(const QString &url) { mURL = url; }
