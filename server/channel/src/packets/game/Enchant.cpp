...
bool Parsers::Enchant::Parse(
  ...
  if (responseCode == EntrustErrorCodes_t::SUCCESS) {
    if (characterManager->GetSynthOutcome(state, exchangeSession,
                                          specialEnchantItemType, successRates,
                                          &effectID, &enchantSpecialID)) {
      int32_t successRate =
          choice == 0 ? successRates.front() : successRates.back();
      ...
      if (successRate > 0 && RNG(int32_t, 0, 100) <= successRate) {
        if (choice == 0) {
          ...
          updateItem =
              characterManager->GenerateItem(specialEnchantItemType, 1);
          if (enchantSpecialID) {
            // Set mod slots if we have them (number can only
            // go up from base item)
            auto specialEnchant =
                definitionManager->GetEnchantSpecialData(enchantSpecialID);
            if (specialEnchant && specialEnchant->GetSlots()) {
              for (uint8_t i = 0; i < specialEnchant->GetSlots() && i < 5;
                   i++) {
                updateItem->SetModSlots((size_t)i, MOD_SLOT_NULL_EFFECT);
              }
            }
          }
  ...
  if (responseCode == EntrustErrorCodes_t::SUCCESS) {
    ...
    if (rewards.size() > 0) {
      ...
      for (uint8_t rewardGroup :
        ...
        for (auto reward : rewards[rewardGroup]) {
          ...
          targetInventory->SetItems((size_t)reward->GetBoxSlot(), NULLUUID);
          ...
          // Give it to the executor of the skill, the source
    ...
    if (inputItem && !success) {
      ...
      switch (exchangeSession->GetType()) {
        case objects::PlayerExchangeSession::Type_t::ENCHANT_TAROT:
          characterManager->UpdateDurability(targetClient, inputItem, -5000);
          break;
        case objects::PlayerExchangeSession::Type_t::ENCHANT_SOUL:
          characterManager->UpdateDurability(targetClient, inputItem, -10000);
          break;
        default:
          break;
      }
    ...
    if (success) {
      ...
      // on failure
      auto it = SVR_CONST.ADJUSTMENT_ITEMS.find(mirror ? mirror->GetType() : 0);
    ...
    for (auto useItem : useItems) {
      if (useItem && useItem->GetItemBox() == targetInventory->GetUUID()) {
        if (useItem->GetStackSize() == 1) {
          ...
          useItem->SetStackSize((uint16_t)(useItem->GetStackSize() - 1));
    ...
    if (!server->GetWorldDatabase()->ProcessChangeSet(dbChanges)) {
      LogGeneralErrorMsg(
          "Enchant result failed to save, disconnecting player(s)\n");
