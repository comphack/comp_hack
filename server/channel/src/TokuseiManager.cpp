...
TokuseiManager::TokuseiManager(const std::weak_ptr<ChannelServer>& server)
    : mServer(server) {}
  ...
  for (auto tPair : allTokusei) {
    ...
    for (auto aspect : tPair.second->GetAspects()) {
      if (aspect->GetType() == TokuseiAspectType::SKILL_ADD) {
        if (tPair.second->GetTargetType() !=
            objects::Tokusei::TargetType_t::SELF) {
          LogTokuseiManagerError([&]() {
            return libcomp::String(
                       "Skill granting tokusei encountered with target type "
                       "other than 'self': %1\n")
                .Arg(tPair.first);
          });
          ...
          LogTokuseiManagerError([&]() {
            return libcomp::String(
                       "Conditional skill granting tokusei encountered: %1\n")
                .Arg(tPair.first);
          });
        ...
        if (aspect->GetValue() == 0) {
          LogTokuseiManagerError([tPair]() {
            return libcomp::String(
                       "Tokusei encountered with expiration triggered effect "
                       "assigned to no trigger value: %1\n")
                .Arg(tPair.first);
          });
        ...
        mExpirationTriggeredTokusei[tPair.first].insert(aspect->GetValue());
      }
    }
      ...
      if (skillData) {
        for (int32_t tokuseiID : skillData->GetCharastic()->GetCharastic()) {
          ...
          if (it != skillGrantTokusei.end()) {
            LogTokuseiManagerError([&]() {
              return libcomp::String(
                         "Skill granted from tokusei '%1' contains a nested "
                         "skill granting effect: '%2'\n")
                  .Arg(skillID)
                  .Arg(tokuseiID);
            });
    ...
    if (tPair.second->SkillConditionsCount() > 0 ||
      ...
      for (auto conditionList : {tPair.second->GetSkillConditions(),
        ...
        for (auto condition : conditionList) {
          if (condition->GetComparator() !=
            ...
            LogTokuseiManagerError([&]() {
              return libcomp::String(
                         "Skill tokusei conditions can only compare simple "
                         "equals/not equal conditions: %1\n")
                  .Arg(tPair.first);
            });
      ...
      if (tPair.second->SkillConditionsCount() &&
        ...
        LogTokuseiManagerError([&]() {
          return libcomp::String(
                     "Skill tokusei encountered with both source and target "
                     "conditions: %1\n")
              .Arg(tPair.first);
        });
      ...
      const std::set<uint8_t> invalidCorrectTypes = {
          (uint8_t)CorrectTbl::STR,        (uint8_t)CorrectTbl::MAGIC,
          (uint8_t)CorrectTbl::VIT,        (uint8_t)CorrectTbl::INT,
          (uint8_t)CorrectTbl::SPEED,      (uint8_t)CorrectTbl::LUCK,
          (uint8_t)CorrectTbl::HP_MAX,     (uint8_t)CorrectTbl::MP_MAX,
          (uint8_t)CorrectTbl::RATE_XP,    (uint8_t)CorrectTbl::RATE_MAG,
          (uint8_t)CorrectTbl::RATE_MACCA, (uint8_t)CorrectTbl::RATE_EXPERTISE};
      ...
      if (invalidSkillAdjust) {
        LogTokuseiManagerError([&]() {
          return libcomp::String(
                     "Skill tokusei encountered with an unsupported skill "
                     "adjustment: %1\n")
              .Arg(tPair.first);
        });
        ...
        for (auto condition : tPair.second->GetSkillConditions()) {
          ...
          switch (condition->GetSkillConditionType()) {
            ...
            case TokuseiSkillConditionType::SKILL_EXPERTISE:
              if (optionGroupIDs.find(ogID) != optionGroupIDs.end()) {
                LogTokuseiManagerError([&]() {
                  return libcomp::String(
                             "Tokusei encountered with cost reduction aspects "
                             "and complex skill conditions: %1\n")
                      .Arg(tPair.first);
                });
            ...
            default:
              LogTokuseiManagerError([&]() {
                return libcomp::String(
                           "Skill tokusei encountered with cost reduction and "
                           "unsupported skill condition: %1\n")
                    .Arg(tPair.first);
              });
          ...
          if (condition->GetComparator() !=
            ...
            LogTokuseiManagerError([&]() {
              return libcomp::String(
                         "Skill tokusei encountered with cost reduction and "
                         "comparator other than equals: %1\n")
                  .Arg(tPair.first);
            });
  ...
  for (int32_t tokuseiID : baseStatTokuseiIDs) {
    ...
    if (it != allTokusei.end()) {
      ...
      if (tokuseiData->ConditionsCount() > 0) {
        LogTokuseiManagerError([&]() {
          return libcomp::String(
                     "Stat conditional enchantment tokusei encountered with "
                     "non-skill conditions: %1\n")
              .Arg(it->first);
        });
      ...
      for (auto ct : cTables) {
        if (ct->GetID() <= CorrectTbl::LUCK &&
          ...
          LogTokuseiManagerError([&]() {
            return libcomp::String(
                       "Stat conditional enchantment tokusei encountered with "
                       "percentage core stat adjustment: %1\n")
                .Arg(it->first);
          });
      ...
      if (skillGrantTokusei.find(it->first) != skillGrantTokusei.end()) {
        LogTokuseiManagerError([&]() {
          return libcomp::String(
                     "Skill granting stat conditional enchantment tokusei "
                     "encountered: %1\n")
              .Arg(it->first);
        });
...
bool TokuseiManager::GatherTimedTokusei(
      ...
      case objects::TokuseiCondition::Type_t::MOON_PHASE:
        switch (condition->GetComparator()) {
          ...
          default:
            LogTokuseiManagerError([&]() {
              return libcomp::String(
                         "Invalid comparator encountered on time restricted "
                         "tokusei '%1'\n")
                  .Arg(tokusei->GetID());
            });
  ...
  if (afterTime.size() > 0 || beforeTime.size() > 0) {
    if (afterTime.size() != beforeTime.size()) {
      LogTokuseiManagerError([&]() {
        return libcomp::String(
                   "Encountered time restricted tokusei with at least one "
                   "condition option group that is not a timespan: '%1'\n")
            .Arg(tokusei->GetID());
      });
    ...
    for (auto timePair : beforeTime) {
      if (afterTime[timePair.first].size() == 0) {
        LogTokuseiManagerError([&]() {
          return libcomp::String(
                     "Encountered time restricted tokusei with condition "
      ...
      if (!success) {
        LogTokuseiManagerError([&]() {
          return libcomp::String(
                     "Encountered time restricted tokusei with invalid "
                     "timespan option group: '%1' (%2)\n")
              .Arg(tokusei->GetID())
              .Arg(timePair.first);
        });
...
std::unordered_map<int32_t, bool> TokuseiManager::Recalculate(
    const std::shared_ptr<ActiveEntityState>& eState,
    std::set<TokuseiConditionType> changes) {
  bool doRecalc = false;

  // Since anything pertaining to party members or summoning a new demon
  // requires a full recalculation check, only check another entity if a partner
  // demon's familiarity changed
  if (eState->GetEntityType() == EntityType_t::PARTNER_DEMON &&
      changes.find(TokuseiConditionType::PARTNER_FAMILIARITY) !=
          changes.end()) {
    auto state =
    ...
    if (state) {
      ...
      doRecalc =
          triggers.find((int8_t)TokuseiConditionType::PARTNER_FAMILIARITY) !=
          triggers.end();
    }
  }
...
std::unordered_map<int32_t, bool> TokuseiManager::Recalculate(
    const std::shared_ptr<ActiveEntityState>& eState, bool recalcStats,
    std::set<int32_t> ignoreStatRecalc) {
  std::list<std::shared_ptr<ActiveEntityState>> entities =
      GetAllTokuseiEntities(eState);
  return Recalculate(entities, recalcStats, ignoreStatRecalc);
}

std::unordered_map<int32_t, bool> TokuseiManager::Recalculate(
    const std::list<std::shared_ptr<ActiveEntityState>>& entities,
    bool recalcStats, std::set<int32_t> ignoreStatRecalc) {
  std::unordered_map<int32_t, bool> result;

  // Effects directly on the entity
  std::unordered_map<
      int32_t, std::unordered_map<bool, std::unordered_map<int32_t, uint16_t>>>
      newMaps;
  ...
  for (auto eState : entities) {
    ...
    for (auto tokusei : GetDirectTokusei(eState)) {
      ...
      if (evaluated.find(tokuseiID) != evaluated.end()) {
        ...
        if (worldCID && mTimedTokusei.find(tokuseiID) != mTimedTokusei.end()) {
  ...
  for (auto eState : entities) {
    ...
    auto otherEntityID =
        eState == cState ? dState->GetEntityID() : cState->GetEntityID();
      ...
      for (auto pair : otherEffects[eState->GetEntityID()]) {
        for (auto bPair : pair.second) {
          if (map[pair.first].find(bPair.first) == map[pair.first].end()) {
            ...
            map[pair.first][bPair.first] =
                (uint16_t)(map[pair.first][bPair.first] + bPair.second);
          }
        }
      }
    }
  ...
  for (auto eState : entities) {
    ...
    if (updated) {
      ...
      for (auto& ePair : mExpirationTriggeredTokusei) {
        if (calcState->EffectiveTokuseiKeyExists(ePair.first) &&
          ...
          for (int32_t expireTrigger : ePair.second) {
            expirationTriggers[eState->GetEntityID()].insert(expireTrigger);
          }
        }
      }
      ...
      for (auto statusPair : mStatusEffectTokusei) {
        bool exists =
            currentEffects.find(statusPair.first) != currentEffects.end();
        ...
        if (apply && !exists) {
          ...
          effects[statusPair.first] =
              StatusEffectChange(statusPair.first, 0, true);
        }
      }
  ...
  if (recalcStats || expirationTriggers.size() > 0) {
    ...
    for (auto eState : updatedEntities) {
      ...
      bool ignoreStats = ignoreStatRecalc.find(eState->GetEntityID()) !=
                         ignoreStatRecalc.end();
      ...
      if (expired) {
        ...
        triggers.remove_if(
            [expireIter](const std::shared_ptr<objects::ServerZoneTrigger>& t) {
              return expireIter->second.find(t->GetValue()) ==
                     expireIter->second.end();
            });

        if (triggers.size() > 0) {
          server->GetZoneManager()->HandleZoneTriggers(zone, triggers, eState,
                                                       client);
        }
      }
...
std::unordered_map<int32_t, bool> TokuseiManager::RecalculateParty(
    const std::shared_ptr<objects::Party>& party) {
  std::unordered_map<int32_t, bool> result;
...
TokuseiManager::GetAllTokuseiEntities(
    const std::shared_ptr<ActiveEntityState>& eState) {
...
std::list<std::shared_ptr<objects::Tokusei>> TokuseiManager::GetDirectTokusei(
  ...
  std::list<std::shared_ptr<objects::MiSpecialConditionData>> statConditionals;
        ...
        switch (conditionType) {
          ...
          default:
            if (conditionType >= 100 && conditionType <= 158) {
              ...
              add = cState->GetExpertiseRank((uint32_t)(conditionType - 100),
                                             definitionManager) >= (uint8_t)p1;
            }
            break;
        }
  ...
  if (statConditionals.size() > 0) {
    ...
    for (auto tokusei : retval) {
      if (tokusei->SkillConditionsCount() == 0 &&
          tokusei->GetTargetType() == objects::Tokusei::TargetType_t::SELF) {
        if (directTokusei.find(tokusei->GetID()) == directTokusei.end()) {
          directTokusei[tokusei->GetID()] = 1;
        } else {
          directTokusei[tokusei->GetID()] =
              (uint16_t)(directTokusei[tokusei->GetID()] + 1);
        }
      }
    }
    ...
    for (auto condition : statConditionals) {
      ...
      int16_t stat =
          tempCalc->GetCorrectTbl((size_t)(condition->GetType() - 10));
      if (stat > -1 && (stat >= condition->GetParams(0))) {
        for (uint32_t tokuseiID : condition->GetTokusei()) {
          auto tokusei = definitionManager->GetTokuseiData((int32_t)tokuseiID);
          if (tokusei) {
            retval.push_back(tokusei);
          }
        }
...
bool TokuseiManager::EvaluateTokuseiConditions(
    const std::shared_ptr<ActiveEntityState>& eState,
    const std::shared_ptr<objects::Tokusei>& tokusei) {
...
bool TokuseiManager::EvaluateTokuseiCondition(
    const std::shared_ptr<ActiveEntityState>& eState, int32_t tokuseiID,
    const std::shared_ptr<objects::TokuseiCondition>& condition) {
  bool numericCompare = condition->GetComparator() !=
                            objects::TokuseiCondition::Comparator_t::EQUALS &&
                        condition->GetComparator() !=
                            objects::TokuseiCondition::Comparator_t::NOT_EQUAL;
  ...
  switch (condition->GetType()) {
    ...
    case TokuseiConditionType::CURRENT_MP:
      // Current HP or MP percent matches the comparison type and value
      {
        ...
        if (condition->GetType() == TokuseiConditionType::CURRENT_HP) {
          currentValue = (int32_t)floor((float)cs->GetHP() /
                                        (float)eState->GetMaxHP() * 100.f);
        } else {
          currentValue = (int32_t)floor((float)cs->GetMP() /
                                        (float)eState->GetMaxMP() * 100.f);
        }
    ...
    case TokuseiConditionType::EQUIPPED_WEAPON_TYPE:
      ...
      if (numericCompare ||
        ...
        if (equip) {
          ...
          equipped =
              itemData &&
              (int32_t)itemData->GetCommon()->GetCategory()->GetSubCategory() ==
                  condition->GetValue();
        }
    ...
    case TokuseiConditionType::EXPERTISE:
      ...
      if (eState->GetEntityType() != EntityType_t::CHARACTER) {
        ...
        // The 2 smallest digits are the expertise ID, the rest are the rank
        // value
        int32_t expertiseID = (int32_t)(condition->GetValue() % 100);
        int32_t rankCompare =
            (int32_t)((condition->GetValue() - expertiseID) / 100);
    ...
    case TokuseiConditionType::LNC:
      ...
      if (numericCompare) {
        ...
        bool containsLNC =
            eState->IsLNCType((uint8_t)condition->GetValue(), false);
    ...
    case TokuseiConditionType::STATUS_ACTIVE:
      ...
      if (numericCompare) {
        ...
        bool exists =
            eState->StatusEffectActive((uint32_t)condition->GetValue());
    ...
    case TokuseiConditionType::MOON_PHASE:
      ...
      return mTimedTokusei[tokuseiID] &&
             eState->GetEntityType() != EntityType_t::ENEMY;
    case TokuseiConditionType::PARTY_DEMON_TYPE:
      ...
      if (numericCompare) {
          ...
          for (auto memberID : party->GetMemberIDs()) {
            ...
            if (state2) {
              ...
              if (dState2->GetZone() == zone && devilData) {
                ...
                demonIDs.insert(devilData->GetUnionData()->GetBaseDemonID());
              }
            }
          }
        }
        ...
        bool exists =
            demonIDs.find((uint32_t)condition->GetValue()) != demonIDs.end();
        return exists == (condition->GetComparator() ==
                          objects::TokuseiCondition::Comparator_t::EQUALS);
      }
      break;
...
double TokuseiManager::CalculateAttributeValue(
    ActiveEntityState* eState, int32_t value, int32_t base,
    ...
    switch (attributes->GetMultiplierType()) {
      ...
      case objects::TokuseiAttributes::MultiplierType_t::HP_LTE:
        ...
        // the precision value, multiply the value by X / 100%
        {
          ...
          auto cs = eState->GetCoreStats();
          if (cs) {
            uint8_t currentValue = (uint8_t)floor(
                (float)cs->GetHP() / (float)eState->GetMaxHP() * 100.f);
      ...
      case objects::TokuseiAttributes::MultiplierType_t::
        ...
        // (of the current demon's family) by the multiplier
        {
          ...
          if (devilData) {
            ...
            result = result *
                     floor((double)dState->GetCompendiumCount(familyID, true) /
                           (double)multValue);
          } else {
            result = 0.0;
          }
...
double TokuseiManager::GetAspectSum(
    ...
    for (auto pair : effectiveTokusei) {
      ...
      for (auto aspect : tokusei->GetAspects()) {
        if (aspect->GetType() == type) {
          val = CalculateAttributeValue(eState.get(), aspect->GetValue(), 0,
    ...
    for (auto pair : effectiveTokusei) {
      ...
      for (auto aspect : tokusei->GetAspects()) {
        if (aspect->GetType() == type) {
          ...
          if (validKeys.size() != 0 && validKeys.find(value) == validKeys.end())
            continue;
          ...
          mod = CalculateAttributeValue(eState.get(), aspect->GetModifier(), 0,
                                        aspect->GetAttributes());
...
std::list<double> TokuseiManager::GetAspectValueList(
    const std::shared_ptr<ActiveEntityState>& eState, TokuseiAspectType type,
    std::shared_ptr<objects::CalculatedEntityState> calcState) {
...
bool TokuseiManager::AspectValueExists(
    const std::shared_ptr<ActiveEntityState>& eState, TokuseiAspectType type,
    double value) {
        ...
        if (!result) {
          switch (condition->GetType()) {
            case TokuseiConditionType::GAME_TIME:
              ...
              {
                result =
                    Compare((int32_t)(clock.Hour * 100 + (int32_t)clock.Min),
                            condition, true);
              }
              break;
            case TokuseiConditionType::MOON_PHASE:
              ...
              // and comparison
              { result = Compare((int32_t)clock.MoonPhase, condition, true); }
              break;
            default:
              break;
          }
...
void TokuseiManager::UpdateMovementDecay(
    const std::shared_ptr<ChannelClientConnection>& client, float distance) {
  ...
  for (int32_t tokuseiID : mMoveDecayTokusei) {
    ...
    if (count > 0) {
      auto tokusei = server->GetDefinitionManager()->GetTokuseiData(tokuseiID);
      for (auto aspect : tokusei->GetAspects()) {
        if (aspect->GetType() == TokuseiAspectType::EQUIP_MOVE_DECAY) {
          auto item =
              character
                  ? character->GetEquippedItems((size_t)aspect->GetModifier())
                        .Get()
                  : nullptr;
...
void TokuseiManager::SendCostAdjustments(
    int32_t entityID, const std::shared_ptr<ChannelClientConnection>& client) {
  if (client) {
    auto state = client->GetClientState();
...
bool TokuseiManager::DeadTokuseiDisabled() {
  const static bool disabled =
      mServer.lock()->GetWorldSharedConfig()->GetDeadTokuseiDisabled();
  return disabled;
}

void TokuseiManager::RecalcCostAdjustments(
    const std::shared_ptr<ActiveEntityState>& eState) {
  ...
  std::map<uint8_t, std::map<uint32_t, std::list<std::pair<double, double>>>>
      defMap;
    ...
    if (mCostAdjustmentTokusei.find(tPair.first) !=
      ...
      for (auto aspect : tokusei->GetAspects()) {
        if (aspect->GetType() == TokuseiAspectType::HP_COST_ADJUST) {
          hpCost = CalculateAttributeValue(eState.get(), aspect->GetValue(), 0,
                                           aspect->GetAttributes());
        } else if (aspect->GetType() == TokuseiAspectType::MP_COST_ADJUST) {
          mpCost = CalculateAttributeValue(eState.get(), aspect->GetValue(), 0,
                                           aspect->GetAttributes());
        }
      }
      ...
      for (uint16_t i = 0; i < tPair.second; i++) {
        defMap[0][0].push_back(std::pair<double, double>(hpCost, mpCost));
      }
    }
  }
    ...
    for (auto& categoryPair : defMap) {
      ...
      for (auto& typePair : categoryPair.second) {
        ...
        for (auto& costPair : typePair.second) {
          hpCost =
              hpCost * (costPair.first <= 0.0 ? 0.0 : (costPair.first / 100.0));
          mpCost = mpCost *
                   (costPair.second <= 0.0 ? 0.0 : (costPair.second / 100.0));
        }
  ...
  if (updated.size() > 0 && (int8_t)eState->GetDisplayState() >=
    ...
    auto client =
        mServer.lock()->GetManagerConnection()->GetEntityClient(entityID);
...
bool TokuseiManager::BuildWorldClockTime(
    std::shared_ptr<objects::TokuseiCondition> condition,
    WorldClockTime& time) {
  switch (condition->GetType()) {
    case objects::TokuseiCondition::Type_t::GAME_TIME:
      if (time.Min != -1 || time.Hour != -1) {
        // Do not set twice
        return false;
      } else if (condition->GetValue() < 0 || condition->GetValue() > 2400 ||
    ...
    case objects::TokuseiCondition::Type_t::MOON_PHASE:
      if (time.MoonPhase != -1) {
        // Do not set twice
        return false;
      } else if (condition->GetValue() < 0 || condition->GetValue() >= 16) {
        // Make sure its in the valid range
        return false;
      } else {
...
bool TokuseiManager::Compare(
    int32_t value, std::shared_ptr<objects::TokuseiCondition> condition,
