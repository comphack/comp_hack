...
bool LobbyServer::Initialize() {
  ...
  mDatabase = GetDatabase(configMap, true, GetDataStore(), "/migrations/lobby");
  ...
  clientPacketManager->AddParser<Parsers::StartGame>(
      to_underlying(ClientToLobbyPacketCode_t::PACKET_START_GAME));
  clientPacketManager->AddParser<Parsers::CharacterList>(
      to_underlying(ClientToLobbyPacketCode_t::PACKET_CHARACTER_LIST));
  clientPacketManager->AddParser<Parsers::WorldList>(
      to_underlying(ClientToLobbyPacketCode_t::PACKET_WORLD_LIST));
  clientPacketManager->AddParser<Parsers::CreateCharacter>(
      to_underlying(ClientToLobbyPacketCode_t::PACKET_CREATE_CHARACTER));
  clientPacketManager->AddParser<Parsers::DeleteCharacter>(
      to_underlying(ClientToLobbyPacketCode_t::PACKET_DELETE_CHARACTER));
  clientPacketManager->AddParser<Parsers::QueryPurchaseTicket>(
      to_underlying(ClientToLobbyPacketCode_t::PACKET_QUERY_PURCHASE_TICKET));
...
void LobbyServer::SendWorldList(
    const std::shared_ptr<libcomp::TcpConnection>& connection) const {
  libcomp::Packet p;
  ...
  for (auto world : worlds) {
    ...
    for (auto channel : channels) {
      ...
      p.WriteString16Little(libcomp::Convert::ENCODING_UTF8, channel->GetName(),
                            true);
...
void LobbyServer::PromptCreateAccount() {
  ...
  while (true) {
    ...
    while (8 > password1.Length()) {
      LogGeneralErrorMsg("Account password must be at least 8 characters.\n");
  ...
  LogGeneralInfoMsg(
      "Default values will be used for this account unless you "
  ...
  if (details.length() >= 1 && tolower(details.at(0)) == 'y') {
    ...
    while (1 > enteredTicketCount || 20 < enteredTicketCount) {
      ...
      if (!ok || 1 > enteredTicketCount || 20 < enteredTicketCount) {
        LogGeneralErrorMsg("You must enter a value between 0 and 20.\n");
      }
    }
    ...
    while (1000000 < enteredCP) {
      ...
      if (!ok || 1000000 < enteredCP) {
        LogGeneralErrorMsg("You must enter a value between 1 and 1,000,000.\n");
      }
    }
    ...
    while (0 > enteredUserLevel || 1000 < enteredUserLevel) {
      ...
      if (!ok || 0 > enteredUserLevel || 1000 < enteredUserLevel) {
        LogGeneralErrorMsg("You must enter a value between 0 and 1,000.\n");
      }
    }
  ...
  account->Register(
      std::dynamic_pointer_cast<libcomp::PersistentObject>(account));
...
AccountManager* LobbyServer::GetAccountManager() { return mAccountManager; }
...
bool LobbyServer::ResetRegisteredWorlds() {
  ...
  auto worldServers =
      libcomp::PersistentObject::LoadAll<objects::RegisteredWorld>(mDatabase);

  for (auto worldServer : worldServers) {
    if (worldServer->GetStatus() ==
        objects::RegisteredWorld::Status_t::ACTIVE) {
      LogGeneralDebug([&]() {
        return libcomp::String(
                   "Resetting registered world (%1) '%2' which did not exit "
...
libcomp::String LobbyServer::ImportAccount(const libcomp::String& data,
  ...
  const tinyxml2::XMLElement* pImportObject =
  ...
  std::list<
      std::pair<libobjgen::UUID, std::shared_ptr<libcomp::PersistentObject>>>
      lobbyObjects;
  std::list<
      std::pair<libobjgen::UUID, std::shared_ptr<libcomp::PersistentObject>>>
      worldObjects;
  ...
  while (nullptr != pImportObject) {
    ...
    auto typeHash =
        libcomp::PersistentObject::GetTypeHashByName(objectType, typeExists);

    if (!typeExists) {
      return libcomp::String("Failed to parse unknown object '%1'.")
          .Arg(objectType);
    }
    ...
    while (nullptr != pMember) {
      if ("uuid" == libcomp::String(pMember->Attribute("name")).ToLower()) {
    ...
    if (uuid.IsNull()) {
      return libcomp::String("Bad UUID '%1' for object '%2'")
          .Arg(uuidText)
          .Arg(objectType);
    }
    ...
    if (!obj || !obj->Load(doc, *pImportObject)) {
      return libcomp::String("Failed to load object '%1' with UUID %2.")
          .Arg(objectType)
          .Arg(uuid.ToString());
    }
    ...
    auto existingObject =
        libcomp::PersistentObject::LoadObjectByUUID(typeHash, db, uuid);

    if (existingObject) {
      return libcomp::String(
                 "Object with UUID '%1' already exists in database.")
          .Arg(uuid.ToString());
    }

    libcomp::String importError =
        CheckImportObject(objectType, obj, lobbyDB, worldDB);
  ...
  if (!lobbyDB->ProcessChangeSet(lobbyChangeSet)) {
    LogGeneralError([&]() {
      return libcomp::String("Import failed with lobby database error: %1\n")
  ...
  if (!worldDB->ProcessChangeSet(worldChangeSet)) {
    LogGeneralError([&]() {
      return libcomp::String("Import failed with world database error: %1\n")
...
libcomp::String LobbyServer::CheckImportObject(
  ...
  if ("Account" == objectType) {
    ...
    if (objects::Account::LoadAccountByUsername(lobbyDB,
                                                account->GetUsername()) ||
        objects::Account::LoadAccountByEmail(lobbyDB, account->GetEmail())) {
      return libcomp::String("Account '%1' exists").Arg(account->GetUsername());
    }
  ...
  if ("Character" == objectType) {
    ...
    if (objects::Character::LoadCharacterByName(worldDB,
                                                character->GetName())) {
      return libcomp::String("Character '%1' exists").Arg(character->GetName());
    }
  }
