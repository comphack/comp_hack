...
bool Parsers::EquipmentMod::Parse(
  ...
  if (modificationItem &&
    ...
    auto equipmentData =
        definitionManager->GetItemData(equipmentItem->GetType());
    ...
    uint8_t groupID =
        equipmentData->GetCommon()->GetCategory()->GetSubCategory();
    ...
    if (isWeapon) {
      ...
      auto modData =
          definitionManager->GetModificationDataByItemID(slotItem->GetType());
      auto triggerData = modData
                             ? definitionManager->GetModificationTriggerData(
                                   modData->GetEffectID())
                             : nullptr;
      auto effectData =
          modData
              ? definitionManager->GetModifiedEffectData(modData->GetEffectID())
              : nullptr;
      ...
      if (modData && triggerData && effectData && successScaleIdx < 8) {
        ...
        LogItemError([&]() {
          return libcomp::String(
                     "Invalid data encountered for weapon modification for "
                     "slot item: %1\n")
      ...
      auto recipeData = definitionManager->GetModificationExtRecipeDataByItemID(
          slotItem->GetType());
      auto effectData = recipeData
                            ? definitionManager->GetModificationExtEffectData(
                                  groupID, recipeData->GetSlot(),
                                  recipeData->GetEffectSubID())
                            : nullptr;
      ...
      if (recipeData && effectData && successScaleIdx < 8) {
        ...
        LogItemError([&]() {
          return libcomp::String(
                     "Invalid data encountered for equipment modification for "
                     "slot item: %1\n")
              .Arg(slotItem->GetType());
        });
    ...
    if (resultCode != RESULT_CODE_ERROR) {
      ...
      if (currentEffectID) {
        if (isWeapon) {
          auto currentEffectData =
              definitionManager->GetModifiedEffectData(currentEffectID);
          ...
          auto currentEffectData =
              definitionManager->GetModificationExtEffectData(groupID, slot,
                                                              currentEffectID);
          if (currentEffectData) {
            currentEffectType = currentEffectData->GetType();
            currentEffectSequenceID = currentEffectData->GetSequenceID();
      ...
      if (currentEffectType && currentEffectType == effectType) {
        if (effectSequenceID != currentEffectSequenceID + 1) {
          ...
          LogItemError([&]() {
            return libcomp::String(
                       "Invalid request to update modification effect %1 from "
                       "rank %2 to %3\n")
                .Arg(effectType)
        ...
        LogItemError([&]() {
          return libcomp::String(
                     "Invalid request to update modification effect %1 "
                     "directly to rank %2\n")
              .Arg(effectType)
              .Arg(effectSequenceID);
        });
    ...
    if (resultCode != RESULT_CODE_ERROR) {
      successRate =
          (int16_t)floor((float)successRate * (successScaling * 0.01f));
      if (successRate > 0 &&
          (successRate >= 10000 || RNG(int16_t, 1, 10000) <= successRate)) {
        resultCode = RESULT_CODE_SUCCESS;

        if (greatSuccessRate > 0 &&
            (greatSuccessRate >= 10000 ||
             RNG(int16_t, 1, 10000) <= greatSuccessRate)) {
          ...
          if (isWeapon) {
            auto nextEffectData =
                definitionManager->GetModifiedEffectData(nextEffectID);
            if (nextEffectData && nextEffectData->GetType() == effectType &&
                nextEffectData->GetSequenceID() ==
                    (int8_t)(effectSequenceID + 1)) {
        ...
        if (greatFailRate > 0 && (greatFailRate >= 10000 ||
                                  RNG(int16_t, 1, 10000) <= greatFailRate)) {
  ...
  if (resultCode != RESULT_CODE_ERROR) {
    ...
    stackAdjustItems[modificationItem] =
        (uint16_t)(modificationItem->GetStackSize() - 1);
  }
  ...
  switch (resultCode) {
    ...
    case RESULT_CODE_GREAT_SUCCESS:
      if (effectID != 0) {
        ...
        auto itemBox = std::dynamic_pointer_cast<objects::ItemBox>(
            libcomp::PersistentObject::GetObjectByUUID(
                equipmentItem->GetItemBox()));
        if (itemBox) {
          characterManager->SendItemBoxData(
              client, itemBox, {(uint16_t)equipmentItem->GetBoxSlot()});
        }
    ...
    case RESULT_CODE_GREAT_FAIL: {
      ...
      if (resultCode == RESULT_CODE_GREAT_FAIL &&
        ...
        characterManager->UpdateDurability(client, equipmentItem, -5000);

        resultCode = RESULT_CODE_FAIL;
      }

      if (destroyItem) {
        stackAdjustItems[destroyItem] =
            (uint16_t)(destroyItem->GetStackSize() - 1);
      }
    } break;
    default:
      break;
  ...
  if (stackAdjustItems.size() > 0) {
    ...
    if (resultCode == RESULT_CODE_GREAT_FAIL && greatFailItemType) {
      auto pieces =
          characterManager->GenerateItem(greatFailItemType, greatFailItemCount);
      if (pieces) {
  ...
  if (success) {
    ...
    server->GetEventManager()->UpdateDemonQuestCount(
        client, objects::DemonQuest::Type_t::EQUIPMENT_MOD,
